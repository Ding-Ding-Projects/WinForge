...
- The Command Palette uses a named shared `SearchPatternBox`, focuses its nested query editor on open, and reserves query-only `Enter` for launching a result; raw-pattern and sample Enter events stay inside the builder. Its validation, no-result, and matcher-error copy is exposed through the live `CommandPaletteSearchStatus` surface, and the search name refreshes with the active language. · Command Palette 使用具名共用 `SearchPatternBox`，開啟時 focus nested query editor，並只將 query editor 嘅 `Enter` 用嚟啟動結果；raw pattern 同 sample 嘅 Enter 會留喺 builder。驗證、冇結果同 matcher error 文案會經 live `CommandPaletteSearchStatus` surface 提供，而搜尋名稱會跟住目前語言更新。
