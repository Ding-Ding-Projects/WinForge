# System & Tweaks · 系統與調校

WinForge's System & Tweaks tools let you inspect, repair and fine-tune Windows from one place — registry, services, devices, monitors and more.
WinForge 嘅「系統與調校」工具，等你喺一個地方檢視、修復同微調 Windows——登錄檔、服務、裝置、監察等等。

## Dashboard · 概覽

Master search and home overview across every module.
跨晒所有模組嘅總搜尋同主頁概覽。

Open in-app: `WinForge.exe --page dashboard`

![Dashboard](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-dashboard.png)

## Registry Editor · 登錄編輯器

Browse and edit the live Windows registry (hives, keys, values); deletion notices reflect the actual registry write outcome.
瀏覽同編輯實時 Windows 登錄檔（hive、機碼、值）；刪除通知會反映真正嘅登錄檔寫入結果。

Open in-app: `WinForge.exe --page regedit`

![Registry Editor](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-regedit.png)

## System Doctors · 系統醫生

Guided Windows controls and repairs: Storage Sense, Filter Keys, association templates, Update pause/resume, driver backup/rollback, Autoruns impact, ResetBase, Store-app repair, spooler, DNS, taskbar, search, icons and more.
引導式 Windows 控制同修復：儲存空間感知、篩選鍵、關聯範本、更新暫停／恢復、驅動備份／回復、Autoruns 影響、ResetBase、商店 app 修復、列印、DNS、工作列、搜尋、圖示等。

[Full guided-maintenance guide · 完整引導式維護指南](#/wiki/Windows-Maintenance-Workflows)

Open in-app: `WinForge.exe --page doctors`

![System Doctors](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-doctors.png)

## Services · 服務

Start, stop and set the startup type of Windows services.
啟動、停止同設定 Windows 服務嘅啟動類型。

Open in-app: `WinForge.exe --page services`

![Services](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-services.png)

## Scheduled Tasks · 排程工作

View and run entries in the Windows Task Scheduler.
檢視同執行 Windows 排程工作。

Open in-app: `WinForge.exe --page tasks`

![Scheduled Tasks](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-tasks.png)

## Devices · 裝置

Enable, disable and inspect hardware devices and drivers.
啟用、停用同檢視硬件裝置同驅動程式。

Open in-app: `WinForge.exe --page devices`

![Devices](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-devices.png)

## ViVeTool · 功能旗標

Toggle hidden Windows feature flags via ViVeTool.
用 ViVeTool 切換隱藏嘅 Windows 功能旗標。

Open in-app: `WinForge.exe --page vivetool`

![ViVeTool](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-vivetool.png)

## Startup Apps · 開機程式

Manage logon and startup items that run at boot.
管理開機同登入時自動執行嘅項目。

Open in-app: `WinForge.exe --page startup`

![Startup Apps](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-startup.png)

## Environment Variables · 環境變數

Edit user/system environment variables with a dedicated PATH editor.
編輯使用者／系統環境變數，附專用 PATH 編輯器。

Open in-app: `WinForge.exe --page envvars`

![Environment Variables](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-envvars.png)

## Event Viewer · 事件檢視器

Read Windows system and application event logs.
讀取 Windows 系統同應用程式事件記錄。

Open in-app: `WinForge.exe --page events`

![Event Viewer](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-events.png)

## System Info (Winfetch) · 系統資訊

neofetch-style read-out of OS, CPU, GPU, memory and specs.
neofetch 風格顯示作業系統、CPU、顯示卡、記憶體等規格。

Open in-app: `WinForge.exe --page winfetch`

![System Info (Winfetch)](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-winfetch.png)

## System Monitor · 系統監察

Live CPU, RAM and network monitor with priority and affinity control.
即時監察 CPU、記憶體同網絡，可調優先權同核心親和性。

Open in-app: `WinForge.exe --page monitor`

![System Monitor](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-monitor.png)

## Process Explorer · 程序總管

Process-tree task manager with command line, threads and kill controls.
程序樹工作管理員，顯示命令列、執行緒，可結束程序。

Open in-app: `WinForge.exe --page procexp`

![Process Explorer](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-procexp.png)

## Battery & Thermal · 電池與散熱

Battery wear/health, temperatures, fan and powercfg energy report.
電池耗損健康、溫度、風扇同 powercfg 能源報告。

Open in-app: `WinForge.exe --page battery`

![Battery & Thermal](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-battery.png)

## Volume Mixer · 音量混合器

Per-app volume and mute control via Core Audio/WASAPI, with playback-device selection, master controls, default-device switching, and an explicit reset of each app to the system default. COM activation and returned interfaces are checked before use; invalid PIDs stop before activation, and aliases sharing one RCW release only through their owner. Unsupported per-app routing fails closed without changing another process. The device controls stack at narrow widths, while icon actions expose localized automation names, keyboard-operable sliders, 44–48 px targets, and full-title tooltips.

用 Core Audio／WASAPI 逐個應用程式調音量同靜音，亦可以揀播放裝置、控制主音量、切換預設裝置，同明確將每個 app 還原到系統預設。使用前會檢查 COM 啟動同回傳介面；無效 PID 喺 activation 前停止，共用同一 RCW 嘅 alias 只經 owner release。Windows 唔支援逐 app 路由時會安全停止，唔會改到其他程序。窄畫面會將裝置控制直向排列；圖示操作有本地化無障礙名稱、slider 可用鍵盤操作、44–48 px 點按範圍，同完整標題 tooltip。

Failure mode · 失敗模式：endpoint changes or build-specific undocumented routing can reject an operation; WinForge keeps the previous route and shows a clear result. No audio content, device identifier, or app route is transmitted or persisted by this page. · 裝置變更或者 Windows 版本特定嘅未公開路由可能拒絕操作；WinForge 會保留原本路由並清楚顯示結果。呢頁唔會傳送或者保存音訊內容、裝置識別碼同 app 路由。

Visual verification · 視覺驗證：fresh app-owned captures were inspected at 1264×791 and 784×691, alongside independent fresh-HWND 1280×800 / 800×700 frames and a 1033×637 driver capture. The header, wrapped bilingual description, stacked device controls, persistent error surface, and 44–48 px actions remain visible without truncation or overlap. The dedicated headless desktops had no playback endpoint, so the canonical image intentionally shows the fail-closed endpoint state rather than invented audio sessions. · 已檢視 1264×791 同 784×691 嘅最新 app-owned capture、獨立 fresh-HWND 1280×800／800×700 圖，同 driver 1033×637 圖；header、雙語換行說明、直排裝置控制、持續錯誤提示同 44–48 px 操作都冇截斷或重疊。專用 headless desktop 冇 playback endpoint，所以正式圖片刻意展示 fail-closed 狀態，唔會虛構 audio session。

Open in-app: `WinForge.exe --page mixer`

![Volume Mixer](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-mixer.png)

## Context Menu · 右鍵選單

Add and remove shell right-click verbs.
新增同移除右鍵選單動詞。

Open in-app: `WinForge.exe --page contextmenu`

![Context Menu](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-contextmenu.png)

## Explorer Right-Click · 檔案總管右鍵選單

Toggle native and PowerToys Explorer right-click integrations.
切換原生同 PowerToys 嘅檔案總管右鍵整合。

Open in-app: `WinForge.exe --page shellmenu`

![Explorer Right-Click](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-shellmenu.png)

## Nilesoft Shell · Nilesoft 右鍵選單

Modern, themeable, customizable context menu via Nilesoft Shell.
用 Nilesoft Shell 整現代、可換主題嘅自訂右鍵選單。

Open in-app: `WinForge.exe --page nilesoftshell`

![Nilesoft Shell](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-nilesoftshell.png)

## Awake · 保持喚醒

Keep the PC awake without changing power settings.
唔改電源設定都令電腦保持唔瞓。

Open in-app: `WinForge.exe --page awake`

![Awake](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-awake.png)

## Settings & Control Panel · 設定與控制台

In-app launcher for ms-settings pages and Control Panel applets.
app 內啟動 ms-settings 頁面同控制台小程式。

Open in-app: `WinForge.exe --page settingshub`

![Settings & Control Panel](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-settingshub.png)

## Native Utilities · 原生工具

Wi-Fi passwords, SMB shares, brightness, certificates, Bluetooth and more.
Wi-Fi 密碼、SMB 共享、亮度、憑證、藍牙等原生雜錦。

Open in-app: `WinForge.exe --page native`

![Native Utilities](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-native.png)

## PowerToys Extras · PowerToys 額外工具

Image Resizer, OCR text extractor and always-on-top helpers.
圖片縮放、OCR 文字擷取同視窗置頂工具。

Open in-app: `WinForge.exe --page powertoys`

![PowerToys Extras](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-powertoys.png)

## World Monitor · 世界監察

News, geopolitics, finance and instability-index intelligence dashboard.
新聞、地緣政治、金融同不穩定指數嘅情報儀表板。

Open in-app: `WinForge.exe --page worldmonitor`

![World Monitor](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-worldmonitor.png)

## Activity Timeline · 活動時間軸

Default-on foreground-window time tracking with local crash recovery and per-app usage insights.
預設開啟前景視窗時間追蹤，有本機閃退復原同逐個應用程式使用量。

Open in-app: `WinForge.exe --page timelens`

![Activity Timeline](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-timelens.png)

[← Wiki Home](Home.md)
