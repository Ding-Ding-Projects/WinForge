...
- Elevated process-owned UI Automation shell smoke: **68/68**, retaining Package Manager and Check Digit checks while adding every Text to Binary control/action, binary/hex conversion, stale-output clearing, explicit Copy, selected-base/input/output language-state retention, accessibility contract, and all aliases; this is not normal-integrity external-query evidence. · 提權、自有 process UI Automation shell 冒煙：**68/68**，保留套件管理同檢查碼檢查，再加入文字轉二進位全部 control／動作、二進位／十六進位轉換、清除舊輸出、明確 Copy、轉語言後保留已揀進位／輸入／輸出、無障礙合約同全部 alias；呢個唔係正常 integrity 外部查詢證據。
