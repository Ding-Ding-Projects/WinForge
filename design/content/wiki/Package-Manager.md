...
The C++20/C++/WinRT migration is honestly **in progress**, not a full UniGetUI clone, parity claim, or production cutover. The native shell exposes all nine views and the same 11 manager choices—WinGet, Scoop, Chocolatey, pip, npm, .NET tools, Windows PowerShell Gallery, PowerShell 7 PSResourceGet, Cargo, Bun, and vcpkg—but only three non-mutating result queries, one source-command probe, and preview-only Details/install/update/uninstall operation plans are active:

C++20／C++/WinRT 遷移會如實標示為**進行中**，唔係完整 UniGetUI 複製品、對等聲稱或者正式版切換。原生 shell 顯示全部九個檢視，同一組 11 個管理器——WinGet、Scoop、Chocolatey、pip、npm、.NET 工具、Windows PowerShell Gallery、PowerShell 7 PSResourceGet、Cargo、Bun 同 vcpkg——但目前只開放三條非修改結果查詢、一個來源指令探測，同只供預覽嘅詳細資料／安裝／更新／解除安裝操作計劃：

| View · 檢視 | Current native C++ truth · 目前原生 C++ 實況 |
|---|---|
| **Discover · 搜尋安裝** | Non-mutating search; per-row Details and Install create preview-only executable/argv plans in Operations. · 非修改搜尋；逐列詳細資料同安裝只會喺操作檢視建立 executable／argv 預覽計劃。 |
| **Updates · 可更新** | Non-mutating enumeration; per-row Details, per-row Update, and Update All create preview-only operation plans. · 非修改列出更新；逐列詳細資料、逐列更新同全部更新只會建立操作預覽計劃。 |
| **Installed · 已安裝** | Non-mutating enumeration; Details and Uninstall create preview-only operation plans. · 非修改列出已安裝套件；詳細資料同解除安裝只會建立操作預覽計劃。 |
...
| **Operations · 操作佇列** | Local probe/query history plus preview-only Details/install/update/uninstall plans; real execution, coordinator, cancel, reorder, and retry remain gated. · 本機探測／查詢歷史加只供預覽嘅詳細資料／安裝／更新／解除安裝計劃；真正執行、協調器、取消、重新排序同重試仍然鎖住。 |

All details, install, update, uninstall, source, and bulk execution remains separately gated. Bundles now has a native snapshot import/export preview, while the rest of the advanced bundle lifecycle remains gated. Native queries and Details/mutation previews use reviewed argument-vector builders, bounded parsers, allowlisted HTTPS endpoints, and a contained Win32 process runner. External manager commands fail closed while WinForge is elevated; no upstream UniGetUI executable or UI is launched.

所有詳細資料、安裝、更新、解除安裝、來源同批次執行仍然另外鎖住。原生查詢同詳細資料／修改預覽使用經審核嘅 argument-vector builder、有界解析器、准許清單 HTTPS endpoint 同受控 Win32 process runner。WinForge 提升權限時，外部管理器指令會 fail closed；完全唔會啟動上游 UniGetUI executable 或 UI。

## Evidence and blockers · 證據同阻礙

- The complete native test executable passes **260/260 in Debug and 260/260 in Release** (218 core route/package-manager checks plus 42 parser checks), including the operation-preview and Details-preview argv formatter assertions. · 完整原生測試 executable 喺 Debug **260/260**、Release **260/260** 通過（218 個 core route／package-manager 檢查加 42 個 parser 檢查），包括操作預覽同詳細資料預覽 argv formatter assertion。
...
Remaining UniGetUI-informed work includes the capability/maintenance/log model; full details execution/rendering, media/share, and open-location actions; durable queue ordering/history/retry; complete bundle/download/installed-detection interoperability; secure opt-in/elevation/batch consent; backup and per-manager settings; actionable notifications/tray commands; and authenticated API/CLI/headless/deep-link/widget surfaces. Linux- and macOS-only managers are outside this Windows product's scope.

尚欠嘅 UniGetUI 參考工作包括 capability／維護／日誌模型；完整詳細資料執行／顯示、媒體／分享同開啟位置動作；可保存佇列次序／歷史／重試；完整清單／下載／已安裝偵測互通；安全 opt-in／提升權限／批次同意；備份同逐管理器設定；可操作通知／系統匣指令；以及已驗證身份嘅 API／CLI／headless／deep-link／widget 介面。只供 Linux 同 macOS 嘅管理器唔屬於呢個 Windows 產品範圍。
