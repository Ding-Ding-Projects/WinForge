...
- Debug and Release native tests: **387/387** each (341 core route/package-manager checks + 46 parser checks), including **42/42** focused batch-coordinator checks and safe cached-Discover PCRE2 regex vectors. · Debug 同 Release 原生測試：各自 **387/387**（341 個 core route／package-manager 檢查加 46 個 parser 檢查），包括 **42/42** 個專項批次協調器檢查同安全已快取 Discover PCRE2 regex vectors。
- Elevated process-owned UI Automation smoke: **166/166**, including all eleven manager filters, cache-only regex/no-query assertions, batch-consent policy, and Package Manager horizontal-bound checks. · 提權、自有 process UI Automation smoke：**166/166**，包括全部十一個管理器篩選、只限快取 regex／唔開查詢 assertion、批次確認政策同 Package Manager 水平邊界檢查。
