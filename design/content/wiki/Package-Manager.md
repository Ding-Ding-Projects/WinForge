...
- The complete native test executable passes **233/233 in Debug and 233/233 in Release** (191 core route/package-manager checks plus 42 parser checks). Every prior Package Manager regression remains, including singular/plural alias-to-view routing; the additional checks cover Check Digit and Text to Binary. · 完整原生測試 executable 喺 Debug **233/233**、Release **233/233** 通過（191 個 core route／package-manager 檢查加 42 個 parser 檢查）。之前全部套件管理回歸都保留，包括單數／複數 alias 對應準確檢視，新增檢查就覆蓋檢查碼同文字轉二進位。
- The elevated, process-owned UI Automation shell passes **59/59**. Every prior Package Manager assertion remains for routing—including exact Discover/Updates/Installed alias selection—filters, view/state contracts, fail-closed elevation behavior, and accessibility; added assertions cover Check Digit and Text to Binary controls, including explicit Copy and selected-base state retention. It does not prove live external queries. · 提權、只控制自己 process 嘅 UI Automation shell **59/59** 通過；之前全部套件管理 assertion 都保留，涵蓋導覽（包括準確揀返 Discover／Updates／Installed alias）、篩選、檢視／狀態合約、fail-closed 提權行為同無障礙，新增 assertion 就覆蓋檢查碼同文字轉二進位 controls，包括明確 Copy 同已揀進位狀態保留。唔代表真實外部查詢已通過。
