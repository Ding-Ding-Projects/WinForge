...
The C++20/C++/WinRT migration is honestly **in progress**, not a full UniGetUI clone, parity claim, or production cutover. The native shell exposes all nine views and the same 11 manager choices—WinGet, Scoop, Chocolatey, pip, npm, .NET tools, Windows PowerShell Gallery, PowerShell 7 PSResourceGet, Cargo, Bun, and vcpkg—but only three non-mutating result queries, bounded read-only Details queries, local ignored/pinned/snoozed update filtering with configurable 1/7/14/30-day snoozes, one source-command probe, and preview-only install/update/uninstall operation plans are active:

C++20／C++/WinRT 遷移會如實標示為**進行中**，唔係完整 UniGetUI 複製品、對等聲稱或者正式版切換。原生 shell 顯示全部九個檢視，同一組 11 個管理器——WinGet、Scoop、Chocolatey、pip、npm、.NET 工具、Windows PowerShell Gallery、PowerShell 7 PSResourceGet、Cargo、Bun 同 vcpkg——但目前只開放三條非修改結果查詢、有界只讀詳細資料查詢、本機忽略／釘選／可設定 1／7／14／30 日暫停更新篩選、一個來源指令探測，同只供預覽嘅安裝／更新／解除安裝操作計劃：

| View · 檢視 | Current native C++ truth · 目前原生 C++ 實況 |
|---|---|
| **Discover · 搜尋安裝** | Non-mutating search; per-row Details runs a bounded read-only query, while Install creates a preview-only executable/argv plan in Operations. · 非修改搜尋；逐列詳細資料會執行有界只讀查詢，而安裝只會喺操作檢視建立 executable／argv 預覽計劃。 |
...
| **Installed · 已安裝** | Non-mutating enumeration; Details runs read-only, while Uninstall creates a preview-only operation plan. · 非修改列出已安裝套件；詳細資料只讀執行，而解除安裝只會建立操作預覽計劃。 |
...
All install, update, uninstall, source, and bulk execution remains separately gated. Bundles now has a native snapshot import/export preview, while the rest of the advanced bundle lifecycle remains gated. Native result queries, Details queries, ignored/pinned/snoozed update filtering, and mutation previews use reviewed argument-vector builders, bounded parsers, allowlisted HTTPS endpoints, and a contained Win32 process runner. External manager commands fail closed while WinForge is elevated; no upstream UniGetUI executable or UI is launched.

所有安裝、更新、解除安裝、來源同批次執行仍然另外鎖住。原生結果查詢、詳細資料查詢、忽略／釘選／暫停更新篩選同修改預覽使用經審核嘅 argument-vector builder、有界解析器、准許清單 HTTPS endpoint 同受控 Win32 process runner。WinForge 提升權限時，外部管理器指令會 fail closed；完全唔會啟動上游 UniGetUI executable 或 UI。

## Evidence and blockers · 證據同阻礙

- The complete native test executable passes **261/261 in Debug and 261/261 in Release** (219 core route/package-manager checks plus 42 parser checks), including the unsafe Details-query fail-closed assertion. · 完整原生測試 executable 喺 Debug **261/261**、Release **261/261** 通過（219 個 core route／package-manager 檢查加 42 個 parser 檢查），包括唔安全詳細資料查詢 fail-closed assertion。
...
Remaining UniGetUI-informed work includes the capability/maintenance/log model; parsed/rich details fields, scheduler/notification integration, media/share, and open-location actions; durable queue ordering/history/retry; complete bundle/download/installed-detection interoperability; secure opt-in/elevation/batch consent; backup and per-manager settings; actionable notifications/tray commands; and authenticated API/CLI/headless/deep-link/widget surfaces. Linux- and macOS-only managers are outside this Windows product's scope.

尚欠嘅 UniGetUI 參考工作包括 capability／維護／日誌模型；已解析嘅豐富詳細資料欄位、自訂暫停時間、排程／通知整合、媒體／分享同開啟位置動作；可保存佇列次序／歷史／重試；完整清單／下載／已安裝偵測互通；安全 opt-in／提升權限／批次同意；備份同逐管理器設定；可操作通知／系統匣指令；以及已驗證身份嘅 API／CLI／headless／deep-link／widget 介面。只供 Linux 同 macOS 嘅管理器唔屬於呢個 Windows 產品範圍。
