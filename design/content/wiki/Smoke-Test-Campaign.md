...
**Focused evidence · 專注證據：** `PackageManagerCore.Tests` passed **26/26**,
...
**63/63** plus its exit-code contract. A fresh self-contained publish launched
...
**專注證據：** `PackageManagerCore.Tests` **26/26** 通過，覆蓋空白預設相容、有效
...
contract。新嘅 self-contained publish 成功開到 `--page packages`
