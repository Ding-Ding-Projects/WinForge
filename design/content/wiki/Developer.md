...
The review-first workbench handles ports, per-shell Node, Corepack, Defender folders, TCP tuning, and measured cache cleanup. See [Developer Workflow Workbench](#/wiki/Developer-Workflow-Workbench). · 先審閱工作台處理 ports、每 shell Node、Corepack、Defender 資料夾、TCP 調校同先量度後清快取；詳情請睇[開發工作流程工作台](#/wiki/Developer-Workflow-Workbench)。
...
The focused safety/session/surface contract is **33/33**. Eight core/common searches use the synchronized builder now; the exhaustive inventory keeps 64 ordinary searches and nine specialized dialect adapters explicitly open. See [Managed Regex Builder](#/wiki/Regex-Builder) for limits, failure modes, visual evidence, and the remaining project-wide work. · 專項安全／session／surface 合約係 **33/33**；八個核心／共用搜尋而家已用同步 builder，而完整清單明確保留 64 個一般搜尋同九個專用方言 adapter。上限、故障模式、視覺證據同餘下全 project 工作請睇[正式受管理正則砌法](#/wiki/Regex-Builder)。
...
## Independent native port · 獨立原生移植版

**EN —** The experimental C++20/C++/WinRT port and its toolchain have moved to [codingmachineedge/WinForge-Native](https://github.com/codingmachineedge/WinForge-Native). This developer guide covers the canonical .NET 11 / WinUI 3 application; native source, dependencies, tests, parity evidence, installer, and release procedures must be changed and verified in the independent repository.

**粵語 —** 實驗性 C++20/C++/WinRT 移植版同工具鏈已搬去 [codingmachineedge/WinForge-Native](https://github.com/codingmachineedge/WinForge-Native)。呢份開發者指南只涵蓋正式 .NET 11／WinUI 3 app；原生 source、dependency、tests、parity 證據、installer 同 release 程序要喺獨立 repository 改同驗證。
