...
- The 8 remaining gaps are confined to Windows 11 and Maintenance. · 8 項餘下缺口只喺 Windows 11 同維護。

## Detailed evidence · 詳細證據

Read the repository's categorized `docs/audits/roadmap-core-capability-audit-2026-07-24.md` ledger. It names the actual catalog IDs, page handlers, service methods, command/registry mechanisms, documentation evidence, and the precise missing behavior for every unchecked item.
...
Developer & Terminal, Home Assistant, and Archives also have changed live surfaces. Fresh headless and canonical driver captures are linked from their focused workflow pages, with no mutating action exercised. · 三個頁面都有最新 headless 同正式 driver 畫面，擷取期間冇執行任何修改動作。
