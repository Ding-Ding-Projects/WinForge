...
LowLevel MCP headless tools were not callable in this session; the screenshot is the repository driver's DEBUG live WinUI visual-tree capture, not a raw desktop image. Full configuration, failure modes, and security details are in the repository's `docs/features/media-capture/media-studio-workflows.md`. · 今次 LowLevel 工具不可呼叫，圖片係 repo driver 嘅 DEBUG live visual-tree 擷取；完整設定／失敗模式／安全資料請睇 repository guide。
