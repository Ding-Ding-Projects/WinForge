...
`gh release create --latest` nevertheless left `/releases/latest` at historical managed `v1.0.256`; official manual `gh release edit native-v1.0.44 --latest` repaired it to `native-v1.0.44`. The corrective branch is adding explicit release edit, fail-closed Latest/asset verification, and official Node 24 `checkout@v7`, `upload-artifact@v7`, `download-artifact@v8`, and `setup-msbuild@v3`. Exact corrective branch/main run IDs, tags, and SHAs remain pending; automatic Latest is not yet proven. Legacy managed workflow `301226619` remains disabled, exactly 28 obsolete failed/cancelled no-release run records were deleted, and successful history/releases remain.

**粵語.** 三項文字分析已有專用 C++/WinRT renderer 同 `TextAnalysis` core；Renderer 計 **27/346**，仲有 **319** 條固定 route 同五組動態家族。完整 shell **388/388（388 passed、0 failed）** 已通過，visual 仍係 `capture-blocked`。功能 `fc2b76e5` 嘅 branch run 29673079883 發佈 exact-SHA `native-v1.0.43` prerelease；merge `f7a9eec4` 嘅 `main` run 29673310778 發佈 exact-SHA stable `native-v1.0.44`，兩者都準確只有 setup 同 portable ZIP，獨立審查亦證實 292 個 ZIP entry、48 個原生 PE、零 CLR／apphost／禁止 artifact。`gh release create --latest` 冇移動 Latest endpoint，要用官方 `gh release edit native-v1.0.44 --latest` 手動修復；修正 workflow 嘅準確 hosted 自動證明仍待完成。
...
- CI: `.github/workflows/native-release.yml` is the sole publisher and emits exactly the C++ portable ZIP plus `WinForge-Native-Setup.exe`. Branch pushes publish native prereleases. Current-main stable/Latest selection is being hardened with explicit release edit and fail-closed Latest/asset verification; exact corrective hosted proof is pending because `gh release create --latest` alone did not move the endpoint during run 29673310778.
