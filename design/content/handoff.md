...
**EN.** `percent`, `percentage`, and `module.percentcalc` are now dependency-free standard-C++ `PercentCalc` logic plus a genuine C++/WinRT renderer. Six cards preserve the managed percentage, change, tip, and ratio workflows, managed-compatible parsing/Unicode trimming/rounding, language retention, route reset, accessibility, and explicit-only Copy. Debug/Release 0 errors, cores **915/915** including Percentage **37/37**, focused UIA **14/14**, catalog parity/installer contract, and **38/346** accounting are green. LowLevel MCP is not callable, but a valid 1962×1311 driver PrintWindow capture was inspected and promoted; visual is `pass`. The C++-only publisher is test-gated/retry-hardened, with earlier hosted API-outage repair pending after the controlled push.

**粵語.** 三個 alias 而家係唔靠依賴嘅標準 C++ `PercentCalc` 加真正 C++/WinRT renderer。六張卡保留 managed 百分比、變化、貼士同比例流程、相容解析／Unicode 修剪／取捨、語言保留、route 重設、無障礙同只限明確 Copy。Debug／Release 0 errors、core **915/915**（Percentage **37/37**）、UIA **14/14**、catalog parity／installer contract 同 **38/346** 都通過。LowLevel MCP 未可呼叫，但有效 driver PrintWindow 截圖已檢視並升格；visual 係 `pass`。唯一 C++ publisher 已有測試 gate／retry 加固，controlled push 後要 repair 較早 hosted API outage。
...
**粵語.** `bmi`、`health` 同 `module.bmi` 而家已整合成標準 C++ `Bmi` core 加真正 C++/WinRT 健康計算器 renderer。Debug／Release 0 errors、core 各 **842/842**（BMI **14/14**）、BMI UIA **14/14**、catalog parity 346+5（319 registry／346 ledger）同 installer contract 都通過；計數係 **35/346**（`35 in-progress / 311 not-started`）。LowLevel MCP 不可呼叫、driver 拒絕空白 fallback，冇升格 PNG，visual 保持 `capture-blocked`；只限 C++ release 界線冇變。
...
## Historical Native Health Calculators feature handoff (pre-integration) · 歷史原生健康計算器功能交接（整合前）

**EN.** `bmi`, `health`, and `module.bmi` now use a genuine C++/WinRT Health Calculators page backed by pure standard-C++ `Bmi` logic. Managed BMI WHO bands, Mifflin–St Jeor BMR, five TDEE factors, US Navy body-fat validation/formulae, raw unit relabelling, language retention, lifecycle reset, and no clipboard mutation are preserved. Debug/Release builds are 0 errors; core is **829/829** in each configuration (BMI **14/14**); catalog parity and installer contract pass; and focused UIA is **14/14** across all aliases. LowLevel MCP is not callable here; the required driver rejected a blank fallback after `CopyFromScreen` was unavailable, leaving no PNG or process, so visual evidence is `capture-blocked`. This isolated handoff changes no workflow, release, GitHub, `main`, or remote branch.

**粵語.** `bmi`、`health` 同 `module.bmi` 而家用純標準 C++ `Bmi` logic 開真正 C++/WinRT 健康計算器頁。managed BMI WHO 分級、Mifflin–St Jeor BMR、五個 TDEE 系數、美國海軍體脂驗證／公式、原始單位重標籤、語言保留、lifecycle reset 同冇剪貼簿改動都有保留。Debug／Release 0 errors、core 各 **829/829**（BMI **14/14**）、catalog parity、installer contract 同全部 alias UIA **14/14** 已通過。今個 session 冇可呼叫 LowLevel MCP；`CopyFromScreen` 唔可用後 driver 拒絕空白 fallback，冇 PNG／冇 process，所以 visual 係 `capture-blocked`。呢個孤立 handoff 冇改 workflow／release／GitHub／`main` 或 remote branch。

**Hosted proof / Hosted 證明：** Unit Price merge `37cc0e8a1d4605864756751265d379a954978b27` is an ancestor of `origin/main`. [Native run 29706847786](https://github.com/codingmachineedge/WinForge/actions/runs/29706847786) passed and published stable `native-v1.0.76` exactly at that SHA with only the native setup and ZIP. Site-data then committed `fbaff0788`; dispatched [native run 29707001900](https://github.com/codingmachineedge/WinForge/actions/runs/29707001900) passed and made `native-v1.0.77` Latest at that exact current-main SHA, again with exactly those two native assets. · Unit Price merge 係 `origin/main` 祖先；native run 29706847786 通過並準確出 stable `native-v1.0.76`，只有原生 setup 同 ZIP。site-data 後 `fbaff0788` 嘅 dispatch run 29707001900 通過，將 `native-v1.0.77` 設為 Latest，仍然準確只有兩個原生 asset。
...
**EN.** `baseconvert` and `module.baseconvert` are integrated as a genuine local C++/WinRT renderer over a dependency-free standard-C++ arbitrary-precision core. Debug/Release builds have 0 errors; both combined cores are **857/857** (Base Converter **15/15**); focused UIA is **14/14** across both aliases; catalog parity is 346+5 with 319 registry and 346 ledger rows; the native installer contract passes; and accounting is **36/346** (`36 in-progress / 310 not-started`). The page also shares managed Unicode `Trim()` diagnostics. LowLevel MCP is not callable in this session; the fresh current driver rejected the blank/near-uniform PrintWindow client after CopyFromScreen was unavailable, so no PNG or process remained and visual evidence is `capture-blocked`. The integration changes no workflow or release policy; controlled push/release verification is next.

**粵語.** `baseconvert` 同 `module.baseconvert` 已整合成真正本機 C++/WinRT renderer 同唔靠依賴嘅標準 C++ 任意精度 core。Debug／Release 0 errors；合併 core 各 **857/857**（Base Converter **15/15**）；兩個 alias UIA **14/14**；catalog parity 346+5（319 registry／346 ledger）；native installer contract 通過；計數係 **36/346**（`36 in-progress / 310 not-started`）。頁面亦同 managed Unicode `Trim()` 診斷對等。今個 session LowLevel MCP 不可呼叫；最新目前 driver 因 CopyFromScreen 不可用而拒絕空白／近乎單色 PrintWindow client，冇 PNG／冇 process，所以 visual 係 `capture-blocked`。整合冇改 workflow 或 release policy；下一步係受控 push／release 驗證。
