...
The combined branch is rebuilt, republished, audited, visually inspected on a fresh dedicated LowLevel desktop, pushed, and exact-tip remote-proved before handoff. Final `main` integration and hosted release observation remain with the coordinating root agent; both preserved stashes remain unchanged. · 合併分支會喺交接前重新 build／publish／audit、用全新 LowLevel 專用 desktop 檢視、push 同 exact-tip remote proof；最終 `main` integration 同 hosted release 觀察交畀 root agent，兩個保留 stash 維持原封不動。
