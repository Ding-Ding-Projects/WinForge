...
Fresh native Dashboard, All Apps, About, Package Manager (`module.packages#updates`), Check Digit (`checkdigit`), Text to Binary (`binarytext`), and Case Converter (`caseconvert`) captures are blocked in this desktop session. Case Converter was also attempted through a LowLevel headless desktop, whose HWND capture showed only the title bar and a blank client frame. `CopyFromScreen` is unavailable and `PrintWindow` returns a blank/near-uniform WinUI client frame, which the driver rejects. No blank, stale, synthetic, or managed screenshot was substituted. UI Automation proves launch and behavior, not visual appearance; the blocked surfaces remain `capture-blocked`.

呢個 desktop session 擷取唔到最新原生 Dashboard、所有 app、About、套件管理（`module.packages#updates`）、檢查碼（`checkdigit`）、文字轉二進位（`binarytext`）同大小寫轉換（`caseconvert`）畫面。大小寫轉換亦經 LowLevel 無頭 desktop 嘗試過，HWND 擷取只有 title bar 同空白 client frame。`CopyFromScreen` 用唔到，而 `PrintWindow` 只回傳空白／接近單色 WinUI client frame，driver 而家會拒絕呢種圖。冇用空白、舊、合成或者受控版截圖頂替。UI Automation 只證明 launch 同 behavior，唔代表視覺通過；相關介面仍然係 `capture-blocked`。
