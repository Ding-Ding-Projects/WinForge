...
- Native test executable: **313/313 in Debug and 313/313 in Release** (267 core route/package-manager/codec/Roman checks plus 46 parser checks), retaining every Package Manager/shell regression plus focused Check Digit, Text to Binary, Case Converter, GUID Generator, codec, and Roman Numerals cases. Linked managed-reference regressions pass Check Digit **24/24** and Text to Binary **18/18**; they are reference tests, not an automated native-vs-managed process comparison. · 原生測試 executable：Debug **313/313**、Release **313/313**（267 個 core route／package-manager／codec／羅馬數字檢查加 46 個 parser 檢查），保留全部套件管理／shell 回歸，再加檢查碼、文字轉二進位、大小寫轉換、GUID 產生器、編解碼同羅馬數字專項案例。連結嘅受控參考回歸通過檢查碼 **24/24** 同文字轉二進位 **18/18**；佢哋係參考測試，唔係自動原生對受控 process 比較。
- Elevated process-owned shell smoke: **141/141**, retaining Package Manager, Check Digit, Text to Binary, Case Converter, GUID Generator, codec, and Roman Numerals coverage, including standard/extended/canonical-parenthesis conversion, copy status, language-state retention, accessibility, and horizontal-bound checks. This verifies UI and fail-closed safety behavior, not normal-integrity external queries. · 提權、自有 process shell 冒煙：**141/141**，保留套件管理、檢查碼、文字轉二進位、大小寫轉換、GUID 產生器、編解碼同羅馬數字覆蓋，包括標準／擴充／標準括號轉換、複製狀態、轉語言後狀態保留、無障礙同水平邊界檢查。呢項驗證 UI 同 fail-closed 安全行為，唔代表正常 integrity 外部查詢。
...
Fresh native Dashboard, All Apps, About, Package Manager (`module.packages#updates`), Check Digit (`checkdigit`), Text to Binary (`binarytext`), Case Converter (`caseconvert`), and Roman Numerals (`romannum`) captures are blocked in this desktop session. Roman Numerals was also attempted through a LowLevel headless desktop, whose inspected HWND capture showed only the title bar and a blank client frame. `CopyFromScreen` is unavailable and the driver's `PrintWindow` returns a blank/near-uniform WinUI client frame, which the driver rejects. No blank, stale, synthetic, or managed screenshot was substituted, and no `screenshot-romannum.png` was created or reused. UI Automation proves launch and behavior, not visual appearance; the blocked surfaces remain `capture-blocked`.

呢個 desktop session 擷取唔到最新原生 Dashboard、所有 app、About、套件管理（`module.packages#updates`）、檢查碼（`checkdigit`）、文字轉二進位（`binarytext`）、大小寫轉換（`caseconvert`）同羅馬數字（`romannum`）畫面。羅馬數字亦經 LowLevel 無頭 desktop 嘗試過，檢查過嘅 HWND 擷取只有 title bar 同空白 client frame。`CopyFromScreen` 用唔到，而 driver 嘅 `PrintWindow` 只回傳空白／接近單色 WinUI client frame，driver 而家會拒絕呢種圖。冇用空白、舊、合成或者受控版截圖頂替，亦冇建立或者重用 `screenshot-romannum.png`。UI Automation 只證明 launch 同 behavior，唔代表視覺通過；相關介面仍然係 `capture-blocked`。
