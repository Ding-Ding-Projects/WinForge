...
- **UI:** build **rich, control-based, animated UI** from real controls (ToggleSwitch/Slider/ComboBox/editors), mirroring `Pages/AwakeModule`. **Do NOT use `Controls/TweakCard`** — it is being phased out (goal: ZERO tweak cards; see `docs/backlog.md`). To render a set of `TweakDefinition`s, use control rows (Action→Button awaiting `RunAsync`, Toggle→ToggleSwitch, Choice→ComboBox, Slider/Number→Slider/NumberBox, Info→refreshable TextBlock) with one persistent `InfoBar` and an `EntranceThemeTransition` — see the converted `Pages/CloudflareModule`/`Pages/ArchivesModule` as the pattern. Use `Services/FileDialogs.cs` (never WinRT pickers). WebView2 is available. Pitfalls: classic `{Binding}` not `x:Bind` in DataTemplates; qualify `Windows.UI.Color`; use `ScrollViewer.*ScrollBarVisibility` attached props; a NAMED `Loc.I.LanguageChanged` handler unsubscribed on `Unloaded` (never an inline lambda — it leaks).
