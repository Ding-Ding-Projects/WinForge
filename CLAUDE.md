...
## What this is
WinForge (a.k.a. 視窗調校) is an all-in-one, **fully bilingual (English + 繁體中文/粵語)** Windows 11 control center — ~277 in-app modules plus a large Windows-tweak catalog — with a **hyper-realistic nuclear-reactor simulator** as its flagship. **.NET `net11.0-windows10.0.26100.0`, WinUI 3**, self-contained, unpackaged.
...
- **Easiest path — use the skill `.claude/skills/run-winforge/`** (SKILL.md + driver.ps1): builds-if-needed, launches any page via `--page <alias>`, screenshots the window. e.g.
  `powershell -ExecutionPolicy Bypass -File .claude/skills/run-winforge/driver.ps1 -Page reactor -Out shot.png`
- **Reactor engine tests (headless):** `dotnet run --project tests/ReactorSim.Tests` (62/62).
...
- **UI:** build **rich, control-based, animated UI** from real controls (ToggleSwitch/Slider/ComboBox/editors), mirroring `Pages/AwakeModule`. **`Controls/TweakCard` is FULLY REMOVED FROM USE (zero tweak cards)** — never reintroduce it; it remains only as (now-dead) definition files pending deletion. To render a list of `TweakDefinition`s, use **`Controls/ControlRowList`** (`SetTweaks(IEnumerable<TweakDefinition>)` / `Clear()`) — it renders control rows (Action→Button awaiting `RunAsync`, Toggle→ToggleSwitch, Choice/RadioGroup→ComboBox/RadioButtons, Slider/Number→Slider/NumberBox, Info→refreshable TextBlock) with one persistent `InfoBar` + `EntranceThemeTransition`. For a bespoke module see the converted `Pages/RainmeterModule`/`Pages/CloudflareModule`; for catalog-driven pages see `Pages/SettingsHubModule` using `ControlRowList`. Use `Services/FileDialogs.cs` (never WinRT pickers). WebView2 is available. Pitfalls: classic `{Binding}` not `x:Bind` in DataTemplates; qualify `Windows.UI.Color`; `RadialGradientBrush.GradientStops` is read-only (Add, don't assign); use `ScrollViewer.*ScrollBarVisibility` attached props; a NAMED `Loc.I.LanguageChanged` handler unsubscribed on `Unloaded` (never an inline lambda — it leaks).
...
- **P2 reactivity calibration — resolved & test-verified.** A fresh fully-rodded cold core is **subcritical (−1018 pcm)**; startup is stable (backward-Euler, no sign oscillation), held in MODE 5 until the operator starts it up, and does not run away or melt. The `tests/ReactorSim.Tests` suite asserts exactly this ("fresh fully-rodded core stays subcritical", "no runaway", "no meltdown on startup") and is **62/62 green**. Background in `docs/reactor-realism-review-001.md`.
