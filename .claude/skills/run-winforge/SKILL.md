...
WinForge is the canonical **.NET 11 WinUI 3 app** in `WinForge.csproj`. The PowerShell driver — **`.claude/skills/run-winforge/driver.ps1`** — publishes it self-contained, deep-links a requested module, and captures the dedicated process window. Treat an output PNG as visual evidence only after inspecting it. All paths below are relative to the repo root.
