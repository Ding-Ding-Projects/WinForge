...
- Managed release assets must stay compatible with `Services/AppUpdateService.cs`, including unsigned Squirrel.Windows `Setup.exe`, `RELEASES`, versioned package assets, and GitHub-provided SHA-256 digest verification. · 正式 release asset 要同 updater 合約相容，包括 unsigned Squirrel.Windows `Setup.exe`、`RELEASES`、有版本號 package 同 GitHub SHA-256 digest 驗證。
