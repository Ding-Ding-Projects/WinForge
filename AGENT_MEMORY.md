...
- Native Percentage Calculator now owns `percent`, `percentage`, and `module.percentcalc` through standard-C++ `PercentCalc` plus a C++/WinRT renderer. It preserves six managed calculation cards, active-culture/invariant numeric parsing, six-place away-from-zero output, banker-rounded tip splits, guarded errors, route lifecycle/reset, and explicit-only Copy. Current branch evidence: Debug/Release native builds 0 errors; core **796/796** in each configuration including Percent Calculator **37/37**; catalog parity 346+5; and focused UIA **13/13** across all aliases. `HasNativeRenderer`/ledger are **31/346** (**31 `in-progress` / 315 `not-started`**). LowLevel MCP is not callable; the required driver rejected a blank/near-uniform fallback after `CopyFromScreen` was unavailable, retained no PNG, and changed no canonical screenshot, so visual is `capture-blocked`. · 原生百分比計算器而家由標準 C++ `PercentCalc` 同 C++/WinRT renderer 處理 `percent`、`percentage` 同 `module.percentcalc`；保留六張 managed 計算卡、目前文化／invariant 解析、六位向遠離零輸出、小費 banker's rounding、受保護錯誤、route lifecycle/reset 同只限明確 Copy。今次分支證據係 Debug／Release 0 errors、core 各 **796/796**（包括 **37/37**）、catalog parity 346+5、三個 alias UIA **13/13**；Renderer／ledger 係 **31/346**（**31 `in-progress`／315 `not-started`**）。今次冇 LowLevel MCP，driver 因 `CopyFromScreen` 唔可用而拒絕空白／近乎單色 fallback，冇 PNG／冇 canonical 截圖變更，所以 visual 係 `capture-blocked`。
