...
- **Delivery boundary / 交付界線：** the audit and memory changes live on `codex/roadmap-reconcile-core`, rebased onto coordinating baseline `4af5e60e9` before push. The branch is intentionally left unmerged for the coordinating parent; exact remote-tip proof is required before handoff and no `main` merge is claimed here. · Audit 同 memory 改動喺 `codex/roadmap-reconcile-core`，push 前已 rebase 到統籌基線 `4af5e60e9`。分支刻意留畀統籌 parent 合併，交接前要做準確 remote-tip proof，呢度唔聲稱已 merge `main`。
