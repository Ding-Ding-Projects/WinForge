...
- **Git state and preserved release records / Git 狀態同保留 release 記錄：** after ancestry proof, the repository has one primary checkout, one local `main`, one remote `main`, no linked worktrees, no stashes, and no task branches. The earlier `v1.1.344` stable record and the `v1.1.345`–`v1.1.347` prerelease records, including the branch-era `v1.1.349` record with placeholder timing, remain immutable audit history; `v1.1.348` is the verified stable latest release. · 完成 ancestry proof 後，repository 只剩一個 primary checkout、一個 local `main`、一個 remote `main`，冇 linked worktree、stash 或 task branch。之前嘅 `v1.1.344` stable record 同 `v1.1.345`–`v1.1.347` prerelease record，連同有 placeholder timing 嘅 branch-era `v1.1.349`，全部保留做 immutable audit history；`v1.1.348` 係已驗證嘅 stable latest release。
