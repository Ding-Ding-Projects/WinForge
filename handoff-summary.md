...
- **Artifact evidence / 資產證據：** the pre-handoff source-matched `1.1.331` probe produced unsigned `Setup.exe`, `RELEASES`, `WinForge-1.1.331-full.nupkg`, and `WinForge-portable-x64-1.1.331.zip`; the final release build must be regenerated from the commit containing this handoff record so the manifest source SHA and release assets remain exact. · 交接記錄之前嘅 source-matched `1.1.331` probe 已產生 unsigned 資產；正式 release build 要由包含呢段 handoff 嘅 commit 重新生成，確保 manifest source SHA 同 release assets 完全一致。
...
- **External evidence boundary / 外部證據界線：** the WinForge task issue is #16 and the rolling General Discussion is #17. GitHub Actions, the final release record, GitHub Wiki synchronization, Pages deployment result, and post-integration ancestry/cleanup proof remain to be recorded after the final source-matched build. The permitted low-level headless binding was unavailable; the repository driver provided the inspected off-screen captures, and the real QR-generated state was not promoted as visual evidence. · WinForge task issue 係 #16，rolling General Discussion 係 #17；GitHub Actions、正式 release record、GitHub Wiki、Pages deployment result 同整合後 ancestry／cleanup proof 要喺 final source-matched build 後記錄。許可嘅 low-level headless binding 不可用；repo driver 提供咗 off-screen capture，但真 QR-generated state 冇冒充做 visual evidence。
