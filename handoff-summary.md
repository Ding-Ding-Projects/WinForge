...
- **Scope / 範圍：** Core Audio activation, interface casts, names, selected Peek items, and per-app route targets are now nullable-safe and fail closed. Resetting a session to “System default” clears its override explicitly. The mixer device controls stack at narrow widths; icon actions, sliders, and default-device controls expose localized automation names/help, full-title tooltips, and 44–48 px targets. · Core Audio 啟動、介面 cast、名稱、Peek 選取項目同逐 app route target 已 nullable-safe 同 fail closed；還原「系統預設」會明確清除 override。窄畫面裝置控制會直排；圖示、slider 同預設裝置控制有本地化 automation 名稱／說明、完整標題 tooltip 同 44–48 px target。
- **Capture safety / 截圖安全：** the AWS-only automation hook was generalized into a DEBUG-only shell capture service with absolute `.png`, delay, and optional-size bounds. PNGs encode already-composited pixels as opaque, preserving translucent M3 surfaces. The driver creates a unique request, restores its environment, validates dimensions/color diversity, cleans its temporary image, and stops only its owned process. A live check exposed that legacy `CopyFromScreen` could capture an overlapping Codex window; that invalid local artifact was immediately overwritten with the validated mixer frame, and `CopyFromScreen` was removed. The driver now prefers the app-owned visual tree and permits only targeted `PrintWindow` fallback. · AWS-only hook 已變成 DEBUG-only shell capture service，有絕對 `.png`、delay 同可選尺寸限制；PNG 用不透明合成 pixels，正確保留半透明 M3 surface。Driver 建立唯一 request、還原 environment、驗證尺寸／色彩、清理 temp 圖，只停止自家 process。即時測試發現舊 `CopyFromScreen` 會影到遮住 WinForge 嘅 Codex 視窗；無效本機檔即時用經驗證 mixer 圖覆蓋，並已移除 `CopyFromScreen`。而家先用 app-owned visual tree，後備只限 targeted `PrintWindow`。
- **Verification / 驗證：** self-contained Debug publish completed with zero compiler warnings; XAML literal safety passed; the source audit resolved **2,870/2,870** handlers and **1,919/1,919** direct actions with zero mismatches or actionable markers. Fresh LowLevel captures at **1284×811** and **784×691**, plus a separate safe-driver **1033×637** capture, were inspected. The header, bilingual wrapping, vertical device controls, error surface, and actions have no clipping, overlap, or off-screen content. The dedicated desktops had no playback endpoint, so the fail-closed error is expected and no synthetic session was introduced. · Self-contained Debug publish 零 compiler warning；XAML safety 通過；source audit 2,870／2,870 handlers、1,919／1,919 direct actions 全 resolve，零 mismatch／marker。已檢視 LowLevel 1284×811、784×691，同 safe-driver 1033×637 圖；header、雙語換行、直排裝置控制、錯誤 surface 同操作冇裁切／重疊／走出畫面。專用 desktop 冇播放 endpoint，所以 fail-closed 錯誤屬預期，冇加入虛構 session。
- **Delivery / 交付：** implementation, current canonical screenshot, categorized feature/security docs, wiki, Pages mirror, roadmap, and persistent memory are prepared together on `codex/nullability-audio`. Final branch push, `main` integration, remote proof, hosted workflows, and cleanup remain with the coordinating integration sequence. · 實作、最新正式截圖、分類功能／安全文件、wiki、Pages mirror、roadmap 同持久記憶已一齊準備喺 `codex/nullability-audio`；最終 branch push、`main` 整合、remote proof、hosted workflow 同清理會由統籌整合流程完成。
