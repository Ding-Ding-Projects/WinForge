...
## Current 2026-07-24 preserved Package Manager stash recovery — audited and hardened · 2026-07-24 保留套件管理 stash recovery — 已審核同強化

- **Scope and provenance / 範圍同來源：** preserved commit `5cc3aa712f9e326dd8d9ae0bdd4c16d8771e1cb6` (`codex-preserve-unrelated`) was compared read-only with first parent `27f343be170c43675e4a97f3de152eafb6c99e20`. Its exact 10-file, +987/−217 package patch was not blindly applied: current main already contains its useful app-global scheduler, native provenance/source confirmation, secure bundle compatibility, manager discovery/update, configured executable/PowerShell host, WinGet detection, source-validation, and license/provenance behavior, usually with later queue/source/cancellation hardening. The external UniGetUI launcher, credential-in-URL path, strip-only sanitization, fixed builders, and automatic source trust remain rejected. The stash itself was not applied, popped, dropped, or mutated. · 保留 commit 已同第一個 parent 做只讀比較；十個檔案 +987/−217 patch 冇盲目套用。Current main 已有有用嘅 app 級排程、原生來源證明／來源刪除確認、安全清單相容、管理器搜尋／更新、自訂 executable／PowerShell host、WinGet 探測、來源驗證同 license 行為，而且通常已有更新 queue／source／cancel 強化。外部 UniGetUI launcher、URL 內嵌認證、只刪字元嘅 sanitization、固定 builder 同自動信任來源繼續拒絕；stash 本身冇 apply／pop／drop／修改。
- **Security and correctness / 安全同正確性：** new `PackageManagerInputPolicy` treats proxy authority and vcpkg triplet as structured command-bound values. Proxy accepts only a bounded credential-free HTTP(S) authority and rejects paths, queries, fragments, control characters, raw percent expansion, and command syntax; triplets are bounded ASCII tokens. Persistence and command construction both fail closed, inline bilingual errors restore the last valid value, and Settings no longer collects credentials it cannot safely use. Detected legacy values stay DPAPI-protected until the user explicitly forgets/resets them and never enter URLs, previews, or process arguments. · 新 policy 將 proxy authority 同 vcpkg triplet 當成結構化指令界線；proxy 只接受有界、唔含認證 HTTP(S) authority，triplet 只接受有界 ASCII token。保存同建立指令兩邊都 fail closed，雙語 inline error 會還原上一個有效值；Settings 唔再收集無法安全使用嘅認證。偵測到嘅舊值保持 DPAPI 保護，直到用戶明確刪除／重設，而且永遠唔會進入 URL、預覽或者 process argument。
- **Accessibility and layout / 無障礙同版面：** the fixed eight-column toolbar and fixed-width settings dialog were replaced with responsive rows and bounded widths. Primary/dynamic actions and toggles use 44-pixel targets; headings, selection controls, and output have automation semantics; long manager/action/batch strips scroll; truncated row metadata has tooltips. · 固定八欄 toolbar 同固定闊度 settings dialog 已改成響應式行同有界闊度；主要／動態 action 同 toggle 用 44 像素 target，heading／選取控制／output 有 automation semantics，長管理器／action／batch 列可捲動，省略 row metadata 有 tooltip。
- **Verification / 驗證：** Package Manager core **29/29**; exact `dotnet build WinForge.sln -c Debug -p:Platform=x64` exit 0 with **0 errors**; exact self-contained publish exit 0; XAML literal safety pass; source-surface audit reports 336 XAML files, 2,875/2,875 handlers, 1,922/1,922 direct actions, zero lifecycle mismatches, and zero actionable markers. · 套件管理 core **29/29**、完整 solution build 零 errors、自包含 publish exit 0、XAML safety 通過；source audit 有 336 個 XAML、2,875/2,875 handler、1,922/1,922 direct action、零 lifecycle mismatch／actionable marker。
- **Visual evidence / 視覺證據：** the final self-contained `packages` page was opened on a dedicated `LowLevelCUHeadless` desktop and inspected at 1049×646 and 720×650. Both captures show the live bilingual page with responsive two-row controls and no overlapping/off-screen action controls; canonical normal and narrow images are tracked in `docs/`. The owned app window closed and the headless desktop was destroyed. The repository driver fallback first found no dedicated window; its longer retry reported success but captured unrelated desktop content, so that payload was rejected, overwritten, and never promoted. · 最終自包含 `packages` 頁喺專用 LowLevel desktop 以 1049×646 同 720×650 檢視；兩張都係 live 雙語頁，響應式兩行控制冇重疊／走出畫面 action control；正式 normal／narrow 圖已放入 `docs/`。自家 app 視窗同 headless desktop 已清理。Repo driver fallback 第一次搵唔到專用視窗；加長重試雖然報成功但影到不相關桌面內容，所以該 payload 已拒絕／覆寫，冇升格。
- **Delivery boundary / 交付界線：** the bounded work lives on `codex/integrate-stash-packages`, based on `origin/main` `e11419dddec0191defc561e96380b8e79002c5e5`. Its containing bilingual commit is pushed and exact-tip remote-proved before handoff; final `main` integration, hosted workflow/release observation, and removal of the deliberately preserved stash remain with the coordinating agent. · 有限工作喺 `codex/integrate-stash-packages`，base 係 `origin/main` `e11419dd`；包含呢份記錄嘅雙語 commit 會喺交接前 push 同做 exact-tip remote proof。最終合併 `main`、hosted workflow／release 觀察，同處理刻意保留 stash 交由統籌 agent。
