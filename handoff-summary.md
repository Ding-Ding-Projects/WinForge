...
- **Delivery state / 交付狀態：** work is on `codex/integrate-claude-remaining`; it is intentionally not merged into `main` by this subtask. The branch push and exact remote-tip proof are the remaining delivery actions after final generated-doc validation. · 工作喺 `codex/integrate-claude-remaining`，呢個 subtask 按界線唔會自行 merge `main`；最後生成文件驗證後會 push branch 同核對 remote tip。
