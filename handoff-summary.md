...
## 2026-09-18 primary checkout closeout · 2026-09-18 primary checkout 收尾

- **Scope / 範圍:** Only the primary `winforge` repository was owned for this closeout. Inventory and fetch completed first; fetched `origin/main` advanced from `552952ffd94fe4650efffd64366745bd877f0c0f` to `a21d7044`, and the clean primary checkout fast-forwarded without a merge conflict. · 今次收尾只處理 primary `winforge` repository；先完成 inventory 同 fetch；fetched `origin/main` 由 `552952ffd94fe4650efffd64366745bd877f0c0f` 前進到 `a21d7044`，clean primary checkout 無 merge conflict fast-forward。
- **Preservation / 保存:** The primary checkout had no uncommitted changes, and no linked worktrees, extra local or remote branches, or stashes were present. Therefore no half-finished path was found that needed a preservation commit, and no recoverable work remains only on disk. · Primary checkout 冇 uncommitted changes，亦冇 linked worktree、額外 local／remote branch 或 stash；所以冇 half-finished path 要做 preservation commit，冇 recoverable work 淨係留喺 disk。
- **Archive / 壓縮檔:** External archive `winforge-20260918T172422Z.7z` was created at the configured external backup location and passed `7z t`. It is 410,145,631 bytes with 7,664 entries, including 983 `.git` entries. The archive used Git’s tracked and non-ignored untracked path lists and excluded ignored paths. · External archive `winforge-20260918T172422Z.7z` 已寫入設定好嘅外部 backup 位置並通過 `7z t`；大小 410,145,631 bytes，共 7,664 entries，包括 983 個 `.git` entries；用 Git tracked 同 non-ignored untracked path list，ignored path 已排除。
- **Cleanup / 清理:** No linked checkout, branch, or stash met the safe-removal conditions, so no Mat Day removal was performed. The primary checkout and `main` remain retained. · 冇 linked checkout、branch 或 stash 符合安全移除條件，所以冇做 Mat Day removal；primary checkout 同 `main` 繼續保留。
- **Remote proof / 遙距證明:** The documentation commit from this closeout must be pushed and then verified with `git ls-remote` and ancestry checks before this handoff is considered complete. · 今次收尾嘅 documentation commit 要先 push，再用 `git ls-remote` 同 ancestry check 驗證，先算完成交接。
...
## Preserved layout-refactor resolution — 2026-08-20 · 已保存版面重構解決記錄

- **Recovered scope / 已恢復範圍:** audited preserved commit `4221670b9f981a2c90954493452e1782bfceb66c` against current `main`. Its useful intent was to avoid reproduced WinUI XAML parser failures, but the snapshot also deleted 621 lines from `AiAgentsModule.xaml.cs`, duplicated control resets throughout `AmmoniaPlantModule.xaml.cs`, and moved ordinary layout values into code-behind without a reproduced failure. Current `main` retains the complete AI Agents implementation and the ordinary declarative layout values. · 已逐項審核保存 commit；有用原意係避開已重現 WinUI XAML parser 問題，但 snapshot 同時刪走 AI Agents 621 行、喺 Ammonia Plant 多處重複重設 control，仲將冇重現失敗嘅一般版面值搬入 code-behind。目前 `main` 保留完整 AI Agents 實作同正常 declarative layout。
- **Semantic resolution / 語義解決:** keep the repository's narrow reproduced-literal policy: only confirmed failing `ToggleSwitch`, `CheckBox`, and `NumberBox` defaults move to managed initialization. The preserved experimental tree is intentionally not applied; its history is joined to `main` with a semantic merge so the work remains auditable without reintroducing deletions or live-state resets. · 保留 repository 只針對已重現 literal 嘅政策：淨係確認失敗嘅 ToggleSwitch、CheckBox 同 NumberBox default 搬去 managed initialization。保存實驗 tree 刻意唔套用；以語義 merge 接入 `main`，保留可審核歷史而唔重新引入刪除或即時狀態重設。
- **Verification / 驗證:** `Test-WinForgeXamlLiteralSafety.ps1` passes with zero direct XAML `IsOn` Boolean literals, 17 protected ToggleSwitch defaults, two protected IsChecked defaults, and 10 reproduced NumberBox defaults under managed initialization. A combined build attempt reached the 120-second harness ceiling without a verdict; that timeout is recorded separately and is not claimed as a compile failure or success. · XAML literal safety 通過；合併 build 嘗試撞到 120 秒 harness 上限而冇 verdict，呢個 timeout 會獨立記錄，唔會冒稱 build 成功或失敗。
