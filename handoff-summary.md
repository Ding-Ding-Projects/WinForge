...
- **Root cause and fix / 根因同修復：** the concrete process adapter decoded stderr and dispatched an empty `ErrorDataReceived` callback for every ffmpeg progress line. Under host contention, 10,000 diagnostic lines could consume the eight-second graceful-exit budget after `q`, forcing a truthful failure even though the encoder protocol was healthy. The adapter now bulk-copies raw stderr bytes to `Stream.Null`; command, graceful-exit, forced-exit deadlines, cleanup ownership, and truthful failure mapping are unchanged. · 舊 adapter 逐行解碼 stderr 再派空 callback；host 高負載下 10,000 行診斷可以喺傳送 `q` 後食晒八秒正常退出時限。依家整批複製 raw byte 去 `Stream.Null`；所有時限、process ownership 同如實失敗對應保持不變。
- **Focused evidence / 專項證據：** process-free recorder/registry seam **10/10**; unchanged real Windows fixture **1/1**; the exact captured-output stress loop improved from **7/12 pass** on base to **12/12 pass** after the fix. Final solution/aggregate/source gates are recorded below when complete. · Process-free seam **10/10**、未改 Windows fixture **1/1**；同一條 stress loop 由 base **7/12** 通過變成修復後 **12/12** 全過。最終 solution／aggregate／source gate 完成後記錄喺下方。
...
- **Delivery boundary / 交付界線：** this isolated branch will be committed and pushed for the coordinating agent; it does not merge `main`, delete any worktree/ref, publish a release, or claim remote-main integration. · 呢個隔離分支會 commit 同 push 交畀統籌 agent；唔會自行 merge `main`、刪 worktree／ref、發 release 或聲稱已整合 remote main。
...
- **Unrelated aggregate disposition / 無關 aggregate 狀態：** the repository-wide runner reached the unchanged `ScreenRecorderLifecycle.Tests` fixture and stopped when its synthetic recorder returned “not saved”; an isolated rerun reproduced that failure. `Services/ScreenRecorder.cs`, `Services/ScreenRecorderProcessLifecycle.cs`, and the fixture are byte-for-byte unchanged from this review branch's base, so no unrelated recorder fix is included and a full aggregate pass is not claimed. · 全 repo runner 去到未改過嘅 `ScreenRecorderLifecycle.Tests` fixture 時，因 synthetic recorder 回傳「not saved」而停止；獨立重跑亦重現。`ScreenRecorder.cs`、`ScreenRecorderProcessLifecycle.cs` 同 fixture 同 review base 完全冇差異，所以今次唔夾帶無關 recorder 修正，亦唔會聲稱 full aggregate 通過。
