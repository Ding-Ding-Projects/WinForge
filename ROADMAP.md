...
- [x] Reconcile the preserved PowerToys/package stash against current code, retain its useful coordinator/schema/discoverability intent in the newer implementations, make portable-bundle saves atomic and truthfully fail-aware, and keep bilingual package actions usable at narrow widths. · 對帳已保存 PowerToys／套件 stash，喺較新實作保留有用 coordinator／schema／discoverability 意圖，令可攜清單原子儲存兼如實回報失敗，並保持雙語套件動作喺窄畫面可用。
