...
- [x] Audit the preserved Package Manager recovery snapshot, retain its native scheduler, engine discovery/update, bundle, source, and provenance work, reject obsolete external-launch and credential-in-URL paths, and harden structured settings plus narrow bilingual layouts. · 審核保留嘅套件管理 recovery snapshot，保留原生排程器、引擎搜尋／更新、清單、來源同來源證明，拒絕過時外部啟動／URL 內嵌認證路徑，並強化結構化設定同窄畫面雙語版面。
