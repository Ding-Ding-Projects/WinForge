...
- The new-tab picker focuses the real query editor when it opens; typing a plain query and pressing `Enter` opens the first rendered result without allowing Enter from the regex builder's raw/sample fields to close the picker. The full builder, validation state, named no-results state, category filter, section headings, and result buttons all expose stable automation names or IDs. · 新分頁選擇器開啟時會聚焦真正嘅 query editor；輸入純文字查詢再撳 `Enter` 會開啟第一個已顯示結果，但 regex builder 嘅 raw／sample 欄位按 Enter 唔會關閉 picker。完整 builder、驗證狀態、具名冇結果狀態、分類篩選、分段標題同結果按鈕都有穩定 automation 名稱或者 ID。
