...
All route forms—`checkdigit`, `luhn`, and `module.checkdigit`—open the native page. Debug and Release each pass 208/208 aggregate native checks, including 48 focused Check Digit cases. Both implementations are compared exactly with the independent 89-row Release 102 fixture, enforce the official Discover range boundaries, and reject Unicode letters that case-fold to ASCII; the managed standards mirror passes 24/24. The 46/46 process-owned UI Automation suite exercises every scheme, malformed/invalid state, accessibility cleanup, and language-state retention. The implementation has no network, file, registry, process, elevation, persistence, clipboard, or secret side effect.

全部 route form——`checkdigit`、`luhn` 同 `module.checkdigit`——都會開原生頁面。Debug 同 Release 各自通過 208/208 原生總檢查，包括 48 個檢查碼專項案例。兩個實作都會同獨立 89 行 Release 102 fixture 精確比較、執行官方 Discover range 邊界，亦拒絕會 case-fold 做 ASCII 嘅 Unicode 字母；受控標準鏡像 24/24 通過。而 46/46 自有 process UI Automation 套件會實際操作每個格式、錯誤／無效狀態、無障礙清理同轉語言保留狀態。實作冇網絡、檔案、registry、process、提升權限、persistence、clipboard 或機密副作用。
