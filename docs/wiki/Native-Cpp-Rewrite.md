...
- Native test executable: Debug **265/265** and Release **265/265** (219 core route/package-manager checks plus 46 parser checks), retaining all Package Manager/shell coverage plus focused Check Digit, Text to Binary, and Case Converter cases. Package Manager coverage includes command validation, safe Details-field parsing, update-rule state, custom snooze duration, and preview queue state. Linked managed-reference regressions separately pass Check Digit **24/24** and Text to Binary **18/18**; they are reference tests, not an automated native-vs-managed process comparison. · 原生測試 executable：Debug **265/265**、Release **265/265**（219 個 core route／package-manager 檢查加 46 個 parser 檢查），保留全部套件管理／shell 覆蓋，加上檢查碼、文字轉二進位同大小寫轉換專項案例。套件管理覆蓋包括指令驗證、安全 Details 欄位解析、更新規則狀態、自訂暫停時長同預覽佇列狀態。連結嘅受控參考回歸另外通過檢查碼 **24/24** 同文字轉二進位 **18/18**；佢哋係參考測試，唔係自動原生對受控 process 比較。
- Elevated process-owned UI Automation shell smoke: **101/101**, retaining Package Manager, Check Digit, Text to Binary, and Case Converter checks while covering exact Package Manager aliases, all 11 manager filters, nine-view routing, update-rule state, custom snooze duration, preview queue policy, run-next/run-last controls, retry-preview controls, fail-closed elevation behavior, and accessibility contracts; this is not normal-integrity external-query evidence. · 提權、自有 process UI Automation shell 冒煙：**101/101**，保留套件管理、檢查碼、文字轉二進位同大小寫轉換檢查，並覆蓋準確套件管理 alias、全部 11 個管理器篩選、九檢視 routing、更新規則狀態、自訂暫停時長、預覽佇列政策、下一個／最後執行控制、重試預覽控制、fail-closed 提權行為同無障礙合約；呢個唔係正常 integrity 外部查詢證據。
