...
- Native core tests: **389/389** in Debug and **389/389** in Release (343 core route/package-manager checks plus 46 parser checks), including **42/42** focused Package Mutation Coordinator checks. · 原生核心測試：Debug **389/389**、Release **389/389**（343 個 core route／package-manager 檢查加 46 個 parser 檢查），包括 **42/42** 個 Package Mutation Coordinator 專項檢查。
...
- Elevated process-owned UI Automation smoke: **171/171**, covering routing, bilingual accessibility, live controls, native regex search/builder, Package Manager batch-consent policy, and clipping checks. · 提權、自有 process UI Automation smoke：**171/171**，涵蓋導覽、雙語無障礙、即時控制、原生 regex 搜尋／建立器、Package Manager 批次確認政策同裁切檢查。
