...
- Native core tests: **387/387** in Debug and **387/387** in Release (341 core route/package-manager checks plus 46 parser checks), including **42/42** focused Package Mutation Coordinator checks. · 原生核心測試：Debug **387/387**、Release **387/387**（341 個 core route／package-manager 檢查加 46 個 parser 檢查），包括 **42/42** 個 Package Mutation Coordinator 專項檢查。
...
- Elevated process-owned UI Automation smoke: **166/166**, covering routing, bilingual accessibility, live controls, native regex search/builder, Package Manager batch-consent policy, and clipping checks. · 提權、自有 process UI Automation smoke：**166/166**，涵蓋導覽、雙語無障礙、即時控制、原生 regex 搜尋／建立器、Package Manager 批次確認政策同裁切檢查。
...
On 2026-07-16, the repository driver launched Dashboard, All Apps, Regex Tester, and Package Discover with `-WaitMs 16000`; `CopyFromScreen` was unavailable and each `PrintWindow` fallback was rejected as blank or near-uniform. The same driver separately attempted the changed Package Manager routes—`package-discover`, `package-updates`, `package-installed`, and `package-operations`—with the same block. The repo-local LowLevel MCP isolated desktop captured the four resolved 1980×1320 Package Manager HWNDs; every inspected full-window frame contained only a title bar and blank client surface. No invalid, stale, synthetic, or managed screenshot is presented as native evidence; changed native pages are `capture-blocked`. The 166/166 UI Automation result remains behavioral/accessibility evidence only. · 2026-07-16，repository driver 用 `-WaitMs 16000` 開啟 Dashboard、所有 app、Regex Tester 同 Package Discover；`CopyFromScreen` 用唔到，而且每個 `PrintWindow` fallback 都因空白／接近單色而拒絕。同一個 driver 亦分別嘗試改過嘅 Package Manager routes——`package-discover`、`package-updates`、`package-installed` 同 `package-operations`——結果受同一限制。repo 本機 LowLevel MCP 隔離 desktop 擷取四個已解析嘅 1980×1320 Package Manager HWND；每張檢查過嘅完整視窗 frame 都只得 title bar 同空白 client surface。冇將無效、舊、合成或者受控截圖當做原生證據；改過原生頁面係 `capture-blocked`。166/166 UI Automation 結果只係行為／無障礙證據。
