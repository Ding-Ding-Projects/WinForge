...
**EN —** On 2026-07-16, the changed native Dashboard, All Apps, Regex Tester, and Package Discover pages were separately launched through `driver.ps1 -Native` at `-WaitMs 16000`. `CopyFromScreen` was unavailable; every `PrintWindow` fallback was blank or near-uniform and rejected. The repo-local LowLevel MCP then created an isolated desktop, launched Regex Tester and Package Discover, captured both full-window and client-only HWND frames, and each inspected client frame was blank. The four invalid LowLevel PNGs were discarded. No canonical screenshot, wiki image, or managed substitute was replaced or reused; all changed native regex pages are honestly `capture-blocked`.

**粵語 —** 2026-07-16，改過嘅原生 Dashboard、所有 app、Regex Tester 同 Package Discover 頁已分別用 `driver.ps1 -Native` 加 `-WaitMs 16000` 開啟。`CopyFromScreen` 用唔到；每個 `PrintWindow` fallback 都係空白／接近單色，所以已拒絕。repo 本機 LowLevel MCP 跟住建立隔離 desktop、開啟 Regex Tester 同 Package Discover，擷取完整視窗同只限 client 嘅 HWND frame，而每張檢查過嘅 client frame 都係空白。四張無效 LowLevel PNG 已經丟棄。冇替換、重用任何 canonical 截圖、wiki 圖或者受控替代品；全部改過嘅原生 regex 頁如實係 `capture-blocked`。

**EN —** On 2026-07-15, a persistent LowLevel HTTP MCP session created isolated desktops and launched Package Manager Discover, Updates, and Installed. Each HWND capture was inspected and rejected because it contained only a title bar plus a blank or near-uniform WinUI client frame; the required driver produced the same block. Operations had the same limitation. The invalid outputs were discarded, so every changed native Package Manager view is honestly `capture-blocked`. `docs/screenshot-packages.png` remains a managed-production reference only, and no stale, synthetic, blank, or managed substitute is presented as native evidence.

**粵語 —** 2026-07-15，持續運行嘅 LowLevel HTTP MCP session 建立隔離 desktop，並開啟 Package Manager Discover、Updates 同 Installed。每張 HWND 圖都已檢查並拒絕，因為只得 title bar 加空白／接近單色 WinUI client frame；必需 driver 都得到同一阻礙。Operations 都有同一限制。無效輸出已丟棄，所以每個改過嘅原生 Package Manager view 如實係 `capture-blocked`。`docs/screenshot-packages.png` 仍然只係受控正式版參考，唔會將舊、合成、空白或者受控圖片當原生證據。
