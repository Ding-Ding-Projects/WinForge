...
**EN —** On 2026-07-15, the changed native Package Manager Discover-filter page was launched with `driver.ps1 -Native -Page package-discover -WaitMs 15000`. `CopyFromScreen` was unavailable; the driver's `PrintWindow` fallback was blank or near-uniform and was rejected. The repo-local LowLevel fallback then created `WinForgePackageDiscoverFilterAudit`, launched the same page, listed its native HWND, and captured a title bar with a blank client frame. No valid PNG was accepted, `docs/screenshot-packages.png` was not replaced, and that existing image remains a managed-production reference only; the native page is `capture-blocked`.

**粵語 —** 2026-07-15 改過嘅原生 Package Manager Discover 篩選頁已經用 `driver.ps1 -Native -Page package-discover -WaitMs 15000` 開啟。`CopyFromScreen` 用唔到；driver 嘅 `PrintWindow` fallback 係空白／接近單色，所以已被拒絕。repo 本機 LowLevel fallback 跟住建立 `WinForgePackageDiscoverFilterAudit`、開同一頁、列出原生 HWND，再擷取到 title bar 加空白 client frame。冇接受有效 PNG、冇替換 `docs/screenshot-packages.png`，而呢張既有圖片仍然只係受控正式版參考；原生頁面係 `capture-blocked`。
