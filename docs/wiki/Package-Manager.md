...
- Package-manager core harness: **30/30 passed**, including atomic create/replace/failure-preservation saves and malicious proxy/triplet rejection; the detailed bundle record is [Portable package bundles](../features/package-management/portable-package-bundles.md). · 套件管理 core harness **30/30 通過**，包括原子建立／取代／失敗保留儲存同惡意 proxy／triplet 拒絕；詳細記錄見[可攜套件清單](../features/package-management/portable-package-bundles.md)。
