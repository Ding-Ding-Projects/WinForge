...
There are therefore three enabled native result-query paths plus one source-command probe. The other four shell views—Bundles, Ignored, Setup bootstrap and the real Operations queue—remain gated, and all install/update/uninstall/source/bulk mutations inside the query views are separately gated. Settings is now a small native persistence slice rather than a bare placeholder.
