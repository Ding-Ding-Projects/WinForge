...
There are therefore three enabled native result-query paths, bounded read-only Details queries, local ignored/pinned/snoozed update filtering with a persisted 1/7/14/30-day default snooze duration, one source-command probe, and preview-only install/update/uninstall plan generation. Bundles now has a native snapshot import/export preview, while scheduler integration, Setup bootstrap and the real Operations queue remain gated; all install/update/uninstall/source/bulk execution remains separately gated. Settings is now a small native persistence slice rather than a bare placeholder, and Operations now keeps a recent local history snapshot instead of a transient-only log.
...
- Parsed/rich details fields, scheduler/notification integration, media/share actions and open-install-location behavior. · 已解析嘅豐富詳細資料欄位、排程／通知整合、媒體／分享動作同開啟安裝位置行為。
