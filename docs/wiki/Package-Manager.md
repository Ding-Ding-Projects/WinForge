...
- A selected package source is now carried from a row or bundle through the command preview, multi-select/bundle identity, operation queue and native runner. `PackageSourcePolicy` is the sole command boundary: WinGet and Chocolatey use a strict registered-source token with `--source`; Scoop installs use a validated `bucket/package` reference; trusted PyPI, npm, NuGet and crates.io labels map only to fixed reviewed endpoints; and PowerShell repositories use their real supported parameters. Empty source keeps each manager's configured default. When an update or uninstall CLI has no source selector, a known safe source remains validated queue metadata but no imaginary flag is emitted. Local, unknown, malformed or unsupported sources are rejected before preview/queue/runner and bundle imports log them as incompatible. · 而家所揀套件來源會由套件列或者清單一路帶到指令預覽、多選／清單身份、操作佇列同原生 runner。`PackageSourcePolicy` 係唯一指令邊界：WinGet 同 Chocolatey 用嚴格嘅已登記來源 token 加 `--source`；Scoop 安裝用已驗證嘅 `bucket/package` 參照；可信嘅 PyPI、npm、NuGet 同 crates.io 標籤只會對應固定、已審核 endpoint；PowerShell repository 就用真正支援嘅參數。留空來源會保留各管理器已設定嘅預設。更新或者解除安裝 CLI 冇來源選擇器時，已知安全來源仍然係經驗證嘅佇列中繼資料，但唔會虛構旗標。本機、未知、格式錯誤或者唔支援嘅來源會喺預覽／排隊／runner 之前拒絕，匯入清單會記錄成不相容。
