...
- Automation can use `--page package-discover`, `--page package-updates`, `--page package-installed`, or `--page module.packages#bundles`; these resolve to `module.packages#discover`, `module.packages#updates`, `module.packages#installed`, and `module.packages#bundles`. Native routing is present, but route availability alone is not a parity claim. · 自動化可用 `--page package-discover`、`--page package-updates`、`--page package-installed` 或 `--page module.packages#bundles`；會解析去 `module.packages#discover`、`module.packages#updates`、`module.packages#installed` 同 `module.packages#bundles`。原生導覽已存在，但淨係有 route 唔代表功能對等。
