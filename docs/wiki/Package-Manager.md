...
- **Expanded native regression suite:** **208/208 in Debug and 208/208 in Release**. Every prior Package Manager case remains: command construction and validation, all parser families and malformed-input bounds, executable/transport/runtime behavior, cancellation, containment, output limits, and singular/plural alias-to-view routing. The added checks belong to the Check Digit parity slice; no Package Manager assertion was removed. · **擴充後原生回歸套件：** Debug **208/208**、Release **208/208**。之前全部套件管理案例都保留：指令建立／驗證、全部解析器類型同錯誤輸入界限、executable／transport／runtime 行為、取消、containment、輸出上限，同單數／複數 alias 對應準確檢視；新增檢查屬於檢查碼對等批次，冇移除任何套件管理 assertion。
- **Elevated process-owned UI Automation smoke:** **46/46**. Every prior Package Manager assertion remains, including exact `package-discover`, `package-updates`, and `package-installed` view selection, the 11 manager filters, nine-view shell, busy/ready/result states, fail-closed elevation behavior and accessibility identifiers; the added assertions cover Check Digit accessibility hardening. It does **not** prove live external manager queries. · **提升權限、只控制自己 process 嘅 UI Automation smoke：** **46/46**。之前全部套件管理 assertion 都保留，包括 `package-discover`、`package-updates` 同 `package-installed` 準確檢視、11 個管理器篩選、九檢視 shell、busy／ready／result 狀態、fail-closed 提升權限行為同無障礙 ID；新增 assertion 係檢查碼無障礙加固。**唔代表**真實外部管理器查詢已通過。
