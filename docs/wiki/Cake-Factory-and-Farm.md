...
**EN —** The Cake Factory & Farm module is a hands-on HTML5 simulator for a reactor-powered ingredient farm and bakery line. It is not a video and it is not fully automatic: the reactor bus, farm, mill, mixer, tunnel oven, cooling, icing, packaging, QA and CIP cleaning all expose live controls and operator release gates.
...
| Ingredient farm · 原料農場 | Wheat, sugar crop, vanilla, pasture health, milk, eggs and cocoa grow over time based on farm intensity and available power. |
| Ingredient conversion · 原料轉換 | Harvest, collect milk/eggs, mill wheat into cake flour, refine sugar crop into sugar and churn milk/cream into butter. |
...
4. Prepare ingredients manually:
...
5. Press **Start batch** only after the inventory panel shows no missing ingredients.
6. Watch the current stage and wait for the release gate:
...
7. Run **CIP clean** when sanitation drops or after production. The simulator locks batching during CIP, then restores sanitation as the wash/rinse/drain loop progresses.
...
The module was tested in two ways:

| Evidence · 證據 | Result · 結果 |
|---|---|
| Headless service scenarios · 無介面服務情景 | `dotnet run --project tests/ReactorSim.Tests/ReactorSim.Tests.csproj -c Debug` passed **21/21** scenarios, including cake power gating, no-auto manual mode, ingredient chain, full manual batch and CIP sanitation. |
