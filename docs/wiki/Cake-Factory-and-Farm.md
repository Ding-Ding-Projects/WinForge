...
**EN —** The Cake Factory & Farm module is a hands-on HTML5 simulator for a feature-bus-powered ingredient farm and bakery line. Healthy live nuclear generation is preferred; a default-off, explicitly allowed, manually fuelled and started simulated emergency diesel can provide session-only fallback power. It is not a video and it is not fully automatic: the feature bus, supply dock, supplier purchase orders, inbound delivery lead time, farm, pet-store cow pickup, dairy ration mixing, milking-parlor washdown, milk pasteurization, poultry-house washdown, utility plant, audited ingredient lots, ingredient factories, starch wet mill, poultry feed mill, compost fertilizer plant, packaging plant, icing tempering kitchen, byproduct hauling, effluent treatment, QA lab release, warehouse batch kitting, plant maintenance, mixer, tunnel oven, cooling, icing, packaging, customer order dispatch, QA, signed `.cake` files and CIP cleaning all expose live controls and operator release gates.

**粵語 —** 蛋糕工廠與農場模組係一個由功能匯流排供電嘅 HTML5 互動模擬器，包含原料農場同烘焙生產線。健康即時核電係首選；預設關閉嘅模擬應急柴油後備要先明確容許，再手動入油兼啟動，先可以喺今次 session 頂住供電。佢唔係影片，亦唔係全自動：功能匯流排、收貨、供應商採購單、入廠送貨等候時間、農場、奶牛飼料混合、擠奶間清洗、禽舍清洗、公用工程、已審核批號、原料工廠、禽鳥飼料廠、堆肥肥料廠、副產物運走、廢水處理、QA 實驗室放行、倉庫備料、廠房維修、攪拌、隧道焗爐、冷卻、裝飾、包裝、客戶訂單出貨、品檢、已簽署 `.cake` 檔同 CIP 清潔全部都有即時控制同操作員放行關卡。
...
|---|---|
| Feature-bus power · 功能匯流排供電 | The factory continuously needs enough power from its preferred healthy live nuclear source or an explicitly allowed, manually fuelled, running simulated EDG with the exact Cake Factory tab's lease. Source loss preserves the page and plant state while locking powered machinery/actions and freezing powered progress—including CIP—at its exact state until that same tab regains power. Passive biological change, milk warming/bacterial spoilage risk, transport, and order clocks continue during the outage. |
| Supply chain inputs · 供應鏈輸入 | Ingredients do not appear from nowhere: seed, irrigation water, dairy forage, grain, limestone, trace mineral concentrate, straw, cocoa beans, brine, raw soda ash feedstock, phosphate, paperboard, label stock, packaging ink, adhesive and filter media are finite stocks delivered through the receiving dock or harvested on the farm. Finished starch carrier, crop fertilizer, livestock bedding, dairy mineral premix, poultry feed, baking soda, baking powder and coded cartons are not delivered directly; the operator runs the starch plant, compost plant, bedding plant, mineral premix plant, feed mill, baking-soda plant, leavening plant and packaging plant to make them. The main HTML5 workflow requires a paid supplier purchase order, a delivery ETA, truck arrival and manual unloading before stocks enter inventory. |
| Ingredient farm · 原料農場 | Powered irrigation, milking, egg grading, and production require feature-bus power and their farm inputs. Passive crop and animal condition still evolves when the bus is dark—crops can grow slowly, herd/flock condition can deteriorate, and output machinery remains stopped. Crop growth consumes released fertilizer lots from the compost plant. The operator can harvest pasture into forage lots, divert mature wheat into feed-grain lots for the dairy ration/feed mill, harvest cacao pods into fermented/dried cocoa bean lots for the cocoa line, and use the pet-store livestock counter to get one guest cow with a short transport ETA that continues more slowly without power. Milk comes from a tracked lactating cow herd that consumes mixed dairy ration, water, released bedding and labor, then passes through a powered milking parlor into an agitated chilled raw bulk tank; without power the tank warms and bacterial/spoilage risk rises. The operator must run the HTST pasteurizer/homogenizer and release the pasteurized milk lot before batching can use it. Eggs come from a laying-hen flock that consumes released poultry feed, water, released bedding and labor before the egg-room washer/grader can stamp an egg lot. |
| Dairy barn · 奶牛畜舍 | The operator mixes TMR from finite forage, grain and released mineral premix. Cows consume that ration, released straw bedding and labor, make manure, lose comfort when bedding/labor/hygiene are poor, and milk output reflects days in milk, dry-matter intake, rumen pH, body condition score and milk urea nitrogen. The guest cow from the pet-store counter joins this same care model for 1 hour before automatic release back into the wild. Milk QA reflects parlor hygiene, bulk-tank agitation/cooling load, bacteria, somatic cells, fat and protein. Washdown consumes process water, steam, compressed air, filter media and labor. |
...
| Utility plant · 公用工程廠 | Process water, culinary steam and compressed air are made by a timed, powered support plant instead of appearing instantly. Running the utility plant consumes raw water and filter media, adds feature-bus demand, moves through RO/boiler/compressor phases, reports conductivity and pressure, then transfers utilities only after completion. |
...
| Customer orders · 客戶訂單 | Packed cakes move into finished goods. Active customer orders track required cakes, due time, reward, cash and reputation; due-time and late-reputation clocks continue during a power outage. Dispatch is manual and requires feature-bus power, enough finished goods, truck battery charge and a cold-chain temperature below the safety limit. |
...
| CIP sanitation · CIP 清潔 | Clean-in-place locks batching while washing the mixer, depositor, oven belt, icing head and packer. It requires feature-bus power; exact-owner power loss freezes wash/rinse/drain progress, and restoring that tab's power resumes the same cycle. |

## Nuclear and emergency-diesel power · 核電同應急柴油供電

**EN —** Cake Factory is the only eligible feature that consumes fallback power continuously, not just when the page is opened. Its threshold is **35 MWe**. A healthy live nuclear source that meets that threshold is always preferred. When nuclear is unavailable, the operator may enable the default-off **Allow emergency-diesel fallback** permission, fill the stopped EDG's **60 L** simulated tank, start it, and wait its full **10-second** start sequence. It burns **1.0 L/min while starting or running**. The permission persists; generator state, fuel, and owner-token module leases are session-only, so every app launch begins empty and stopped.

**粵語 —** 蛋糕工廠係唯一會持續消耗後備電、唔係淨係開頁時檢查一次嘅合資格功能；門檻係 **35 MWe**。健康即時核電只要達門檻就永遠優先。核電不可用時，操作員可以開啟預設關閉嘅**容許應急柴油後備**權限，趁 EDG 停機將 **60 L** 模擬油缸入滿油、啟動，再等足 **10 秒**。啟動中或者運行中每分鐘耗油 **1.0 L**。權限會保存，但發電機狀態、燃油同 owner-token 模組 lease 只限今次 session，所以每次開 app 都由空缸兼停機開始。

Use **Reactor Settings** (`WinForge.exe --page reactorsettings`) or the feature-power recovery page to allow, fill, and start the EDG. Its **250 MWe** rating is checked per module, and the Cake Factory tab/instance occupies one of only two exact owner-token EDG lease slots. A third EDG-backed module tab waits until a slot is released by closing, navigating away, or otherwise leaving fallback power. Stopping that source or exhausting its fuel while nuclear remains unavailable does **not** navigate away from Cake Factory or discard its in-memory model. The live page and plant state stay intact; powered farm machinery, factories, utilities, bakery stages, cold-chain equipment/charging, dispatch controls, and an active CIP cycle pause or lock at their exact state. Passive crop/animal condition, unchilled-milk warming and bacterial/spoilage risk, guest-cow and supply-truck transport, and customer order clocks continue; an unpowered bakery batch may also lose quality. Restoring healthy nuclear or an EDG outlet to that same owner resumes powered work; another tab's lease cannot power it. A manual stop preserves remaining fuel only for this app session; disabling fallback clears it. This simulation never starts a real generator, touches hardware or fuel, changes Windows power settings, or rewrites the public reactor-status API. · 請喺**反應堆設定**（`WinForge.exe --page reactorsettings`）或者功能電源復原頁容許、入油兼啟動 EDG。**250 MWe** 會逐模組檢查，而蛋糕工廠分頁／instance 會佔兩個準確 owner-token EDG lease 位其中一個；第三個後備模組分頁要等現有分頁關閉、轉頁或者離開後備電而釋放個位。冇核電時該電源停機或者燒光油，WinForge **唔會**導走蛋糕工廠頁面或者丟棄記憶體內模型。即時頁面同廠房狀態會完整保留；有電農場機器、工廠、公用工程、烘焙工序、冷鏈設備／充電、出貨控制，同進行中嘅 CIP 都會暫停或鎖喺原本狀態。被動農作／動物狀況、冇冷凍牛奶嘅升溫同細菌／變壞風險、奶牛／供應貨車運輸，同客戶訂單時鐘會繼續；失電中嘅烘焙 batch 亦可能跌品質。同一 owner 恢復健康核電或 EDG 插槽後，有電流程會繼續；第二個分頁嘅 lease 唔可以借電。手動停機只會為今次 app session 保留餘油；關閉後備權限就會清空。呢個模擬永遠唔會啟動真實發電機、掂硬件或真燃油、改 Windows 電源設定，亦唔會改寫公開反應堆狀態 API。

![Cake Factory running on one emergency-diesel owner lease](https://raw.githubusercontent.com/Ding-Ding-Projects/WinForge/main/docs/screenshot-cakefactory-feature-power.png)
...
| Unload delivery · 卸貨入倉 | After the supplier truck reaches the receiving dock, unloads and books audited seed, water, dairy forage, grain, limestone, trace mineral concentrate, straw, cocoa beans, brine, raw soda ash feedstock, phosphate, paperboard, label stock, packaging ink, adhesive, process water, culinary steam, compressed air and filter media into inventory. Finished starch carrier, fertilizer, bedding, baking soda, baking powder, coded cartons and dairy mineral premix are not unloaded directly; they are made in the starch, compost, bedding, baking-soda, leavening, packaging and mineral premix plants. |
| Run utilities · 運行公用工程 | Runs the RO skid, clean-steam boiler and compressor. The run consumes raw water/filter media up front, draws feature-bus power while timed, then produces process water, culinary steam and compressed air after release. |
...
| Get cow · 取得奶牛 | Uses the pet-store livestock counter to reserve one guest cow with a short trailer ETA. Intake consumes a small amount of cash, mixed ration, water, released bedding and barn labor, then the cow joins the lactating herd for 1 hour before release back into the wild. |
...
| Baking soda · 小蘇打廠 | Converts audited raw soda ash feedstock into baking soda using solution make-up, carbonator gas scrubbing, bicarbonate crystallization, centrifuge dewatering, fluid-bed drying, sieve checks and alkalinity/purity/moisture QA. |
| Leavening plant · 膨鬆劑廠 | Converts released baking soda, finite audited phosphate and released starch carrier into usable baking powder using barcode verification, dehumidified micro-weighing, ribbon blending, dust collection, sifter classification and CO2 release QA. |
...
| Prepare icing · 準備糖霜 | Converts released sugar, butter, pasteurized cow milk, vanilla and cocoa when required into a finite prepared icing lot using the icing tempering kitchen. The run consumes utilities, adds feature-bus demand, waits for QA lab release and must be reserved by warehouse kitting before decorating. |
...
| Dispatch order · 訂單出貨 | Ships the active customer order after finished goods, feature-bus power, truck charge and cold-chain temperature are ready. Dispatch consumes finished goods, pays cash and updates reputation. |
| CIP clean · CIP 清潔 | Starts a powered clean-in-place sanitation loop. Batching is locked until it completes; exact-owner power loss freezes the cycle at its current progress and power restoration resumes it. |
...
2. If the banner says **Waiting for nuclear or emergency-diesel feature-bus power**, use the preferred path by opening the reactor and restoring generation, or explicitly allow fallback, manually add simulated diesel, start the EDG and wait 10 seconds. Cake Factory also needs one of the two EDG lease slots for this exact tab. The page and factory state remain loaded while neither source is usable; new powered actions lock and active powered timers—including CIP—freeze until this tab regains power.
3. Select a recipe and set farm intensity / line speed. Higher settings make the scene more active but increase plant load.
4. Keep the supply chain stocked. Use **Order supplies** when seed, irrigation water, dairy forage, grain, limestone, trace minerals, straw, labor, cocoa beans, brine, raw soda ash, phosphate, paperboard, label stock, packaging ink, adhesive or factory utilities are low. Wait for the supplier truck ETA, then press **Unload delivery** after it reaches the dock. Unloading records a new receiving manifest; finished starch carrier, fertilizer, bedding, baking soda, baking powder and dairy mineral premix must be made on site.
5. Prepare ingredients manually:
   ...
   - Press **Get cow** when you want an easy guest cow. The pet-store livestock counter charges a small fee, consumes intake ration, water, released bedding and labor, runs a short transport ETA, adds the cow to the lactating herd, then releases it back into the wild after 1 hour.
   - Keep the cow herd fed, watered and bedded, then collect raw milk when the dairy buffer is ready. Watch mixed ration, dry-matter intake, rumen pH, body condition, milk urea nitrogen, manure, parlor hygiene, bulk-tank agitation/cooling and raw milk QA.
   ...
   - Run the baking-soda plant when baking soda is low; wait for raw soda ash assay, solution make-up, carbonation, crystallization, centrifuging, fluid-bed drying, sieve checks and QA lab release.
   - Run the leavening plant when baking powder is low; wait for released baking-soda lot verification, dehumidified micro-weighing, starch dosing, ribbon blending, dust collection, sifter classification, CO2 release QA and lab release.
...
14. Run **CIP clean** when sanitation drops or after production. The simulator locks batching during CIP, then restores sanitation as the powered wash/rinse/drain loop progresses. If this exact tab loses its source, progress freezes without losing the cycle and resumes only when its power returns.
...
- It does not erase or reconstruct the factory merely because its exact-owner power source disappears. The page and in-memory plant state remain alive while every power-dependent command locks and every active powered timer, including CIP, freezes at its exact progress.
...
- Cows do not appear from nowhere. The low-friction **Get cow** action uses the pet-store livestock counter, cash, intake ration, water, released bedding and labor, then the guest cow joins the normal dairy care model for 1 hour before release back into the wild.
...
- Baking soda does not arrive as a finished final input. Supplier deliveries provide audited raw soda ash feedstock, and the operator must run the baking-soda plant through carbonation, crystallization, centrifuging, drying, sieving and alkalinity/purity/moisture QA before leavening can consume it.
- Baking powder does not arrive as a finished final input. Supplier deliveries provide audited phosphate, baking soda comes from the baking-soda plant, starch comes from the starch wet mill, and the operator must run the leavening plant through dehumidified micro-weighing, ribbon blending, sifting, dust collection, CO2 release QA and lab release before batching can consume it.
...
- Milk specifically comes from lactating cows, which consume traceable mixed ration, water, released bedding and barn labor before the powered milking parlor can transfer raw milk into cold storage. Herd output now depends on days in milk, dry-matter intake, rumen pH, body condition score and milk urea nitrogen, while the raw bulk tank tracks agitation rpm and cooling load. Raw milk is not recipe-ready: warm, high-count, low-solids, high-SCC, poor rumen-balance, out-of-range MUN, stopped-agitation or low-hygiene raw milk is held by the pasteurizer gate, and the HTST pasteurizer/homogenizer must create a QA-released pasteurized milk lot before batching or icing can use it.
...
- Ingredient factories are not instant: they consume finite inputs and utilities, add feature-bus demand, move through unit-operation phases, report process QA, pause on low power and release usable ingredients only after the process completes. The flour mill specifically consumes a wheat lot, cleaning water, compressed air and filter media before cake flour exists.
...
|---|---|
| Controls are disabled · 控制不能按 | No usable feature-bus source is available for this exact tab. Restore healthy nuclear generation, or allow fallback, add simulated diesel, start the EDG and wait 10 seconds. If two other EDG-backed module instances/tabs hold the outlets, close or navigate one away to release its lease; another tab's lease cannot power this factory. |
| Unload delivery is disabled · 無法卸貨 | No supplier truck is scheduled, the truck is still en route, feature-bus power is low, forklift battery is low or pallet space is full. |
| Stage kit is disabled · 無法備料 | Missing recipe ingredients, lab hold, active CIP cycle, no feature-bus power, low forklift battery or no staging pallet space. |
| Start batch is disabled · 未能開批 | No staged warehouse kit, active CIP cycle, no feature-bus power, wrong selected recipe for the staged kit, or a batch already on the line. |
| Farm output stalls · 農場停產 | Seed, irrigation water, released fertilizer, feed-mill inputs or feature-bus power is low. Order supplies, unload the delivery, run/release compost fertilizer and restore nuclear or EDG power. |
| Milk production stalls · 牛奶停產 | Mixed ration, water, released bedding, labor, pasture health or feature-bus power is low. Blend/release mineral premix, mix dairy ration, make/release bedding, order supplies, unload the delivery, wash the parlor and restore power. |
| Get cow is disabled · 不能取得奶牛 | Feature-bus power, cash, mixed ration, water, released bedding, barn labor, pickup availability or the previous guest-cow release timer is not ready. |
...
| Pasteurize milk is disabled · 不能巴氏殺菌牛奶 | Raw bulk-tank milk is low, raw milk lot data is missing, raw milk QA is on hold, feature-bus power is low, a factory run or lab hold is active, utilities are low, or byproduct/effluent capacity is full. |
| Churn butter is disabled · 不能打牛油 | Raw milk reserve, raw milk lot data, raw milk QA, process water, culinary steam, compressed air, filter media, feature-bus power, lab clearance or waste/effluent capacity is not ready. |
| Refine sugar is disabled · 不能煉糖 | Sugar crop, crop lot data, process water, culinary steam, compressed air, filter media, feature-bus power, lab clearance or waste/effluent capacity is not ready. |
| Egg production stalls · 雞蛋停產 | Released poultry feed, water, released bedding, labor, hen-house hygiene or feature-bus power is low. Mill feed, make/release bedding, release the feed lot, order supplies, unload the delivery, wash poultry and restore power. |
...
| Run utilities is disabled · 未能運行公用工程 | Feature-bus power is low, raw water/filter media is low, the utility plant is already active, or process-water/steam/air storage is too full. |
| Harvest feed is disabled · 不能收成飼料作物 | Pasture/wheat is not mature enough, feature-bus power is low, or barn labor is unavailable. |
| Harvest cocoa is disabled · 不能收成可可 | Cacao pods are not mature enough, feature-bus power is low, farm labor is unavailable or forklift battery is too low. |
| Starch plant is disabled · 不能製造澱粉 | Feed grain, process water, culinary steam, compressed air, filter media, feature-bus power, lab clearance or waste/effluent capacity is not ready. |
| Salt works is disabled · 鹽廠不能啟動 | Brine quantity, brine lot data, process water, culinary steam, compressed air, filter media, feature-bus power, lab clearance or waste/effluent capacity is not ready. |
| Baking soda plant is disabled · 小蘇打廠不能啟動 | Raw soda ash feedstock, soda ash lot data, process water, culinary steam, compressed air, filter media, feature-bus power, lab clearance or waste/effluent capacity is not ready. |
| Leavening plant is disabled · 不能製造膨鬆劑 | Released baking soda, phosphate, released starch carrier, audited source lots, compressed air, filter media, feature-bus power, lab clearance or waste/effluent capacity is not ready. |
| Mill feed is disabled · 不能製造飼料 | Grain, starch carrier, released mineral premix, utilities, feature-bus power, lab clearance or waste/effluent capacity is not ready. |
| Compost fertilizer is disabled · 不能製造堆肥肥料 | Dairy manure, poultry manure, compostable organics, process water, compressed air, filter media, feature-bus power or lab clearance is not ready. |
| Make bedding is disabled · 不能製造墊料 | Straw feedstock, process water, compressed air, filter media, feature-bus power, lab clearance or waste/effluent capacity is not ready. |
| Blend minerals is disabled · 不能混合礦物 | Limestone, trace mineral concentrate, phosphate, released salt, process water, compressed air, filter media, feature-bus power, lab clearance or waste/effluent capacity is not ready. |
...
| Lab release hold · 實驗室放行暫停 | A factory output lot is waiting for QA lab release. Press Release lab lot after restoring feature-bus power and lab utilities. |
| Service plants is disabled · 無法維修廠房 | A factory run is active, feature-bus power is low, or process water/steam/compressed air/filter media is insufficient. |
...
| Release step is disabled · 未能放行 | Stage timer is still running, kill-step/cooling/sanitation gate is not met, or feature-bus power is unavailable. |
| Dispatch order is disabled · 無法出貨 | Finished goods are below the active order quantity, feature-bus power is low, truck battery charge is low, or the dispatch cold chain is too warm. |
...
| CIP seems to stop production · CIP 停止生產 | Expected behavior: CIP locks batching until its sanitation loop finishes. If the exact-owner nuclear/EDG source is lost, the active cycle holds its precise wash/rinse/drain progress; restore power to the same tab to resume it. |
...
- `Pages/CakeFactoryModule.xaml.cs` — WebView2 bridge, nuclear-preferred feature-bus snapshot resolution/posting and operator action handling.
- `SimAssets/cake/index.html` — immersive HTML5 canvas, controls and JavaScript UI.
- `Services/CakeFactoryService.cs` — authoritative simulator model for recipes, inventory, stages, quality, sanitation and continuous feature-bus dependency.
