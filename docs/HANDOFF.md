...
- Native Slugify: `slug`, `slugify`, and `module.slugify` use the standard-C++ `Slugify` core plus a C++/WinRT renderer. Debug/Release builds exit 0 with 0 errors, core is **777/777** in both configurations (Slugify **18/18**), catalog parity covers 346+5, and `-SlugifyRoutesOnly -AllowClipboardMutation` is **12/12** across all aliases. It preserves managed line/diacritic/Unicode/separator/case/max-length/preview/reset/localization behavior and writes the clipboard only after explicit Copy. LowLevel MCP is unavailable in this session; the required driver rejected a blank fallback, so the route remains `in-progress` / `capture-blocked`. · 原生網址別名：三個 alias 用標準 C++ `Slugify` core 加 C++/WinRT renderer；Debug／Release 0 errors、core 各 **777/777**（Slugify **18/18**）、catalog parity 346+5、`-SlugifyRoutesOnly -AllowClipboardMutation` **12/12**。保留 managed 分行／重音／Unicode／分隔符／大小寫／長度／預覽／重設／本地化行為，並且只喺明確 Copy 寫剪貼簿。今次冇 LowLevel MCP，driver 拒絕空白 fallback，所以 route 保持 `in-progress`／`capture-blocked`。
