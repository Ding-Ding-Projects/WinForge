...
- Native Percentage Calculator: `percent`/`percentage`/`module.percentcalc` now use standard-C++ `PercentCalc` plus a C++/WinRT renderer for all six managed cards, culture/invariant parsing, six-place away-from-zero output, banker-rounded tip splits, guarded errors, lifecycle reset, and explicit-only Copy. Debug/Release builds have 0 errors, core is **796/796** including **37/37**, catalog parity is 346+5, and focused UIA is **13/13**. LowLevel MCP is unavailable here; the native driver rejected its blank fallback, so this route remains `in-progress` / `capture-blocked`. · 原生百分比計算器：`percent`／`percentage`／`module.percentcalc` 而家用標準 C++ `PercentCalc` 同 C++/WinRT renderer，保留六張 managed 卡、文化／invariant 解析、六位向遠離零輸出、小費 banker's rounding、受保護錯誤、lifecycle reset 同只限明確 Copy。Debug／Release 0 errors、core **796/796**（包括 **37/37**）、catalog parity 346+5、專項 UIA **13/13**；今次冇 LowLevel MCP，driver 拒絕空白 fallback，所以 route 保持 `in-progress`／`capture-blocked`。
