...
- Latest-channel correction: `gh release create --latest` left `/releases/latest` at historical managed `v1.0.256`; official manual `gh release edit native-v1.0.44 --latest` repaired it to `native-v1.0.44`. Corrective hardening adds explicit edit, fail-closed Latest/asset verification, and official Node 24 `checkout@v7`, `upload-artifact@v7`, `download-artifact@v8`, and `setup-msbuild@v3`. Exact corrective branch/main run IDs, tags, and SHAs remain pending hosted success; do not claim the automatic postcondition yet.
