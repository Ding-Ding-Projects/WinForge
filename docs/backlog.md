...
- **2026-07-17 — ⚛️ Grid Load-Shed Dispatcher shipped → 320. The reactor-loads epic's last named item is built.**
  `module.loadshed` (`--page loadshed|mwbudget`): the epic's "live MW-budget dashboard" — 8 city feeders (hospitals→industry, P1..P5, 990 MW catalog) dispatched every tick against the live reactor MW minus an operator-set spinning reserve; **strict priority cutoff** (first non-fitting feeder sheds with everything below it), instant trip + anti-flap reclose window (10 ticks), unserved-MWh and shed-event accounting, operator breakers (off ≠ unserved). Pure `Services/GridLoadShedService.cs`; `Pages/GridLoadShedModule` builds the feeder board from real controls in code-behind (no DataTemplates). **New harness scenario** covering healthy dispatch → sag shed split (350/640) → unserved integration → anti-flap reclose → operator-off → blackout → reset; harness now **65 scenarios**, Linux run 63/65 (only the two known Windows-only disk-probing waste scenarios fail environmentally), Roslyn syntax + name/handler + XML checks clean. WinUI solution build still needs the Windows box before merging to main.
- **Next focus:** general modules + deepening the ⚡ economy (e.g. loads *spending* ⚡ on feedstock, a market price curve); more reactor loads welcome (uranium enrichment cascade? aluminium electrolysis?); continue evidence-gated native parity slices. At **320 modules**.
