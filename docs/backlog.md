...
- **Batches 21+ — depth + independent-native era (status sync 2026-07-24).** Before the two industrial-load additions, the registry held **318 registered modules**. Post-batch-20 additions include ⚛️ **Cement Kiln** (`module.cementkiln`) plus Color Mixer, JSON Sort, Lorem Text, Phonetic Alphabet, Word Frequency, and others; the C++20/C++/WinRT experiment is now maintained independently in [WinForge-Native](https://github.com/codingmachineedge/WinForge-Native).
- **2026-07-24 — ⚛️ Ammonia / Fertilizer Plant shipped → 319 modules.**
  `module.ammonia` (`--page ammonia|fertilizer|fertiliser`): a reactor-powered green-ammonia Haber–Bosch plant — electrolytic H₂ plus air-separated N₂, a synthesis loop that pressurises toward 200 bar, NH₃ production above 150 bar, ~10 MWh/t at up to 350 MW, avoided grey-ammonia CO₂, and NH₃ sales into the ⚡ economy at 9 ⚡/t. The default 280 MW setpoint is intentionally above the model's ~263 MW steady synthesis threshold; duplicate integer ticks recompute outputs without advancing pressure or lifetime production. `Services/AmmoniaPlantService.cs` and `Pages/AmmoniaPlantModule` remain in-process, bilingual, theme-aware, and reactor-bus gated.
- **2026-07-24 — ⚛️ Grid Load-Shed Dispatcher shipped → 320 modules / 321 registered entries. The reactor-loads epic's last named item is built.**
  `module.loadshed` (`--page loadshed|mwbudget`): the epic's live MW-budget dashboard — eight city feeders (hospitals→industry, P1..P5, 990 MW catalog) dispatch against live reactor MW minus operator-set spinning reserve. It uses a **strict priority cutoff**, instant trip, a ten-tick anti-flap reclose window, unserved-MWh and shed-event accounting, and independent operator breakers (off ≠ unserved). A cold bus reports all enabled demand as shed without inventing a served→shed event, and duplicate ticks do not accumulate energy or reclose progress. `Pages/GridLoadShedModule` uses responsive real controls and accessible breaker names.
- **Windows verification (2026-07-24):** `dotnet build WinForge.sln -c Debug -p:Platform=x64` completed with **0 errors**; `tests/ReactorSim.Tests` passed **65/65**, including the two new industrial-load scenarios and duplicate-tick/cold-bus regressions; XAML literal safety and the source-surface audit passed. Both `ammonia` and `loadshed` opened on a dedicated LowLevel headless desktop. Visual evidence is `capture-blocked`: LowLevel returned solid-black 1574×887 client captures for both fresh WinUI handles, the repository driver rejected a blank/near-uniform fallback, and switching the headless desktop visible failed with access denied. No invalid frame is promoted as a screenshot.
- **Next focus:** general modules + deepening the ⚡ economy (e.g. loads *spending* ⚡ on feedstock, a market price curve); more reactor loads welcome (uranium enrichment cascade? aluminium electrolysis?); continue evidence-gated work in the appropriate managed or independent-native repository. At **320 modules / 321 registered entries**.
...
- [x] **Freezes — round 3: DONE (audit closed 2026-07-17).** `CropAndLockService.CreateFromScreenRectAsync` converted earlier (TCS + `Task.WhenAny`); `NativeMessagePump._ready.Wait(1500)` deliberately left (inside a `lock`, shared by 4 services, near-instant). Sync-over-async audit of the four flagged files: **AdvancedPaste, PlainTextPaste and Ftp are clean** (zero `.Result`/`.GetAwaiter().GetResult()`/`.Wait(` — grep-verified). The single remaining site, `Sdk/ReactorStatusClient.IsReactorGenerating()` (line ~259), is a **deliberate public-SDK sync convenience**: MMF fast path first, pipe fallback, an explicit "background thread only" doc warning, an async counterpart right above it — and **no in-app callers at all** (grep-verified). No freeze risk; keep as-is for external SDK consumers.
...
`ElectricPowerMW`, gate work, show a MW meter), verified green + reactor tests 65/65 before merge.
