...
The query field owns the flyout keyboard contract: Down/Up move into the current filtered option list, Enter commits the highlighted option, and Escape clears an active query before closing on the next Escape. Language and both persisted funny-level settings refresh the flyout label, placeholder, status copy, accessible names, and help text live; category names and IDs remain factual. The flyout width and option-list height are calculated from the live `XamlRoot` viewport at opening, and the nested builder receives the same width cap. This viewport calculation is runtime-only; source contracts cover the fallback and sizing path, while a real narrow-layout HuiShot requires the repository's off-screen driver and a user-approved first-run state. · query field 自己擁有 flyout 鍵盤合約：Down／Up 入到目前篩選後嘅選項清單，Enter 確認 highlight 嘅選項，而 Escape 先清除 active query，再撳一次先關閉。語言同兩個持久 funny-level settings 會即時更新 flyout label、placeholder、status copy、accessible names 同 help text；分類名稱同 ID 保持事實原樣。Flyout 寬度同選項清單高度會喺開啟時按 live `XamlRoot` viewport 計算，nested builder 亦收到同一個 width cap。呢個 viewport 計算係 runtime-only；source contracts 會守住 fallback 同 sizing path，但真正窄版 HuiShot 需要 repo off-screen driver 同使用者批准嘅 first-run state。
