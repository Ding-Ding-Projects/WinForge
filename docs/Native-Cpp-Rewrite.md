...
Native Discover now also mirrors the managed cached-search refinement contract: **Both name and ID**, **Package name**, **Package ID**, **Exact match**, and **Show similar packages**, plus case-sensitive and ignore-special-character choices. The pure-C++ filter keeps the raw cached rows and source order, reports visible/cached counts locally, requires two non-whitespace characters for a new query, persists its choices with remembered filters, and never starts another process, network request, or package query.

原生 Discover 而家亦跟受控版已快取搜尋微調合約：**名稱及 ID**、**套件名稱**、**套件 ID**、**完全相同**、**顯示相似套件**，再加大小寫敏感同忽略特殊字元選擇。純 C++ 篩選會保留原始快取資料列同來源次序、本機報告可見／快取數量、新查詢要最少兩個非空白字元、開啟記住篩選時保存選擇，而且絕對唔會開另一個 process、網絡要求或者套件查詢。
...
Focused codec tests pass 15/15; after the Package Manager Discover-filter slice the aggregate native executable now passes 322/322 in Debug and Release (276 core route/package-manager/codec/Roman checks plus 46 parser checks), and the process-owned UI Automation shell passes 144/144, including accessibility names, encode/decode, swap, malformed-output clearing, alias routing, language-rerender preservation, horizontal-clipping checks, and the new Discover-filter cache-only assertions. Fresh native visual capture remains `capture-blocked`: the required repository driver’s `PrintWindow` fallback and the inspected LowLevel MCP off-screen desktop HWND capture each returned a blank/near-uniform WinUI client frame; no blank, stale, synthetic, or managed image was substituted.

編解碼專項測試係 15/15；套件管理 Discover 篩選批次之後完整原生 executable 喺 Debug 同 Release 都係 322/322（276 個 core route／package-manager／codec／羅馬數字檢查加 46 個 parser 檢查），只控制自己 process 嘅 UI Automation shell 係 144/144，涵蓋無障礙名稱、編碼／解碼、對調、錯誤輸出清空、alias routing、轉語言時保留狀態、水平裁切檢查，同新嘅 Discover 篩選只用快取 assertion。最新原生視覺擷取仍然係 `capture-blocked`：必需嘅 repository driver `PrintWindow` fallback 同檢查過嘅 LowLevel MCP 離屏 desktop HWND 擷取都得到空白／接近單色 WinUI client frame；冇用空白、舊、合成或者受控版圖片頂替。
...
- Current Package Manager Discover-filter slice: Debug and Release native tests each pass **322/322** (276 core checks plus 46 parser checks), including nine cached-filter cases; the elevated process-owned UI Automation shell passes **144/144**, including three new assertions for filter accessibility/clipping, cache-only re-filtering without another package query, and language-rerender persistence. This is read-only behavior evidence, not normal-integrity external-query or mutation evidence. · 目前套件管理 Discover 篩選批次：Debug 同 Release 原生測試各自通過 **322/322**（276 個 core 檢查加 46 個 parser 檢查），包括九個快取篩選案例；提權、自有 process UI Automation shell 通過 **144/144**，包括三個新 assertion：篩選無障礙／裁切、只用快取重新篩選唔開另一個套件查詢，同轉語言重繪後保留。呢個係只讀行為證據，唔係正常 integrity 外部查詢或者修改證據。

- Current aggregate after the Package Manager Discover-filter slice: native Debug and Release tests **322/322** (276 core route/package-manager/codec/Roman checks plus 46 parser checks, including nine focused cached-filter cases) and elevated process-owned UI Automation **144/144** (including three new filter assertions). Earlier per-slice totals below remain historical evidence for those slices. · 套件管理 Discover 篩選批次之後目前整體：原生 Debug／Release 測試 **322/322**（276 個 core route／package-manager／codec／羅馬數字檢查加 46 個 parser，包括九個快取篩選專項案例）同提權、自有 process UI Automation **144/144**（包括三個新篩選 assertion）。下面較舊逐批次總數保留作歷史證據。
...
- Native test executable: **322/322 in Debug and 322/322 in Release** (276 core route/package-manager/codec/Roman checks plus 46 parser checks). It retains every Package Manager and shell regression, adds nine cached Discover-filter cases, and keeps the 48 focused Check Digit, 25 focused Text to Binary, 10 Case Converter, 16 GUID Generator, 15 codec, and 17 Roman Numerals cases green. · 原生測試 executable：Debug **322/322**、Release **322/322**（276 個 core route／package-manager／codec／羅馬數字檢查加 46 個 parser 檢查）。佢保留全部套件管理同 shell 回歸，加入九個快取 Discover 篩選案例，並保持 48 個檢查碼、25 個文字轉二進位、10 個大小寫轉換、16 個 GUID 產生器、15 個編解碼同 17 個羅馬數字專項案例通過。
...
Fresh native Dashboard, All Apps, About, Package Manager (`module.packages#updates`, `package-ignored`, `package-settings`, `package-discover`, and the latest `package-operations` preview-queue slice), Check Digit (`checkdigit`), Text to Binary (`binarytext`), Base32 / 58 / 85 (`base32`), GUID Generator (`guidgen`), Case Converter (`caseconvert`), and Roman Numerals (`romannum`) capture attempts were made with the required repository driver. On 2026-07-15 the changed `package-discover` filter page was specifically retried with `-Native -Page package-discover -WaitMs 15000`; the driver rejected its blank/near-uniform fallback. `WinForgePackageDiscoverFilterAudit` on the repo-local LowLevel fallback then launched the same page, listed its HWND, and produced the same title-bar/blank-client result. All changed native surfaces remain blocked in this desktop session. `CopyFromScreen` was unavailable. `PrintWindow(PW_RENDERFULLCONTENT)` returned a title bar but a blank/near-uniform WinUI client frame, so the improved driver rejected it with:

已經用指定 repo driver 重新嘗試擷取原生 Dashboard、所有 app、About、套件管理（`module.packages#updates`、`package-ignored`、`package-settings`、`package-discover` 同最新 `package-operations` 預覽佇列批次）、檢查碼（`checkdigit`）、文字轉二進位（`binarytext`）、Base32／58／85（`base32`）、GUID 產生器（`guidgen`）、大小寫轉換（`caseconvert`）同羅馬數字（`romannum`）。2026-07-15 特別為改過嘅 `package-discover` 篩選頁用 `-Native -Page package-discover -WaitMs 15000` 重試；driver 拒絕咗空白／接近單色 fallback。repo 本機 LowLevel fallback 嘅 `WinForgePackageDiscoverFilterAudit` 跟住開同一頁、列出 HWND，並得到同樣 title-bar／空白-client 結果。所有已改原生介面喺呢個 desktop session 仍然受阻。`CopyFromScreen` 用唔到；`PrintWindow(PW_RENDERFULLCONTENT)` 雖然有 title bar，但 WinUI client frame 係空白／接近單色，所以改良後 driver 拒絕咗：
