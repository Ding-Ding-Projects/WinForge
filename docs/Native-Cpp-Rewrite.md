...
Selected cached Discover, Updates, or Installed rows—and the current eligible **Review Update all** set—now enter one all-or-nothing native batch of at most 25 reviewed commands. Every redacted argv remains visible on one accessible batch card; a single later confirmation queues the retained commands serially. The batch owns cancellation and retry: cancellation asks the active process to stop and terminal-cancels pending children, while retry returns only failed, timed-out, or cancelled children to fresh consent and never replays a successful child. Submission rejects an invalid child, duplicate identity, elevation, hooks, unsafe IDs, custom mutation arguments, oversized previews, or unavailable capacity without retaining a partial batch. Capacity cleanup preserves every child of a still-active batch and only reclaims a complete terminal batch. Debug and Release both pass **387/387** (341 core + 46 parser), including **42/42** focused coordinator checks; catalog parity passes 346 fixed routes, five dynamic families, 319 registry records, 22 categories, and 346 ledger rows; the elevated process-owned UI Automation smoke passes **166/166**. Its package-manager dynamic mutation branch was not exercised against a live external manager in this elevated session; the native runtime correctly fails closed, so this is core/UI and containment evidence—not normal-integrity execution proof. · 所選已快取 Discover、Updates 或 Installed 資料列——同目前合資格嘅 **檢視全部更新**集合——而家會進入一個全有或全無、最多 25 條已檢視指令嘅原生批次。每個已遮蔽 argv 會留喺同一張可存取批次卡；之後一次確認會將保留嘅指令按次序串行排隊。批次控制取消同重試：取消會要求執行中 process 停止並將等候子項終止取消；重試只會將失敗、超時或者已取消子項返回全新確認，絕對唔會重播成功子項。提交會因無效子項、重複 identity、提升權限、hooks、唔安全 ID、自訂修改參數、過長預覽或者冇容量而拒絕，唔會留低部分批次。容量清理會保留仍然活躍批次嘅每個子項，只會回收完整終止批次。Debug 同 Release 都通過 **387/387**（341 個 core + 46 個 parser），包括 **42/42** 個專項協調器檢查；目錄對等通過 346 條固定路線、五組動態家族、319 條 registry 記錄、22 個分類同 346 條 ledger rows；提權、自有 process UI Automation smoke 通過 **166/166**。呢個提權 session 冇向即時外部 manager 操作 package-manager 動態修改分支；原生 runtime 正確 fail closed，所以呢個係 core／UI 同 containment 證據，唔係正常 integrity 執行證明。

### 2026-07-16 Native PCRE2 regex/search update · 2026-07-16 原生 PCRE2 regex／搜尋更新

All three native search/filter surfaces now support bounded PCRE2-16: Shell catalog, All Apps, and cached Package Discover. `module.regextester` is a real four-step wizard for flags, escaped literal/class tokens, groups/alternatives/quantifiers, capture preview, and target-aware application. Discover regex is local-cache only and a native guard prevents it from reaching argv or HTTPS. The combined current Debug and Release regression passes **387/387** (341 core + 46 parser); catalog parity passes 346 fixed routes and five dynamic families; the elevated process-owned UI Automation sweep passes **166/166**, retaining valid/invalid regex, named capture preview, All Apps apply, cache-only Discover behavior, batch policy, and horizontal-bound checks. The repository driver and a LowLevel MCP isolated desktop both produced blank or near-uniform client frames for changed pages, so visual evidence is `capture-blocked`, not a visual pass. · 三個原生搜尋／篩選位置而家都支援有界 PCRE2-16：Shell 目錄、所有 app 同已快取 Package Discover。`module.regextester` 係真正四步精靈，處理旗標、已 escape literal／class token、group／alternative／quantifier、capture 預覽同按目標套用。Discover regex 只限本機快取，而且原生 guard 阻止佢去 argv 或 HTTPS。目前合併嘅 Debug 同 Release 回歸通過 **387/387**（341 個 core + 46 個 parser）；目錄對等通過 346 條固定路線同五組動態路線；提權、自有 process UI Automation sweep 通過 **166/166**，保留有效／無效 regex、named capture 預覽、所有 app 套用、只限快取 Discover 行為、批次政策同水平邊界檢查。repository driver 同 LowLevel MCP 隔離 desktop 都為改過頁面得到空白／接近單色 client frame，所以視覺證據係 `capture-blocked`，唔係 visual pass。
...
`module.regextester` is now a real four-step native builder: target/flags, escaped literal/class/common-token composition, group/alternation/quantifier composition, then match/capture preview and apply. It can apply to Shell, All Apps, cached Package Discover, or remain tester-only. It is not a claim of complete .NET regex or replacement parity.

`module.regextester` 而家係真正四步原生建立器：目標／旗標、已 escape literal／class／常用 token 組合、group／alternation／quantifier 組合，之後 match／capture 預覽同套用。佢可以套用去 Shell、所有 app、已快取 Package Discover，或者只留喺測試器。呢個唔係完整 .NET regex 或 replacement 對等聲稱。

Package Discover regex is local cached-result filtering only: it is never copied into argv or sent to HTTPS, cannot start a package query while active, disables external Search, and exposes a monotonic `NativePackageQueryAudit` epoch that changes only when a genuine CLI/HTTPS query starts. Regex mode disables Similar and ignore-special normalization; Exact invokes PCRE2 full-match semantics. PCRE2's shipped attribution is in [`THIRD-PARTY-NOTICES.txt`](../THIRD-PARTY-NOTICES.txt), included in the portable ZIP and Inno Setup installer.

Package Discover regex 只限本機已快取結果篩選：絕對唔會複製入 argv 或傳去 HTTPS、啟用時唔可以開始套件查詢、會停用外部 Search，而且有單調遞增嘅 `NativePackageQueryAudit` epoch，只有真正 CLI／HTTPS 查詢開始先會改。regex 模式會停用 Similar 同忽略特殊字元正規化；Exact 會用 PCRE2 full-match 語義。PCRE2 會用 [`THIRD-PARTY-NOTICES.txt`](../THIRD-PARTY-NOTICES.txt) 歸屬，亦會放入可攜 ZIP 同 Inno Setup installer。
