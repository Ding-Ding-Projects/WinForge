...
After controlled integration with Reference Text, native Debug and Release core executables each passed **783/783**, including **24/24** focused Morse tests. The focused `Invoke-NativeShellSmoke.ps1 -MorseRoutesOnly` run passed **13/13** across both aliases, catalog parity covers 346 fixed routes plus five dynamic families, and the exhaustive native shell passes **430/430**. The earlier isolated-branch aggregate of **741/741** is retained only as history.

同 Reference Text 受控整合後，native Debug 同 Release core 各自通過 **783/783**，包括 **24/24** 個專項 Morse 測試；`Invoke-NativeShellSmoke.ps1 -MorseRoutesOnly` 兩個 alias 合共通過 **13/13**，catalog parity 覆蓋 346 條固定 route 同五組動態家族，完整 native shell 通過 **430/430**。早前獨立 branch 嘅 **741/741** 只保留做歷史。
