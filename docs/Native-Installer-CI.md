...
- For a fresh current-`main` release, the target policy explicitly applies `--latest --prerelease=false --draft=false`, then fails closed unless `/releases/latest` resolves to that exact stable native SHA with exactly the two expected assets. Every release run verifies this invariant. A noncurrent run must restore a separately verified current-main stable native candidate whenever GitHub points Latest to a managed, prerelease, draft, wrong-SHA, or wrong-asset release. Exact amended branch/main hosted proof remains pending. · 對最新 current-`main` release，目標政策會明確套用 `--latest --prerelease=false --draft=false`，之後除非 `/releases/latest` 解析到同一個準確 stable 原生 SHA，而且準確只有兩個預期 asset，否則 fail closed。每個 release run 都要驗證呢個 invariant；noncurrent run 如果見到 GitHub Latest 指向 managed、prerelease、draft、錯 SHA 或錯 asset release，就要恢復一個已另行核實嘅 current-main stable 原生 candidate。修正版 branch／main 嘅準確 hosted 證明仍待完成。
...
The amended policy now requires explicit stable/draft flags for current-main promotion, fail-closed stable-native/two-asset Latest verification on every release run, and verified current-main candidate restoration by noncurrent runs when GitHub points to managed or invalid Latest. Exact amended branch/main run IDs, tags, and SHAs remain pending and must be recorded only after hosted success. Runs 29673079883, 29673310778, and 29673965179 must not be used to claim this automatic invariant. · 修正版政策而家要求 current-main promotion 明確設定 stable／draft flags、每個 release run 都 fail-closed 驗證 Latest 係 stable native 同準確兩個 asset，並要求 noncurrent run 喺 GitHub 指向 managed 或無效 Latest 時恢復已核實 current-main candidate。修正版 branch／main 嘅準確 run ID、tag 同 SHA 仍待 hosted 成功後先記錄；不可用 run 29673079883、29673310778 或 29673965179 聲稱自動 invariant 已證明。
