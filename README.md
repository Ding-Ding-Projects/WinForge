...
**Current collation and evidence · 目前排序及證據：** Commit `c6f8a24d52e4949596efd58e070e0601a2939511` aligns Word Frequency `CurrentCultureIgnoreCase` with managed .NET width/kana tailoring: fullwidth case folds while width/kana remain significant, canonical-equivalent terms keep stable order, and a shared initialized collator removes per-comparison serialization. The named total remains assistive-technology queryable without a per-edit polite live announcement; attribution is in `THIRD-PARTY-NOTICES.txt`. `HasNativeRenderer` and the ledger remain **27/346 fixed routes**, leaving **319** plus five dynamic families and **27 `in-progress` / 319 `not-started`** rows. Native Debug and Release core each pass **717/717**, including Text Analysis **87/87**; focused UIA passes **40/40**; post-c6 native shell passes **388/388 (388 passed, 0 failed)**; and catalog parity covers 346 routes plus five families. The repo-local LowLevel checkout was present but not callable in this Codex session; the fresh `wordfreq` driver retry rejected a blank/near-uniform fallback and retained no PNG. All three routes remain honestly `capture-blocked` and `in-progress`. · Commit `c6f8a24d52e4949596efd58e070e0601a2939511` 對齊詞頻 `CurrentCultureIgnoreCase` 同 managed .NET 闊度／假名 tailoring：fullwidth case 會折疊，但闊度／假名仍有分別；canonical-equivalent 詞語保持穩定次序，而共用已初始化 collator 移除每次比較嘅序列化。命名 total 仍可畀輔助科技查詢，但每次修改唔再發 polite live announcement；歸屬資料喺 `THIRD-PARTY-NOTICES.txt`。`HasNativeRenderer` 同 ledger 維持 **27/346** 條固定 route，仲有 **319** 條同五組家族，並保持 **27 `in-progress` / 319 `not-started`**。Debug／Release core 各 **717/717**，Text Analysis **87/87**，專項 UIA **40/40**，c6 後完整 native shell **388/388**，catalog parity 覆蓋 346 條加五組家族。本機 LowLevel checkout 喺度但呢個 session 冇可呼叫工具；最新 `wordfreq` driver 重試拒絕空白／近乎單色 fallback，冇保留 PNG。三條 route 如實保持 `capture-blocked` 同 `in-progress`。
