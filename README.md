...
For a release-matched local build, pass the same explicit version to both scripts, for example `build.bat /s 1.1.348` followed by `build-installer.bat /s 1.1.348`. The installer script prints every required artifact path and SHA-256; it never publishes, tags, or signs. · 要做同 release 對得上嘅本機 build，就對兩個 script 傳同一個明確版本，例如先行 `build.bat /s 1.1.348`，再行 `build-installer.bat /s 1.1.348`。Installer script 會印出所有必要資產路徑同 SHA-256；佢永遠唔會 publish、tag 或簽名。
...
- **Resilient RustDesk install · 穩陣 RustDesk 安裝** — the RustDesk module tries the exact WinGet package first, then recovers from the current catalog gap by validating and installing the latest official Windows x64 release (URL, size, PE header, and SHA-256) without bundling the upstream binary. · RustDesk 模組會先試準確 WinGet package；遇到目前 catalog 缺口就驗證並安裝最新官方 Windows x64 發佈（URL、大小、PE header 同 SHA-256），唔會將上游 binary 打包入嚟。
...
- **Shared experience controls · 共用體驗控制** — shared live settings include the emoji-message switch, user-renamable School mode with credential-vault recovery, opt-in serialized notification narration, an offline changelog viewer with date/regex filtering and export, a complete bundled feature/wiki documentation browser, the new-tab picker with the shared regex builder, pinned tabs, scheduled settings with validated local/API/Home Assistant sources, a local Support Tickets recovery desk, and local TOTP pairing QR generation. The remaining universal contract is documented as unfinished work rather than implied. · 共用即時設定包括 emoji 訊息開關、可改名並用 credential vault 解鎖嘅 School mode、選擇性序列化通知旁白、附日期／regex 篩選同匯出嘅離線變更紀錄、完整捆綁功能／wiki 文件瀏覽器、接駁共用 regex builder 嘅新分頁 picker、釘選分頁、支援驗證本機／API／Home Assistant 來源嘅排程設定、本機支援工單復原台，同本機 TOTP 配對 QR 產生。其餘共用合約會清楚記錄做未做，唔會扮成已完成。
...
- **Dim-sum startup surprise · 啟動點心驚喜** — after the first usable layout, eligible launches make one fresh 10% draw, then asynchronously show a local cached image with the catalog's exact English and Traditional Chinese name. Three verified `catalog-v1*` public release partitions supply the URL mapping; the image is cached only under application data, and School mode, first-run terms, deep links, minimized launches, and visible update/error work suppress the surprise. An unavailable or malformed source simply leaves startup usable. · 第一個可用 layout 之後，合資格啟動會每次重新抽一次 10% 機會，再用非阻塞方式顯示本機 cache 圖片同 catalog 準確英文／繁體中文名。三個已驗證 `catalog-v1*` public release partition 提供 URL 對應；圖片只會 cache 喺 application data，School mode、首次 terms、deep link、最小化啟動同可見 update／error 工作會停用驚喜。來源唔得或者格式錯就照樣開到 app。
...
- **Bounded guided regex builder · 有界引導式正則砌法** — the managed .NET 11 tester constructs literals, character classes, anchors, groups, alternation, and quantifiers; the same full builder is synchronized with fifteen XAML/code-built search surfaces, including the new-tab picker and Command Palette, while plain text remains the default. The inventory now records 102 search/query controls (15 builder-backed), with the remaining ordinary, specialized, and dropdown/menu/picker migrations explicit. Raw editing, five flags, session-only samples, live matches/captures, replacement preview, and explicit copy remain local, size-bounded, and timeout-protected. · 正式 .NET 11 測試器可引導砌字面文字、字元類、錨點、群組、二選一同量詞；同一個完整版砌法已同十五個 XAML／code-built 搜尋介面同步，包括新分頁 picker 同 Command Palette，而純文字繼續做預設。清單而家記低 102 個搜尋／查詢控制（15 個已接 builder），其餘一般、專用同 dropdown／menu／picker 遷移會照實列出。原始編輯、五旗標、只限今次工作階段 sample、即時配對／擷取、替換預覽同明確複製全部只喺本機、有大小上限兼有超時保護。
...
The current contract is **35/35**, covering guided tokens, syntax failures, Unicode, multiline anchors, capture groups, no-match, zero-width progress, result/size caps, adversarial timeout, plain-text-versus-regex behavior, the complete changelog `Spec` handoff, all synchronized flags/session state, every integrated surface, the full shared control, and the complete XAML/code-built inventory. · 目前合約係 **35/35**，覆蓋引導 token、語法錯誤、Unicode、多行錨點、擷取群組、無配對、零寬度安全推進、結果／大小上限、對抗式超時、純文字對 regex 行為、完整 changelog `Spec` 交接、全部同步旗標／session 狀態、每個已整合介面、完整版共用控制，同完整 XAML／code-built 清單。
...
These captures came from the self-contained WinForge build on a dedicated hidden desktop. They show the actual Settings, About/changelog search, tab shell, Support Tickets, TOTP authenticator, and rendered offline-documentation surfaces; they are not mockups or design exports. The new-tab picker is not pictured in this pass because the real artifact opened its first-run Terms & Conditions consent surface and no legal consent was accepted; the functional and source evidence remains separate and explicit. · 以下擷取來自專用隱藏 desktop 上面嘅自包含 WinForge build，顯示真實 Settings、About／changelog 搜尋、分頁外殼、Support Tickets、TOTP authenticator 同離線文件渲染介面；唔係 mockup 或 design export。今次冇放新分頁 picker 圖，因為真實 artifact 開出嚟係首次 Terms & Conditions consent surface，而冇人代用戶接受法律同意；功能同 source evidence 會分開清楚記錄。
