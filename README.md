...
- **Auditable package management · 可審核套件管理** — the in-app [Package Manager](docs/wiki/Package-Manager.md) covers 11 engines and nine review-first views, with a shared operation queue, secure bundle/source handling, credential-free proxy command construction, validated vcpkg triplets, and responsive bilingual controls. · App 內[套件管理器](docs/wiki/Package-Manager.md)涵蓋 11 個引擎同九個先檢視後執行畫面，附共用操作佇列、安全清單／來源處理、唔會將認證放入指令嘅 proxy 設定、已驗證 vcpkg triplet，同響應式雙語控制。
