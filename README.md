...
| **Cake Factory & Farm · 蛋糕工廠與農場** | HTML5 reactor-powered cake factory game with cow milk provenance, finite supplies/utilities, timed instrumented ingredient factories, manual HACCP gates and signed `.cake` files. <br> HTML5 反應堆供電蛋糕工廠遊戲，附牛奶來源、有限補給／公用工程、計時同具遙測嘅原料工廠、手動 HACCP 放行關卡同已簽署 `.cake` 檔。 | `cakefactory` |
