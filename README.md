...
- **Bounded guided regex builder · 有界引導式正則砌法** — the managed .NET 11 tester constructs literals, character classes, anchors, groups, alternation, and quantifiers; the same full builder is synchronized with fourteen XAML/code-built search surfaces, including the new-tab picker, while plain text remains the default. The inventory now records 102 search/query controls (14 builder-backed), with the remaining ordinary, specialized, and dropdown/menu/picker migrations explicit. Raw editing, five flags, session-only samples, live matches/captures, replacement preview, and explicit copy remain local, size-bounded, and timeout-protected. · 正式 .NET 11 測試器可引導砌字面文字、字元類、錨點、群組、二選一同量詞；同一個完整版砌法已同十四個 XAML／code-built 搜尋介面同步，包括新分頁 picker，而純文字繼續做預設。清單而家記低 102 個搜尋／查詢控制（14 個已接 builder），其餘一般、專用同 dropdown／menu／picker 遷移會照實列出。原始編輯、五旗標、只限今次工作階段 sample、即時配對／擷取、替換預覽同明確複製全部只喺本機、有大小上限兼有超時保護。
