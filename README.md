...
**EN —** A genuine C++20/C++/WinRT rewrite is now underway beside the shipping managed app. Debug and Release x64 native WinUI 3 builds finish with 0 errors, the inventory covers **346 fixed routes + five dynamic route families**, the complete native test executable passes **400/400 in Debug** and **400/400 in Release** (354 core route/package-manager/utility checks plus 46 parser checks), and the elevated process-owned UI Automation shell passes **185/185**. Release PE inspection also finds a zero COM descriptor and no `coreclr`, `hostfxr`, or `mscoree` import. This is a migration milestone, not a false full-port claim: unported features display an explicit pending page, and the managed app remains authoritative until every parity row passes.

**粵語 —** 真正嘅 C++20／C++/WinRT 重寫而家會同發佈中受控 app 並行。Debug 同 Release x64 原生 WinUI 3 都以 0 errors 建置，清單涵蓋 **346 條固定路線 + 五組動態路線**，完整原生測試 executable 喺 Debug 係 **400/400**、Release 亦係 **400/400**（354 個 core route／package-manager／utility 檢查加 46 個 parser 檢查），而提權、只控制自己 process 嘅 UI Automation shell 亦係 **185/185**。Release PE 審查亦證實 COM descriptor 係零，而且冇 `coreclr`、`hostfxr` 或 `mscoree` import。呢個係遷移里程碑，唔係扮成全功能已移植：未完成功能會顯示明確 pending 頁，而每項對等清單通過之前，受控 app 仍然係權威版本。

### Native safe regex search & builder · 原生安全正規搜尋同建立器

**EN —** `RegexSearchSurface.h` explicitly registers every current native search control: Shell catalog, All Apps, and cached Package Discover. Each routes through bounded PCRE2-16. The full four-step `module.regextester` wizard now provides flags; escaped literal recipes (contains, exact, prefix, suffix, whole word); route/package/version starters; character classes and tokens; groups, alternatives, word-boundary and lookaround assertions, quantifiers, a safe match/capture preview, and target-aware Apply. Invalid patterns cannot be applied. Package Discover remains deliberately local-cache only: its builder target forces Discover, retains only a genuine Discover search cache, never enters a package argv or HTTPS request, disables remote Search, and leaves the monotonic query audit unchanged. PCRE2 uses UTF/UCP semantics with no JIT and strict interactive resource limits; it is not a claim of full .NET regex or replacement parity.

**粵語 —** `RegexSearchSurface.h` 明確登記咗而家每個原生搜尋控制：Shell 目錄、所有 app 同已快取 Package Discover；每個都會經有界 PCRE2-16。完整四步 `module.regextester` 精靈而家有旗標、已 escape literal recipe（包含、完全、開頭、結尾、完整字）、路線／套件／版本起始式、字元類別同 token、group、alternative、word-boundary 同 lookaround assertion、quantifier、安全 match／capture 預覽同按目標 Apply；無效模式唔可以套用。Package Discover 有意只限本機快取：builder 目標會強制 Discover、只保留真正 Discover 搜尋快取、絕對唔會入套件 argv 或 HTTPS 請求、會停用遠端 Search，而且單調遞增嘅 query audit 唔會改。PCRE2 用 UTF/UCP 語義、唔用 JIT 同有嚴格互動資源限制；呢個唔係完整 .NET regex 或 replacement 對等聲稱。
...
Current aggregate evidence after the native UUID v7 slice is **400/400 native tests** in Debug and Release (354 core + 46 parser) and **185/185 native UI Automation checks**; older per-feature counts below are retained as historical slice evidence. · 原生 UUID v7 批次之後目前整體證據係 Debug 同 Release **原生測試 400/400**（354 個 core + 46 個 parser）同 **原生 UI Automation 185/185**；下面較舊嘅逐功能數字保留作歷史批次證據。
...
**EN —** The complete native executable passes **400/400 in Debug** and **400/400 in Release** (354 core route/package-manager/utility checks plus 46 parser checks), including 42 focused Package Mutation Coordinator checks, the safe PCRE2 core/builder, the UUID v7 core vectors, recipe/assertion vectors, and the three-surface regex contract. The process-owned elevated UI Automation smoke passes **185/185**, including UUID v7 generation/decoding/copy/rerender/alias checks, invalid-Apply blocking, Shell/All Apps target application, deterministic Package Discover/no-query routing, the Package Manager batch-consent policy, and horizontal-bound checks. This verifies routing, controls, fail-closed behavior, accessibility, and clipping—not normal-integrity external manager execution. On 2026-07-16, the repository driver launched UUID v7 but `CopyFromScreen` was unavailable and its `PrintWindow` fallback was blank/near-uniform. LowLevel MCP separately launched UUID v7 on an isolated desktop; its inspected 1980×1320 full-window frame contained only a title bar and blank client surface, and its 1958×1264 client capture was blank. No PNG is accepted as native visual evidence, and the gallery image below remains a **managed production reference** only.

**粵語 —** 完整原生 executable 喺 Debug 係 **400/400**、Release 亦係 **400/400**（354 個 core route／package-manager／utility 檢查加 46 個 parser 檢查），包括 42 個 Package Mutation Coordinator 專項檢查、安全 PCRE2 core／builder、UUID v7 core vectors、recipe／assertion vectors 同三個搜尋位置 regex 合約。只控制自己 process 嘅提升權限 UI Automation smoke 係 **185/185**，包括 UUID v7 產生／解碼／Copy／重建／alias 檢查、無效 Apply 阻擋、Shell／All Apps 目標套用、確定性 Package Discover／唔開查詢導覽、Package Manager 批次確認政策同水平邊界 assertion。呢個驗證導覽、控制項、fail-closed 行為、無障礙同裁切，而唔代表正常 integrity 外部管理器執行。2026-07-16，repository driver 開啟 UUID v7，但 `CopyFromScreen` 用唔到，而 `PrintWindow` fallback 係空白／接近單色。LowLevel MCP 亦喺隔離 desktop 開啟 UUID v7；檢查過嘅 1980×1320 完整視窗 frame 只得 title bar 加空白 client surface，而 1958×1264 client 擷取亦係空白。冇 PNG 會被當成原生視覺證據，下面畫廊圖片仍然只係**受控正式版參考**。
