...
**EN —** The native shell now measures pages to the available viewport, keeps a vertical overflow path, and enables horizontal scrolling as a last-resort accessibility escape hatch so bilingual headings, action rows, and the Package Manager toolbar are not silently clipped. [`.github/workflows/native-release.yml`](.github/workflows/native-release.yml) is the sole release publisher. It builds and tests the C++ Release x64 runtime, packages one clean runtime, compiles and smoke-tests `WinForge-Native-Setup.exe`, and publishes exactly two native assets: the portable ZIP and installer. Every successful branch push publishes a native-only prerelease; the target `main` policy is stable/Latest for the current exact SHA and stable non-Latest for an older SHA. Corrective hardening is adding an explicit release edit, fail-closed Latest/asset verification, and the official Node 24 actions `checkout@v7`, `upload-artifact@v7`, `download-artifact@v8`, and `setup-msbuild@v3`; hosted proof of that automatic postcondition remains pending. · 原生 shell 而家會按可用 viewport 量度頁面，保留垂直溢出路徑，並以水平捲動作最後嘅無障礙保底，避免雙語標題、動作列同套件管理工具列靜默被裁切。[`.github/workflows/native-release.yml`](.github/workflows/native-release.yml) 係唯一 release publisher。佢會建置同測試 C++ Release x64 runtime、封裝一份乾淨 runtime、編譯同 smoke-test `WinForge-Native-Setup.exe`，並只發佈可攜 ZIP 同安裝程式兩個原生 asset。成功 branch push 會發佈 native-only prerelease；`main` 目標政策係目前 exact SHA 做 stable/Latest，較舊 SHA 做 stable non-Latest。修正加固正加入明確 release edit、fail-closed Latest／asset 驗證，同官方 Node 24 actions `checkout@v7`、`upload-artifact@v7`、`download-artifact@v8`、`setup-msbuild@v3`；呢個自動 postcondition 嘅 hosted 證明仍待完成。
...
**Latest-channel correction · Latest channel 修正：** `gh release create --latest` did not move `/releases/latest` from historical managed `v1.0.256`. The official manual repair `gh release edit native-v1.0.44 --latest` made the endpoint resolve exactly to `native-v1.0.44`. The completed runs therefore prove exact-SHA native-only releases, but not the automatic Latest postcondition. Exact corrective branch/main run IDs, tags, and SHAs remain pending and will be recorded only after hosted success. · `gh release create --latest` 冇將 `/releases/latest` 由歷史 managed `v1.0.256` 移走；用官方 `gh release edit native-v1.0.44 --latest` 手動修復後，endpoint 先準確解析到 `native-v1.0.44`。已完成 run 證明 exact-SHA native-only release，但未證明自動 Latest postcondition；準確修正 branch／main run ID、tag 同 SHA 仍待 hosted 成功後先記錄。
