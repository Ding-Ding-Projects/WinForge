...
**EN —** The native shell now measures pages to the available viewport, keeps a vertical overflow path, and enables horizontal scrolling as a last-resort accessibility escape hatch so bilingual headings, action rows, and the Package Manager toolbar are not silently clipped. [`.github/workflows/native-release.yml`](.github/workflows/native-release.yml) is the sole release publisher. It pins the VS 2022 runner, proves/provisions its C++ v143 UWP toolset, builds and tests the C++ Release x64 runtime, packages a clean portable runtime, compiles and smoke-tests `WinForge-Native-Setup.exe`, and publishes exactly two native assets: the portable ZIP and installer. Every push and every pull request into `main` runs the complete native gate. Every successful branch push publishes a native-only prerelease; the current `origin/main` tip publishes the stable/latest release, while an older exact-SHA `main` run publishes a stable non-latest release. Pull requests validate without publishing. · 原生 shell 而家會按可用 viewport 量度頁面，保留垂直溢出路徑，並以水平捲動作最後嘅無障礙保底，避免雙語標題、動作列同套件管理工具列靜默被裁切。[`.github/workflows/native-release.yml`](.github/workflows/native-release.yml) 係唯一 release publisher。佢會固定 VS 2022 runner、驗證／準備 C++ v143 UWP 工具組、建置同測試 C++ Release x64 runtime、封裝乾淨可攜 runtime、編譯同 smoke-test `WinForge-Native-Setup.exe`，並只發佈兩個原生 asset：可攜 ZIP 同安裝程式。每次 push 同每個合併去 `main` 嘅 pull request 都會執行完整原生 gate；成功 branch push 會發佈 native-only prerelease；目前 `origin/main` tip 會發佈 stable/latest，而較舊 exact-SHA `main` run 會發佈 stable non-latest。Pull request 只驗證，唔發佈。

**CI proof · CI 證明：** The current native-only release contract passes local PowerShell parsing, Prettier, actionlint 1.7.12, installer-contract validation, a 292-file clean-stage simulation, all-48-PE scanning, the release-mode matrix, manual CLR/apphost/PDB rejection checks, and the existing Inno native-PE check. Exact hosted proof for the current branch and subsequent `main` integration remains pending; the older non-`main` run predates and does not describe the current branch-release policy. · 目前 native-only release 合約已通過本機 PowerShell parser、Prettier、actionlint 1.7.12、installer contract、292-file 乾淨 stage 模擬、全部 48 個 PE scan、release-mode matrix、手動 CLR／apphost／PDB 拒絕檢查，同既有 Inno native-PE 檢查。目前 branch 同之後 `main` 整合嘅準確 hosted 證明仍待完成；舊 non-`main` run 早於而家政策，唔代表目前 branch-release 行為。
