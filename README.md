...
**CI proof · CI 證明：** The prior [`main` native-release run](https://github.com/codingmachineedge/WinForge/actions/runs/29453406232) proved the C++ Release, native test, catalog-parity, portable-runtime, and setup-build stages; it predates the silent install/uninstall smoke above. The next `main` run is the first proof for that new installer lifecycle gate. · 之前嘅 [`main` native-release 執行](https://github.com/codingmachineedge/WinForge/actions/runs/29453406232) 證明咗 C++ Release、原生測試、目錄對等、可攜 runtime 同 setup-build 階段；佢早過上面新增嘅靜默安裝／解除安裝 smoke。下一次 `main` 執行先會係新安裝程式生命週期 gate 嘅第一份證明。
