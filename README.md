...
**EN —**
- **All-in-one control center** — 318 modules and 1,213 total features in one app; OSS-inspired additions are remade as native WinForge tabs instead of installer-only launchers. · **全方位控制中心** — 一個 app 有 318 個模組同 1,213 項總功能；受開源 app 啟發嘅新增功能會重製成 WinForge 原生分頁，而唔係只做安裝／啟動器。
...
| **Dew Encryption · Dew 加密** | Git-backed file history, rollback-safe restore and password-encrypted 7z export with hidden file names. <br> Git 檔案歷史、可 rollback 安全還原，同連檔名都隱藏嘅密碼加密 7z 匯出。 | `dew-encryption` |
