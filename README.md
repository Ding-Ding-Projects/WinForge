...
**EN —** A genuine C++20/C++/WinRT rewrite is now underway beside the shipping managed app. Debug and Release x64 native WinUI 3 builds finish with 0 errors, the inventory covers **346 fixed routes + five dynamic route families**, the complete native test executable passes **326/326 in Debug** and **326/326 in Release** (280 core route/package-manager/codec/Roman checks plus 46 parser checks), and the elevated process-owned UI Automation shell passes **146/146**. Release PE inspection also finds a zero COM descriptor and no `coreclr`, `hostfxr`, or `mscoree` import. This is a migration milestone, not a false full-port claim: unported features display an explicit pending page, and the managed app remains authoritative until every parity row passes.

**粵語 —** 真正嘅 C++20／C++/WinRT 重寫而家會同發佈中受控 app 並行。Debug 同 Release x64 原生 WinUI 3 都以 0 errors 建置，清單涵蓋 **346 條固定路線 + 五組動態路線**，完整原生測試 executable 喺 Debug 係 **326/326**、Release 亦係 **326/326**（280 個 core route／package-manager／codec／羅馬數字檢查加 46 個 parser 檢查），而提權、只控制自己 process 嘅 UI Automation shell 亦係 **146/146**。Release PE 審查亦證實 COM descriptor 係零，而且冇 `coreclr`、`hostfxr` 或 `mscoree` import。呢個係遷移里程碑，唔係扮成全功能已移植：未完成功能會顯示明確 pending 頁，而每項對等清單通過之前，受控 app 仍然係權威版本。
...
Current aggregate evidence after the Package Manager bundle-workspace slice is **326/326 native tests** and **146/146 native UI Automation checks**; older per-feature counts below are retained as historical slice evidence. · 套件管理 bundle 工作區批次之後目前整體證據係**原生測試 326/326**同**原生 UI Automation 146/146**；下面較舊嘅逐功能數字保留作歷史批次證據。
...
**EN —** The **shipping managed .NET Package Manager** remains the production implementation. It has substantial functionality across 11 Windows package engines and nine views: search/install, update and uninstall operations, portable bundles, source management, ignored/pinned/snoozed updates, engine setup, saved settings, scheduling, notifications, and a bounded shared operation queue with history, output, cancellation and retry. The new **C++20/C++/WinRT batch is not a full UniGetUI clone or parity cutover**. It currently provides non-mutating result queries for **Discover, Updates, and Installed**; Discover locally re-filters cached raw rows by **Both, Name, ID, Exact, or Similar** mode plus case-sensitive and ignore-special-character options. Cached Discover, Updates, and Installed rows support source-aware, action-scoped multi-select. A delimiter-safe normalized manager/action/ID/source identity de-duplicates a current cached view, and a selected batch adds at most 25 exact Install, Update, or Uninstall argv previews to Operations; selection clears when the query or view changes, and no package command runs. Read-only per-row **Details** queries parse safe common fields above bounded raw output; local persisted update-suppression rules (**all-version ignores, update-version pins, and configurable 1/7/14/30-day snoozes**) hide matching Updates rows; and a read-only **Sources command probe** withholds raw configuration and diagnostics until manager-specific secret redaction is proven. Per-row Install/Update/Uninstall and Update All still create preview-only executable/argv plans in Operations using the same validated command builders; those plans now persist as structured queue entries with run-next/run-last reorder and retry-preview markers, but they do not execute package-manager mutation processes. Package-view, search-text, sort-mode, Discover filter preferences, ignored/pinned/snoozed rules, manager-filter preferences, and the default snooze duration persist in native JSON state, while Setup bootstrap, scheduler integration, the broader per-manager Settings surface, and the real mutation Operations coordinator remain gated.

**粵語 —** **發佈中受控 .NET 套件管理器**仍然係正式版本。佢喺 11 個 Windows 套件引擎同九個檢視提供大量實際功能：搜尋／安裝、更新同解除安裝、可攜清單、來源管理、忽略／釘選／暫停更新、引擎設定、已儲存設定、排程、通知，同有上限嘅共用操作佇列（附歷史、輸出、取消同重試）。新嘅 **C++20／C++/WinRT 批次唔係完整 UniGetUI 複製品，亦未到功能對等切換**；目前提供**搜尋安裝、可更新同已安裝**嘅非修改結果查詢；Discover 可以喺已快取原始資料列本機用**名稱及 ID、套件名稱、套件 ID、完全相同、相似套件**模式，加上大小寫敏感同忽略特殊字元選項重新篩選。已快取嘅 Discover、Updates 同 Installed 資料列支援識別來源、識別動作嘅多選。有長度前綴、已正規化嘅管理器／動作／ID／來源 identity 會為目前快取檢視去重；所選批次最多只會喺 Operations 加入 25 條準確安裝／更新／解除安裝 argv 預覽；查詢或者檢視改變時會清除選擇，完全唔會執行套件指令。逐列只讀**詳細資料**查詢會喺有界原始輸出之上解析安全常用欄位；本機持久化更新隱藏規則（**全部版本忽略、更新版本釘選同可設定 1／7／14／30 日暫停**）會隱藏相符 Updates 資料列；另有只讀**來源指令探測**，但逐管理器機密遮罩未證實之前會刻意隱藏原始設定同診斷。逐列安裝／更新／解除安裝同全部更新仍然只會用同一套已驗證 command builder 喺操作檢視建立 executable／argv 預覽計劃；呢啲計劃而家會以結構化佇列項目保存，支援下一個／最後執行排序標記同只供預覽嘅重試標記，但唔會執行套件修改 process。套件檢視、搜尋文字、排序模式、Discover 篩選偏好、忽略／釘選／暫停規則、管理器篩選偏好同預設暫停時長會保存喺原生 JSON 狀態；Setup bootstrap、排程整合、更完整逐管理器設定同真正修改操作協調器仍然鎖住。
...
The package-manager Settings tab now persists package-view, search-text, sort-mode, Discover filter mode/toggles, ignored/pinned/snoozed update rules, manager-filter preferences, and the default 1/7/14/30-day snooze duration in native JSON state. The Operations tab now keeps structured preview queue entries, imports old string history, exposes run-next/run-last/retry-preview controls, and still offers clear history. The broader per-manager defaults, backup, restore, scheduler, notification, and real mutation queue-coordinator workflows remain gated.

套件管理嘅 Settings tab 而家會用原生 JSON 狀態保存套件檢視、搜尋文字、排序模式、Discover 篩選模式／toggle、忽略／釘選／暫停更新規則同管理器篩選偏好。Operations tab 而家會保存結構化預覽佇列項目、匯入舊字串歷史、提供下一個／最後執行／重試預覽控制，並保留清除歷史動作。更完整逐管理器預設、備份、還原、排程、通知同真正修改佇列協調器流程仍然鎖住。

**EN —** The complete native executable passes **326/326 in Debug** and **326/326 in Release** (280 core route/package-manager/codec/Roman checks plus 46 parser checks), including nine cached-Discover-filter cases, three source/action selection-identity cases, and the new version/source-aware bundle merge and incompatible-audit partition case. The process-owned elevated UI Automation smoke passes **146/146**, including Discover-filter accessibility/clipping plus the Bundle empty-workspace, export/import-control, and clipping assertions. That elevated smoke verifies routing—including exact Discover/Updates/Installed/Ignored aliases—controls, fail-closed gates and accessibility, not live external manager execution. The separate normal-integrity live smoke is still externally blocked because Windows returned a token that failed the standard-user proof even for an interactive scheduled task registered with `RunLevel=Limited`; no live-query pass is claimed. LowLevel MCP is present on disk at `C:\Users\Administrator\Documents\GitHub\lowlevel-computer-use-mcp`; this Codex session still does not expose a callable LowLevel MCP namespace, but the repo-local `lowlevel-computer-use-cheap.exe` fallback created `WinForgeBundleWorkspaceAudit`, launched `WinForge.exe` for `package-discover`, `package-installed`, and `package-bundles`, listed each headless WinUI window, and captured each HWND. Every inspected client frame was blank/near-uniform, so no PNG is accepted as visual proof. The gallery image below is therefore labelled as a **managed production reference**, never current native visual evidence.

**粵語 —** 完整原生 executable 喺 Debug 係 **326/326**、Release 亦係 **326/326**（280 個 core route／package-manager／codec／羅馬數字檢查加 46 個 parser 檢查），包括九個快取 Discover 篩選專項案例、三個識別來源／動作嘅選擇 identity 案例，同新嘅識別版本／來源 bundle 合併同不相容審計分流案例；只控制自己 process 嘅提升權限 UI Automation smoke 係 **146/146**，包括 Discover 篩選無障礙／裁切，同 Bundles 空白工作區、匯出／匯入控制同裁切 assertion。提升權限 smoke 驗證導覽（包括 Discover／Updates／Installed／Ignored 準確 alias）、控制項、fail-closed 閘同無障礙，唔代表真實外部管理器執行。獨立嘅正常權限 live smoke 仍然受外部環境阻擋：即使互動式排程工作已登記做 `RunLevel=Limited`，Windows 回傳嘅 token 仍然未能通過標準使用者證明，所以唔會聲稱 live query 已通過。LowLevel MCP checkout 存在於 `C:\Users\Administrator\Documents\GitHub\lowlevel-computer-use-mcp`；呢個 Codex session 仍然冇曝露可直接呼叫嘅 LowLevel MCP namespace，但 repo 本機 `lowlevel-computer-use-cheap.exe` fallback 已建立 `WinForgeBundleWorkspaceAudit`、為 `package-discover`、`package-installed` 同 `package-bundles` 開啟 `WinForge.exe`、列出每個無頭 WinUI 視窗並用 HWND 擷取。檢查過嘅 client frame 全部都係空白／接近單色，所以冇 PNG 會被當成視覺證據。所以下面畫廊圖片只係**受控正式版參考**，絕對唔係目前原生版視覺證據。
...
| **Package Manager · 套件管理** | Shipping managed workspace over 11 Windows package engines and nine views. The C++ migration exposes three non-mutating result views, read-only Details, update-rule state, a source probe, preview-only argv plans, and a bounded metadata-only UniGetUI-v3 Bundle workspace: Discover/Installed selection append, compatible/incompatible audit separation, safe import/export, and no package execution. <br> 發佈中受控工作區支援 11 個 Windows 套件引擎同九個檢視；C++ 遷移提供三個非修改結果檢視、只讀詳細資料、更新規則狀態、來源探測、只供預覽 argv 計劃，同有界、只限 metadata 嘅 UniGetUI v3 Bundle 工作區：Discover／已安裝選擇附加、相容／不相容審計分開、安全匯入／匯出，唔會執行套件。 | `packages` · `package-discover` · `package-updates` · `package-installed` · `package-bundles` |
