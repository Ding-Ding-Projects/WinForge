...
### 2026-07-18 native line-processing wave · 原生行文字處理批次
...
**EN —** Line Tools (`module.linetools`), Line Sort & Dedupe (`module.textsort`), and Text Wrap (`module.textwrap`) now have dedicated C++/WinRT renderer source backed by one testable standard-C++ Line Processing core. The shared core preserves the managed UTF-16 line/count contracts, numbering and cleanup transforms, ordinal and natural sorting, first-win deduplication, cryptographic shuffling, paragraph wrap/reflow, prefixing, and hanging indent. `HasNativeRenderer` now covers **24/346 fixed routes**, leaving **322 fixed routes** explicitly pending plus five dynamic route families. Native Debug and Release builds each exit 0; both core executables pass **630/630**, including **70/70** focused Line Processing contracts. Catalog parity passes for all 346 fixed routes plus five dynamic families. The managed `dotnet build WinForge.sln -c Debug -p:Platform=x64` check exits 0 with 0 errors; that observed run reported 161 existing warnings.

**粵語 —** 行工具（`module.linetools`）、行排序同去重（`module.textsort`）同文字換行（`module.textwrap`）而家都有專用 C++/WinRT renderer source，背後共用一個可測試嘅標準 C++ Line Processing core。共用 core 保留 managed 版 UTF-16 分行／統計合約、編號同清理轉換、ordinal／自然排序、保留第一項去重、密碼學打亂、按段落換行／重排、加前綴同懸掛縮排。`HasNativeRenderer` 而家涵蓋 **24/346 條固定 route**，仲有 **322 條固定 route** 同五組動態 route 家族待完成。原生 Debug 同 Release build 各自 exit 0，兩個 core executable 各自通過 **630/630**，包括 Line Processing **70/70**；catalog parity 亦通過全部 346 條固定 route 同五組動態家族。Managed `dotnet build WinForge.sln -c Debug -p:Platform=x64` exit 0、0 errors；嗰次實際 run 報告 161 個既有 warning。
...
**Release provenance · 版本來源：** Every native prerelease tag is pinned to the immutable workflow commit, so a concurrent `main` update cannot relabel artifacts built from an earlier SHA. · 每個原生 prerelease 標籤都會固定去不可變嘅 workflow commit，所以就算 `main` 同時更新，都唔會將較早 SHA 建置嘅 artifacts 改標。
