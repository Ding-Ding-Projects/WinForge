...
**EN —** A genuine C++20/C++/WinRT rewrite is now underway beside the shipping managed app. Debug and Release x64 native WinUI 3 builds finish with 0 errors, the inventory covers **346 fixed routes + five dynamic route families**, the complete native test executable passes **387/387 in Debug** and **387/387 in Release** (341 core route/package-manager checks plus 46 parser checks), and the elevated process-owned UI Automation shell passes **166/166**. Release PE inspection also finds a zero COM descriptor and no `coreclr`, `hostfxr`, or `mscoree` import. This is a migration milestone, not a false full-port claim: unported features display an explicit pending page, and the managed app remains authoritative until every parity row passes.

**粵語 —** 真正嘅 C++20／C++/WinRT 重寫而家會同發佈中受控 app 並行。Debug 同 Release x64 原生 WinUI 3 都以 0 errors 建置，清單涵蓋 **346 條固定路線 + 五組動態路線**，完整原生測試 executable 喺 Debug 係 **387/387**、Release 亦係 **387/387**（341 個 core route／package-manager 檢查加 46 個 parser 檢查），而提權、只控制自己 process 嘅 UI Automation shell 亦係 **166/166**。Release PE 審查亦證實 COM descriptor 係零，而且冇 `coreclr`、`hostfxr` 或 `mscoree` import。呢個係遷移里程碑，唔係扮成全功能已移植：未完成功能會顯示明確 pending 頁，而每項對等清單通過之前，受控 app 仍然係權威版本。
...
Current aggregate evidence after the native package-batch slice is **387/387 native tests** in Debug and Release (341 core + 46 parser) and **166/166 native UI Automation checks**; older per-feature counts below are retained as historical slice evidence. · 原生套件批次之後目前整體證據係 Debug 同 Release **原生測試 387/387**（341 個 core + 46 個 parser）同 **原生 UI Automation 166/166**；下面較舊嘅逐功能數字保留作歷史批次證據。
...
**EN —** The complete native executable passes **387/387 in Debug** and **387/387 in Release** (341 core route/package-manager checks plus 46 parser checks), including 42 focused Package Mutation Coordinator checks, the safe PCRE2 core/builder, and cached-Discover regex vectors. The process-owned elevated UI Automation smoke passes **166/166**, including the Package Manager batch-consent policy and horizontal-bound checks. This verifies routing, controls, fail-closed behavior, accessibility, and clipping—not normal-integrity external manager execution. On 2026-07-16, the repository driver attempted `package-discover`, `package-updates`, `package-installed`, and `package-operations`; `CopyFromScreen` was unavailable and its `PrintWindow` fallback was blank/near-uniform. LowLevel MCP separately captured the same isolated-desktop HWNDs and the inspected full-window frames contained only a title bar and blank client surface. No PNG is accepted as native visual evidence, and the gallery image below remains a **managed production reference** only.

**粵語 —** 完整原生 executable 喺 Debug 係 **387/387**、Release 亦係 **387/387**（341 個 core route／package-manager 檢查加 46 個 parser 檢查），包括 42 個 Package Mutation Coordinator 專項檢查、安全 PCRE2 core／builder 同已快取 Discover regex vectors。只控制自己 process 嘅提升權限 UI Automation smoke 係 **166/166**，包括 Package Manager 批次確認政策同水平邊界 assertion。呢個驗證導覽、控制項、fail-closed 行為、無障礙同裁切，而唔代表正常 integrity 外部管理器執行。2026-07-16，repository driver 嘗試 `package-discover`、`package-updates`、`package-installed` 同 `package-operations`；`CopyFromScreen` 用唔到，而 `PrintWindow` fallback 係空白／接近單色。LowLevel MCP 亦喺隔離 desktop 擷取同樣 HWND，而檢查過嘅完整視窗 frame 只得 title bar 加空白 client surface。冇 PNG 會被當成原生視覺證據，下面畫廊圖片仍然只係**受控正式版參考**。
