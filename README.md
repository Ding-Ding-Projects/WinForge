...
**EN —** A genuine C++20/C++/WinRT rewrite is now underway beside the shipping managed app. Debug and Release x64 native WinUI 3 builds finish with 0 errors, the inventory covers **346 fixed routes + five dynamic route families**, the complete native test executable passes **374/374 in Debug** and **374/374 in Release** (328 core route/package-manager checks plus 46 parser checks), and the elevated process-owned UI Automation shell passes **164/164**. Release PE inspection also finds a zero COM descriptor and no `coreclr`, `hostfxr`, or `mscoree` import. This is a migration milestone, not a false full-port claim: unported features display an explicit pending page, and the managed app remains authoritative until every parity row passes.

**粵語 —** 真正嘅 C++20／C++/WinRT 重寫而家會同發佈中受控 app 並行。Debug 同 Release x64 原生 WinUI 3 都以 0 errors 建置，清單涵蓋 **346 條固定路線 + 五組動態路線**，完整原生測試 executable 喺 Debug 係 **374/374**、Release 亦係 **374/374**（328 個 core route／package-manager 檢查加 46 個 parser 檢查），而提權、只控制自己 process 嘅 UI Automation shell 亦係 **164/164**。Release PE 審查亦證實 COM descriptor 係零，而且冇 `coreclr`、`hostfxr` 或 `mscoree` import。呢個係遷移里程碑，唔係扮成全功能已移植：未完成功能會顯示明確 pending 頁，而每項對等清單通過之前，受控 app 仍然係權威版本。

### Native safe regex search & builder · 原生安全正規搜尋同建立器

**EN —** The native Shell catalog search, All Apps filter, and Package Discover cached-result filter support bounded PCRE2-16 regular expressions. The real four-step `module.regextester` wizard provides flags, escaped literal and character-class composition, groups, alternatives, quantifiers, a safe match/capture preview, and target-aware apply. Package Discover regex is deliberately local-cache only: it never enters a package argv or HTTPS request, disables remote Search, and leaves the monotonic query audit unchanged. PCRE2 uses UTF/UCP semantics with no JIT and strict interactive resource limits; it is not a claim of full .NET regex or replacement parity.

**粵語 —** 原生 Shell 目錄搜尋、所有 app 篩選同 Package Discover 已快取結果篩選都支援有界 PCRE2-16 正規表示式。真正四步 `module.regextester` 精靈提供旗標、已 escape literal 同字元類別組合、group、alternative、quantifier、安全 match／capture 預覽同按目標套用。Package Discover regex 有意只限本機快取：絕對唔會入套件 argv 或 HTTPS 請求、會停用遠端 Search，而且單調遞增嘅 query audit 唔會改。PCRE2 用 UTF/UCP 語義、唔用 JIT 同有嚴格互動資源限制；呢個唔係完整 .NET regex 或 replacement 對等聲稱。
...
Current aggregate evidence after the native regex/search slice is **374/374 native tests** in Debug and Release (328 core + 46 parser) and **164/164 native UI Automation checks**; older per-feature counts below are retained as historical slice evidence. · 原生 regex／搜尋批次之後目前整體證據係 Debug 同 Release **原生測試 374/374**（328 個 core + 46 個 parser）同 **原生 UI Automation 164/164**；下面較舊嘅逐功能數字保留作歷史批次證據。
...
**EN —** The complete native executable passes **374/374 in Debug** and **374/374 in Release** (328 core route/package-manager checks plus 46 parser checks), including the safe PCRE2 core/builder and cached-Discover regex vectors. The process-owned elevated UI Automation smoke passes **164/164**, including Shell, All Apps, Regex Tester, Package Discover cache-only/no-query, and horizontal-bound assertions. This verifies routing, controls, fail-closed behavior, accessibility, and clipping—not normal-integrity external manager execution. On 2026-07-16, the repository driver retried Dashboard, All Apps, Regex Tester, and Package Discover, while LowLevel MCP captured Regex Tester and Package Discover on an isolated desktop; every inspected frame was a title bar plus blank/near-uniform client area. No PNG is accepted as native visual evidence, and the gallery image below remains a **managed production reference** only.

**粵語 —** 完整原生 executable 喺 Debug 係 **374/374**、Release 亦係 **374/374**（328 個 core route／package-manager 檢查加 46 個 parser 檢查），包括安全 PCRE2 core／builder 同已快取 Discover regex vectors。只控制自己 process 嘅提升權限 UI Automation smoke 係 **164/164**，包括 Shell、所有 app、Regex Tester、Package Discover 只限快取／唔開查詢同水平邊界 assertion。呢個驗證導覽、控制項、fail-closed 行為、無障礙同裁切，而唔代表正常 integrity 外部管理器執行。2026-07-16，repository driver 重新嘗試 Dashboard、所有 app、Regex Tester 同 Package Discover，而 LowLevel MCP 喺隔離 desktop 擷取 Regex Tester 同 Package Discover；每張檢查過嘅畫面都只係 title bar 加空白／接近單色 client area。冇 PNG 會被當成原生視覺證據，下面畫廊圖片仍然只係**受控正式版參考**。
