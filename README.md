...
**EN —** The **shipping managed .NET Package Manager** remains the production implementation. It has substantial functionality across 11 Windows package engines and nine views: search/install, update and uninstall operations, portable bundles, source management, ignored/pinned/snoozed updates, engine setup, saved settings, scheduling, notifications, and a bounded shared operation queue with history, output, cancellation and retry. The new **C++20/C++/WinRT batch is not a full UniGetUI clone or parity cutover**. It currently provides non-mutating result queries for **Discover, Updates, and Installed**, plus a read-only **Sources command probe** whose raw configuration and diagnostics are deliberately withheld until manager-specific secret redaction is proven. Per-row Install/Update/Uninstall and Update All now create preview-only executable/argv plans in Operations using the same validated command builders, but they do not execute package-manager processes. Package-view, search-text, sort-mode, and manager-filter preferences persist in native JSON state, while Ignored, Setup bootstrap, the broader per-manager Settings surface, and the real Operations coordinator remain gated.

**粵語 —** **發佈中受控 .NET 套件管理器**仍然係正式版本。佢喺 11 個 Windows 套件引擎同九個檢視提供大量實際功能：搜尋／安裝、更新同解除安裝、可攜清單、來源管理、忽略／釘選／暫停更新、引擎設定、已儲存設定、排程、通知，同有上限嘅共用操作佇列（附歷史、輸出、取消同重試）。新嘅 **C++20／C++/WinRT 批次唔係完整 UniGetUI 複製品，亦未到功能對等切換**；目前提供**搜尋安裝、可更新同已安裝**嘅非修改結果查詢，另有只讀**來源指令探測**，但逐管理器機密遮罩未證實之前會刻意隱藏原始設定同診斷。逐列安裝／更新／解除安裝同全部更新而家會用同一套已驗證 command builder 喺操作檢視建立只供預覽嘅 executable／argv 計劃，但唔會執行套件管理 process。套件檢視、搜尋文字、排序模式同管理器篩選偏好會保存喺原生 JSON 狀態；已忽略、Setup bootstrap、更完整逐管理器設定同真正操作協調器仍然鎖住。
...
| **Package Manager · 套件管理** | Shipping managed workspace over 11 Windows package engines and nine views. The C++ migration currently exposes three non-mutating result views, a source-command probe, and preview-only install/update/uninstall argv plans; execution and the remaining full workflows stay gated. <br> 發佈中受控工作區支援 11 個 Windows 套件引擎同九個檢視；C++ 遷移目前開放三個非修改結果檢視、一個來源指令探測，同只供預覽嘅安裝／更新／解除安裝 argv 計劃；真正執行同其餘完整流程仍然鎖住。 | `packages` · `package-discover` · `package-updates` · `package-installed` |
