...
- **Bounded guided regex builder · 有界引導式正則砌法** — the managed .NET 11 tester constructs literals, character classes, anchors, groups, alternation, and quantifiers; the same full builder is synchronized with eleven XAML search surfaces plus the code-built Settings and About searches, while plain text remains the default. The inventory now records 102 search/query controls (13 builder-backed), including the four code-built fields that remain explicitly pending. Raw editing, five flags, session-only samples, live matches/captures, replacement preview, and explicit copy remain local, size-bounded, and timeout-protected. · 正式 .NET 11 測試器可引導砌字面文字、字元類、錨點、群組、二選一同量詞；同一個完整版砌法已同十一個 XAML 搜尋介面，加上 code-built Settings 同 About 搜尋同步，而純文字繼續做預設。清單而家記低 102 個搜尋／查詢控制（13 個已接 builder），包括四個仍然明確列做後續嘅 code-built 欄位。原始編輯、五旗標、只限今次工作階段 sample、即時配對／擷取、替換預覽同明確複製全部只喺本機、有大小上限兼有超時保護。
