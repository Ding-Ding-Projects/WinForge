...
**EN —** A genuine C++20/C++/WinRT rewrite is now underway beside the shipping managed app. Debug and Release x64 native WinUI 3 builds finish with 0 errors, the inventory covers **346 fixed routes + five dynamic route families**, the complete native test executable passes **325/325 in Debug** and **325/325 in Release** (279 core route/package-manager/codec/Roman checks plus 46 parser checks), and the elevated process-owned UI Automation shell passes **144/144**. Release PE inspection also finds a zero COM descriptor and no `coreclr`, `hostfxr`, or `mscoree` import. This is a migration milestone, not a false full-port claim: unported features display an explicit pending page, and the managed app remains authoritative until every parity row passes.

**粵語 —** 真正嘅 C++20／C++/WinRT 重寫而家會同發佈中受控 app 並行。Debug 同 Release x64 原生 WinUI 3 都以 0 errors 建置，清單涵蓋 **346 條固定路線 + 五組動態路線**，完整原生測試 executable 喺 Debug 係 **325/325**、Release 亦係 **325/325**（279 個 core route／package-manager／codec／羅馬數字檢查加 46 個 parser 檢查），而提權、只控制自己 process 嘅 UI Automation shell 亦係 **144/144**。Release PE 審查亦證實 COM descriptor 係零，而且冇 `coreclr`、`hostfxr` 或 `mscoree` import。呢個係遷移里程碑，唔係扮成全功能已移植：未完成功能會顯示明確 pending 頁，而每項對等清單通過之前，受控 app 仍然係權威版本。
...
Current aggregate evidence after the Package Manager cached action-preview slice is **325/325 native tests** and **144/144 native UI Automation checks**; older per-feature counts below are retained as historical slice evidence. · 套件管理快取動作預覽批次之後目前整體證據係**原生測試 325/325**同**原生 UI Automation 144/144**；下面較舊嘅逐功能數字保留作歷史批次證據。
...
**EN —** The complete native executable passes **325/325 in Debug** and **325/325 in Release** (279 core route/package-manager/codec/Roman checks plus 46 parser checks), including the nine cached-Discover-filter cases plus three source/action selection-identity cases. The process-owned elevated UI Automation smoke passes **144/144**, including Discover-filter accessibility/clipping, local re-filtering without a package query, and language-rerender persistence. That elevated smoke verifies routing—including exact Discover/Updates/Installed/Ignored aliases—controls, fail-closed gates and accessibility, not live external manager execution. The separate normal-integrity live smoke is still externally blocked because Windows returned a token that failed the standard-user proof even for an interactive scheduled task registered with `RunLevel=Limited`; no live-query pass is claimed. LowLevel MCP is present on disk at `C:\Users\Administrator\Documents\GitHub\lowlevel-computer-use-mcp`; this Codex session still does not expose a callable LowLevel MCP namespace, but the repo-local `lowlevel-computer-use-cheap.exe` fallback created `WinForgePackageBatchActionAudit`, launched `WinForge.exe` for `package-discover`, `package-updates`, and `package-installed`, listed each headless WinUI window, and captured each HWND. Every inspected client frame was blank/near-uniform, so no PNG is accepted as visual proof. The gallery image below is therefore labelled as a **managed production reference**, never current native visual evidence.

**粵語 —** 完整原生 executable 喺 Debug 係 **325/325**、Release 亦係 **325/325**（279 個 core route／package-manager／codec／羅馬數字檢查加 46 個 parser 檢查），包括九個快取 Discover 篩選專項案例同三個識別來源／動作嘅選擇 identity 案例；只控制自己 process 嘅提升權限 UI Automation smoke 係 **144/144**，包括 Discover 篩選無障礙／裁切、本機重新篩選唔開套件查詢，同轉語言後保留。提升權限 smoke 驗證導覽（包括 Discover／Updates／Installed／Ignored 準確 alias）、控制項、fail-closed 閘同無障礙，唔代表真實外部管理器執行。獨立嘅正常權限 live smoke 仍然受外部環境阻擋：即使互動式排程工作已登記做 `RunLevel=Limited`，Windows 回傳嘅 token 仍然未能通過標準使用者證明，所以唔會聲稱 live query 已通過。LowLevel MCP checkout 存在於 `C:\Users\Administrator\Documents\GitHub\lowlevel-computer-use-mcp`；呢個 Codex session 仍然冇曝露可直接呼叫嘅 LowLevel MCP namespace，但 repo 本機 `lowlevel-computer-use-cheap.exe` fallback 已建立 `WinForgePackageBatchActionAudit`、為 `package-discover`、`package-updates` 同 `package-installed` 開啟 `WinForge.exe`、列出每個無頭 WinUI 視窗並用 HWND 擷取。檢查過嘅 client frame 全部都係空白／接近單色，所以冇 PNG 會被當成視覺證據。所以下面畫廊圖片只係**受控正式版參考**，絕對唔係目前原生版視覺證據。
