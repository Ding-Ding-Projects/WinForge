...
- **Dim-sum startup surprise · 啟動點心驚喜** — eligible launches make one fresh 10% draw, then asynchronously show a local cached image with the catalog's exact English and Traditional Chinese name. The catalog and release asset hosts are allowlisted, the image is cached only under application data, School mode suppresses the surprise, and an unavailable or malformed source simply leaves startup usable. · 合資格啟動會每次重新抽一次 10% 機會，再用非阻塞方式顯示本機 cache 圖片同 catalog 準確英文／繁體中文名。catalog 同 release asset host 有 allowlist，圖片只會 cache 喺 application data；School mode 會停用驚喜，來源唔得或者格式錯就照樣開到 app。
