...
**Hosted release proof · Hosted 發佈證明：** Slugify merge `88672704` passed [native run 29702620758](https://github.com/codingmachineedge/WinForge/actions/runs/29702620758) and published non-draft [`native-v1.0.70`](https://github.com/codingmachineedge/WinForge/releases/tag/native-v1.0.70) with exactly the native setup and versioned native ZIP. Successful site-data [run 29702620742](https://github.com/codingmachineedge/WinForge/actions/runs/29702620742) committed `ef822c1d`; dispatched [run 29702777841](https://github.com/codingmachineedge/WinForge/actions/runs/29702777841) then passed and promoted stable [`native-v1.0.71`](https://github.com/codingmachineedge/WinForge/releases/tag/native-v1.0.71) to Latest for that exact current main SHA, again with exactly two C++ assets. · Slugify merge `88672704` 通過原生 run 29702620758，發佈非 draft `native-v1.0.70`，準確只有 native setup 同版本化 native ZIP。成功 site-data run 29702620742 提交 `ef822c1d`；dispatch run 29702777841 再通過，將嗰個準確 current-main SHA 嘅 stable `native-v1.0.71` 升做 Latest，仍然準確只有兩個 C++ asset。

### 2026-07-19 controlled native Base Converter integration · 2026-07-19 受控原生進位轉換整合

**EN —** `baseconvert` and `module.baseconvert` are now integrated as a genuine C++/WinRT renderer over dependency-free standard-C++ arbitrary-precision logic. It preserves base 2–36 parsing/rendering, grouped binary, signed 64-bit presentation, BigInteger-style bitwise operations, localization/state lifecycle, explicit-only Copy, and managed Unicode `Trim()` diagnostics. Debug and Release builds have 0 errors, combined core suites are **857/857** (Base Converter **15/15**), focused UIA is **14/14**, catalog parity is **346 + 5** with 319 registry and 346 ledger rows, the installer contract passes, and accounting is **36/346** (`36 in-progress / 310 not-started`). The C++-only release boundary is unchanged. The fresh current driver rejected the blank/near-uniform PrintWindow fallback after CopyFromScreen was unavailable; no PNG or process remained, so visual status is honestly `capture-blocked`. See [Native Base Converter](docs/Native-Base-Converter.md).

**粵語 —** `baseconvert` 同 `module.baseconvert` 而家已受控整合成真正 C++/WinRT renderer，同唔靠依賴嘅標準 C++ 任意精度邏輯相連。佢保留 2–36 進制解析／輸出、二進制分組、signed 64-bit 顯示、BigInteger 式 bitwise、語言／狀態 lifecycle、只限明確 Copy 同 managed Unicode `Trim()` 診斷。Debug／Release 0 errors、合併 core 各 **857/857**（Base Converter **15/15**）、專項 UIA **14/14**、catalog parity **346 + 5**（319 registry／346 ledger）、installer contract 通過，計數係 **36/346**（`36 in-progress / 310 not-started`）。只限 C++ release 界線冇變。最新目前 native driver 因 CopyFromScreen 不可用而拒絕空白／近乎單色 PrintWindow fallback；冇 PNG／冇殘留 process，所以 visual 如實係 `capture-blocked`。詳情見[原生進位轉換](docs/Native-Base-Converter.md)。

### 2026-07-20 controlled native ASCII Table integration · 2026-07-20 受控原生 ASCII 表整合

**EN —** `ascii`, `asciitable`, and `module.asciitable` now use a genuine C++/WinRT ASCII Table over the dependency-free standard-C++ `AsciiTable` core. It preserves the default 0–127 range, explicit Latin-1 0–255 extension, control/NBSP distinctions, radix columns, invariant search, virtualized rows, language retention, fresh-route reset, and explicit-only raw-character Copy. Debug and Release builds have 0 errors; both cores are **878/878** (ASCII Table **21/21**), focused UIA is **16/16**, catalog parity is **346 + 5**, and the installer contract passes. Accounting is **37/346** fixed routes (`37 in-progress / 309 not-started`). The broad aggregate stopped after the observed pre-existing `wordfreq` launch stall and is not claimed as a full-shell pass. LowLevel tools are unavailable and the driver rejected a blank frame, so visual evidence remains `capture-blocked`; publishing remains C++-only. See [Native ASCII Table](docs/Native-Ascii-Table.md).

**粵語 —** `ascii`、`asciitable` 同 `module.asciitable` 而家用真正 C++/WinRT ASCII 表同唔靠依賴嘅標準 C++ `AsciiTable` core。保留預設 0–127、明確 Latin-1 0–255、控制碼／NBSP 分別、進制欄、invariant 搜尋、虛擬化列、語言保留、新 route 重設同只限明確 raw-character Copy。Debug／Release 0 errors、core 各 **878/878**（ASCII Table **21/21**）、專項 UIA **16/16**、catalog parity **346 + 5** 同 installer contract 通過；固定 route 係 **37/346**（`37 in-progress / 309 not-started`）。廣泛 aggregate 喺觀察到既有 `wordfreq` launch 卡住後已停止，唔當 full-shell pass。LowLevel 工具未可用、driver 拒絕空白畫面，visual 保持 `capture-blocked`；發佈繼續只限 C++。詳情見[原生 ASCII 表](docs/Native-Ascii-Table.md)。
...
**粵語 —** 拼讀字母表（`module.phonetic`）、文字方框／橫幅（`module.boxtext`）同 HTML 實體（`module.htmlentities`）而家都有專用 C++/WinRT renderer，背後共用一個可測試嘅標準 C++ `ReferenceText` core。拼讀頁保留三種字母表、數字、大寫／標點選項、同 managed 版一致嘅 UTF-16 code-unit 列同朗讀輸出；文字方框保留八種邊框／註解風格、三種對齊、有界 padding、標題、多行／tab-aware 排版同 Unicode 顯示闊度；HTML 實體保留具名／數字編碼解碼、可選非 ASCII 編碼、無效 UTF-16 安全、managed 相容掃描同本機 50 項雙語參考。三項都只喺本機做，只有明確 Copy 先會寫剪貼簿。

**Current local evidence; visual capture blocked · 目前本機證據；視覺擷取受阻：** `HasNativeRenderer` and the provisional ledger account for **30/346 fixed routes**, leaving **316 fixed routes** plus five dynamic route families; the ledger is **30 `in-progress` / 316 `not-started`**. Fresh native Debug and Release x64 builds each exit 0 with 0 errors, Debug and Release core executables each pass **759/759**, catalog parity passes 346 fixed routes plus five dynamic families, focused reference-text UI Automation passes **29/29** across all eight aliases, and the full native shell passes **417/417 (417 passed, 0 failed)**. LowLevel Computer Use MCP is not callable in this Codex session. A fresh post-integration `htmlentities` repository-driver retry found `CopyFromScreen` unavailable and rejected a blank/near-uniform `PrintWindow` client. No PNG was created or retained, no WinForge process remained, and no canonical screenshot changed; all three routes remain honestly `capture-blocked` and `in-progress`. · Renderer 計 **30/346**，仲有 **316** 條固定 route 同五組動態家族；ledger 係 **30 `in-progress` / 316 `not-started`**。最新 Debug／Release 0 errors、兩個 core 各 **759/759**、catalog parity、八個 alias UIA **29/29** 同完整 native shell **417/417（417 passed、0 failed）** 已通過。今次 session 冇 LowLevel MCP；整合後最新 `htmlentities` repository-driver 重試發現 `CopyFromScreen` 唔可用，並拒絕空白／近乎單色 `PrintWindow` client。冇建立或保留 PNG、冇殘留 WinForge process、冇改 canonical 截圖，所以三條 route 如實保持 `capture-blocked` 同 `in-progress`。

### 2026-07-19 native text-analysis collation wave · 原生文字分析排序批次

**EN —** Text Statistics (`module.textstats`), Word Frequency (`module.wordfreq`), and String Compare (`module.stringcompare`) now have dedicated C++/WinRT renderers backed by one testable standard-C++ `TextAnalysis` core. Text Statistics preserves managed UTF-16/CJK-aware word, sentence, paragraph, average-length, reading/speaking-time, Flesch-readability, and top-ten-word contracts and stays entirely local. Word Frequency supports word, bigram, and Unicode-scalar modes, managed options and stop-word behavior, current-culture ordering, count/bar/percentage rows, and exact CSV; it writes the clipboard only after explicit Copy. String Compare preserves normalization plus UTF-16 metrics, optimal-string-alignment distance, Jaro–Winkler, longest common substring/subsequence, and the managed greater-than-2,000 guard; its exact greedy Jaro matching is optimized subquadratically, retained route state is released on exit, and only explicit Copy changes the clipboard.

**粵語 —** 文字統計（`module.textstats`）、詞頻統計（`module.wordfreq`）同字串相似度（`module.stringcompare`）而家都有專用 C++/WinRT renderer，背後共用一個可測試嘅標準 C++ `TextAnalysis` core。文字統計保留 managed 版 UTF-16／CJK 字詞、句子、段落、平均長度、閱讀／朗讀時間、Flesch 可讀性同頭十個常用詞合約，全部只喺本機運算。詞頻支援單詞、bigram 同 Unicode scalar 模式、managed 選項／停用詞、依目前文化排序、數量／條形／百分比列同準確 CSV；只有明確 Copy 先會寫剪貼簿。字串比較保留正規化、UTF-16 指標、OSA distance、Jaro–Winkler、最長公共子串／子序列同超過 2,000 字元 guard；準確 greedy Jaro 匹配已優化到 subquadratic，離開 route 會清掉保留狀態，亦只有明確 Copy 會改剪貼簿。

**Historical c6 collation checkpoint · 歷史 c6 排序檢查點：** Commit `c6f8a24d52e4949596efd58e070e0601a2939511` aligns Word Frequency `CurrentCultureIgnoreCase` with managed .NET width/kana tailoring: fullwidth case folds while width/kana remain significant, canonical-equivalent terms keep stable order, and a shared initialized collator removes per-comparison serialization. The named total remains assistive-technology queryable without a per-edit polite live announcement; attribution is in `THIRD-PARTY-NOTICES.txt`. At that checkpoint, the ledger was **27/346 fixed routes**, Debug and Release core each passed **717/717** including Text Analysis **87/87**, focused UIA passed **40/40**, and the shell passed **388/388**. The Reference Text wave above advances accounting to **30/346**; all six text routes remain honestly `capture-blocked` and `in-progress`. · Commit `c6f8a24d52e4949596efd58e070e0601a2939511` 對齊詞頻 `CurrentCultureIgnoreCase` 同 managed .NET 闊度／假名 tailoring：fullwidth case 會折疊，但闊度／假名仍有分別；canonical-equivalent 詞語保持穩定次序，而共用已初始化 collator 移除每次比較嘅序列化。命名 total 仍可畀輔助科技查詢，但每次修改唔再發 polite live announcement；歸屬資料喺 `THIRD-PARTY-NOTICES.txt`。嗰個 checkpoint 時 ledger 係 **27/346**、core 各 **717/717**（Text Analysis **87/87**）、UIA **40/40**、shell **388/388**；上面 Reference Text 批次已將計數推進到 **30/346**，六條文字 route 都如實保持 `capture-blocked` 同 `in-progress`。

### Earlier 2026-07-18 native line-processing wave · 2026-07-18 較早原生行文字處理批次
...
**EN —** Line Tools (`module.linetools`), Line Sort & Dedupe (`module.textsort`), and Text Wrap (`module.textwrap`) now have dedicated C++/WinRT renderer source backed by one testable standard-C++ Line Processing core. The shared core preserves the managed UTF-16 line/count contracts, numbering and cleanup transforms, ordinal and natural sorting, first-win deduplication, cryptographic shuffling, paragraph wrap/reflow, prefixing, and hanging indent. At this earlier checkpoint, `HasNativeRenderer` covered **24/346 fixed routes**, leaving **322 fixed routes** explicitly pending plus five dynamic route families. Native Debug and Release builds each exited 0; both core executables passed **630/630**, including **70/70** focused Line Processing contracts. Catalog parity passed for all 346 fixed routes plus five dynamic families, and the managed solution build exited 0 with 0 errors.

**粵語 —** 行工具（`module.linetools`）、行排序同去重（`module.textsort`）同文字換行（`module.textwrap`）而家都有專用 C++/WinRT renderer source，背後共用一個可測試嘅標準 C++ Line Processing core。共用 core 保留 managed 版 UTF-16 分行／統計合約、編號同清理轉換、ordinal／自然排序、保留第一項去重、密碼學打亂、按段落換行／重排、加前綴同懸掛縮排。喺呢個較早檢查點，`HasNativeRenderer` 涵蓋 **24/346 條固定 route**，仲有 **322 條固定 route** 同五組動態 route 家族待完成。原生 Debug 同 Release build 各自 exit 0，兩個 core executable 各自通過 **630/630**，包括 Line Processing **70/70**；catalog parity 亦通過全部 346 條固定 route 同五組動態家族，managed solution build exit 0、0 errors。

**Focused validation green; visual capture blocked · 專項驗證通過；視覺擷取受阻：** Focused `-LineRoutesOnly` UI Automation passes **42/42** across all seven registered aliases, and the full owned native shell smoke passes **348/348** with exit 0. LowLevel Computer Use MCP 1.28.1 launched the three canonical entry commands `--page lines`, `--page textsort`, and `--page textwrap` on separate private desktops and resolved one **852×880** window per launch. Every `PrintWindow` capture returned `rendered_ok`, but inspection found composition-white invalid frames. A compositor-backed retry failed with Win32 error 5 before monitor capture, and the required run-winforge driver rejected blank or near-uniform fallback frames for all three routes. All invalid images were deleted, the artifact directory is empty, no canonical screenshot was replaced, and no visual pass is claimed. Hosted release-per-push proof and Git integration/remote proof remain pending. · `-LineRoutesOnly` 專項 UI Automation 喺全部七個 registered alias 通過 **42/42**，完整自有原生 shell smoke 亦以 exit 0 通過 **348/348**。LowLevel Computer Use MCP 1.28.1 喺獨立 private desktop 開過三個 canonical entry command：`--page lines`、`--page textsort` 同 `--page textwrap`；每次 launch 都解析到一個 **852×880** window。`PrintWindow` 擷取雖然回報 `rendered_ok`，但檢查後係 composition-white 無效畫面；compositor-backed retry 未開始 monitor capture 就因 Win32 錯誤 5 失敗，而必需 run-winforge driver 亦拒絕三頁嘅空白／近乎單色 fallback。無效圖已刪除、artifact directory 已清空、冇替換 canonical 截圖，亦唔聲稱 visual pass。每次 push 嘅 hosted release 證明同 Git 整合／remote 證明仍待完成。

### Earlier 2026-07-18 native design-utility wave · 較早原生設計工具批次

**EN —** Text Diff (`module.textdiff`), Aspect Ratio (`module.aspectratio`), and CSS Unit Converter (`module.cssunits`) now have dedicated genuine C++/WinRT renderers backed by testable standard-C++ cores. `HasNativeRenderer` therefore covers **21/346 fixed routes**, with **325 fixed routes** still explicitly pending; the catalog also retains five dynamic route families. Debug and Release native core suites each pass **560/560**; the Design Tools suite passes **94/94**, the focused utility UI Automation shell passes **39/39**, and the full owned shell remains **300/300**. Text Diff contributes **27/27** focused contracts, including the exact 6,000,000-cell LCS guard and invariant Unicode casing. These tools are local and deterministic: they never mutate Windows, and their only side effect is an explicit operator-requested clipboard Copy.

**粵語 —** 文字差異比對（`module.textdiff`）、長寬比計算（`module.aspectratio`）同 CSS 單位換算（`module.cssunits`）而家都有專用真正 C++/WinRT renderer，背後用可測試嘅標準 C++ core。`HasNativeRenderer` 因而涵蓋 **21/346 條固定 route**，另外 **325 條固定 route** 仍然明確待完成；目錄亦繼續涵蓋五組動態 route 家族。Debug 同 Release 原生 core suite 各自通過 **560/560**；Design Tools suite 通過 **94/94**，工具專項 UI Automation shell 通過 **39/39**，完整自有 shell 亦保持 **300/300**。文字差異比對有 **27/27** 個專項合約，包括準確 6,000,000-cell LCS 上限同 invariant Unicode 大小寫。三項工具都只喺本機做確定性運算，唔會改 Windows；唯一副作用係操作員明確撳 Copy 先寫入剪貼簿。

**Visual evidence · 視覺證據：** LowLevel Computer Use MCP 1.28.1 and the repository driver separately launched all three pages, but every inspected client capture was uniformly white because WinUI composition is unavailable in this desktop session. Blank images were discarded and no canonical screenshot was replaced; the exact bilingual record is in `docs/wiki/Screenshots.md`, and all three routes are honestly `capture-blocked`. · LowLevel Computer Use MCP 1.28.1 同 repository driver 都分別開過三頁，但呢個 desktop session 冇 WinUI composition，所以每張檢查過嘅 client 擷取都係一致白色。空白圖已丟棄，冇替換 canonical 截圖；準確雙語紀錄喺 `docs/wiki/Screenshots.md`，三條 route 如實係 `capture-blocked`。
...
**Release provenance · 版本來源：** Every native release tag is pinned to the immutable workflow commit, so a concurrent `main` update cannot relabel artifacts built from an earlier SHA. · 每個原生 release 標籤都會固定去不可變嘅 workflow commit，所以就算 `main` 同時更新，都唔會將較早 SHA 建置嘅 artifacts 改標。

**Current native-only CI proof · 目前只限原生 CI 證明：** The earlier pending-language below is historical. `1732af0b458454f144d1ac32be222b9e9015e5c5` passed [run 29694567950](https://github.com/codingmachineedge/WinForge/actions/runs/29694567950), proving staged site-data normalization and publishing native-only prerelease `native-v1.0.55`. Main integration `03b1e66f21b07c28fe2bd0b30ef60fe4af134e59` passed [run 29695569334](https://github.com/codingmachineedge/WinForge/actions/runs/29695569334) and published stable, non-draft [native-v1.0.57](https://github.com/codingmachineedge/WinForge/releases/tag/native-v1.0.57) at the exact SHA. `/releases/latest` resolved to that same release with exactly `WinForge-Native-Setup.exe` and `WinForge-native-x64-1.0.57.zip`, never a managed app. The paired [site-data run 29695569335](https://github.com/codingmachineedge/WinForge/actions/runs/29695569335) staged first and successfully logged `No change to design/winforge-data.js.` · 以下「pending」字樣只係歷史背景。`1732af0b458454f144d1ac32be222b9e9015e5c5` 通過 run 29694567950，證明 site-data 先 stage 再處理正規化無變更，並發佈 native-only prerelease `native-v1.0.55`。Main integration `03b1e66f21b07c28fe2bd0b30ef60fe4af134e59` 通過 run 29695569334，以準確 SHA 發佈 stable／non-draft `native-v1.0.57`。`/releases/latest` 同樣解析到佢，而且只有 `WinForge-Native-Setup.exe` 同 `WinForge-native-x64-1.0.57.zip`，冇 managed app。配對嘅 site-data run 29695569335 成功記錄 `No change to design/winforge-data.js.`

**EN —** The native shell now measures pages to the available viewport, keeps a vertical overflow path, and enables horizontal scrolling as a last-resort accessibility escape hatch so bilingual headings, action rows, and the Package Manager toolbar are not silently clipped. [`.github/workflows/native-release.yml`](.github/workflows/native-release.yml) is the sole release publisher. It builds and tests the C++ Release x64 runtime, packages one clean runtime, compiles and smoke-tests `WinForge-Native-Setup.exe`, and publishes exactly two native assets: the portable ZIP and installer. Every push publishes a native-only release: the current `main` tip is stable, while every noncurrent branch or tag ref is a prerelease. For fresh current main, the target applies Latest + stable + non-draft and verifies `/releases/latest` resolves to the exact native SHA with exactly two assets; noncurrent runs restore a verified current-main stable native candidate if GitHub points to managed or invalid Latest. Official Node 24 actions are `checkout@v7`, `upload-artifact@v7`, `download-artifact@v8`, and `setup-msbuild@v3`. Hosted proof is complete: `03b1e66f` / [run 29695569334](https://github.com/codingmachineedge/WinForge/actions/runs/29695569334) published stable `native-v1.0.57` with no managed app. · 原生 shell 而家會按可用 viewport 量度頁面，保留垂直溢出路徑，並以水平捲動作最後嘅無障礙保底。[`.github/workflows/native-release.yml`](.github/workflows/native-release.yml) 係唯一 release publisher，只發佈可攜 ZIP 同安裝程式兩個原生 asset。每次 push 都會發佈 native-only release：而家 `main` tip 係 stable，所有非 current branch 或 tag ref 係 prerelease。對最新 current main，目標會明確套用 Latest + stable + non-draft，再驗證 `/releases/latest` 指向同一個準確原生 SHA 同兩個 asset；noncurrent run 見到 managed 或無效 Latest 就恢復已核實 current-main stable 原生 candidate。官方 Node 24 actions 係 `checkout@v7`、`upload-artifact@v7`、`download-artifact@v8` 同 `setup-msbuild@v3`。Hosted 證明已完成：`03b1e66f`／run 29695569334 發佈 stable `native-v1.0.57`，冇 managed app。

**Completed text-analysis release proof · 已完成文字分析版本證明：** Feature commit `fc2b76e52171e4f81ab1d15f9fb1da5818791171` passed branch run [29673079883](https://github.com/codingmachineedge/WinForge/actions/runs/29673079883) and published exact-SHA prerelease [native-v1.0.43](https://github.com/codingmachineedge/WinForge/releases/tag/native-v1.0.43). Merge commit `f7a9eec44aeffdf829f5c07f5eeb364f08a7677f` passed `main` run [29673310778](https://github.com/codingmachineedge/WinForge/actions/runs/29673310778) and published stable exact-SHA [native-v1.0.44](https://github.com/codingmachineedge/WinForge/releases/tag/native-v1.0.44). Each release has exactly the native setup executable and portable ZIP. Independent download verification matched the digests and found 292 ZIP entries, 48 PE files, zero CLR/apphost or forbidden managed/build artifacts, and an AMD64 PE32+ `WinForge.exe`. · 功能 commit `fc2b76e52171e4f81ab1d15f9fb1da5818791171` 通過 branch run [29673079883](https://github.com/codingmachineedge/WinForge/actions/runs/29673079883)，發佈 exact-SHA prerelease [native-v1.0.43](https://github.com/codingmachineedge/WinForge/releases/tag/native-v1.0.43)；merge commit `f7a9eec44aeffdf829f5c07f5eeb364f08a7677f` 通過 `main` run [29673310778](https://github.com/codingmachineedge/WinForge/actions/runs/29673310778)，發佈 stable exact-SHA [native-v1.0.44](https://github.com/codingmachineedge/WinForge/releases/tag/native-v1.0.44)。每個 release 都準確只含原生 setup 同 portable ZIP。獨立下載驗證確認 digest 吻合、ZIP 有 292 個 entry 同 48 個 PE file、CLR／apphost／禁止 managed 或 build artifact 全部係零，而 `WinForge.exe` 係 AMD64 PE32+。

**Latest-channel correction · Latest channel 修正：** `gh release create --latest` did not move `/releases/latest` from historical managed `v1.0.256`. A later bare `gh release edit native-v1.0.44 --latest` was also insufficient: release 44 was observed as `prerelease=true` at that edit timestamp, so Latest fell back to managed `v1.0.256`. Explicit `gh release edit native-v1.0.44 --latest --prerelease=false --draft=false` restored stable/non-draft `native-v1.0.44`; `/releases/latest` then resolved exactly to merge `f7a9eec44aeffdf829f5c07f5eeb364f08a7677f` with two native assets. · `gh release create --latest` 冇將 `/releases/latest` 由歷史 managed `v1.0.256` 移走；之後只用 `gh release edit native-v1.0.44 --latest` 亦唔足夠，release 44 喺該 edit 時間被觀察到係 `prerelease=true`，Latest 因而跌返去 managed `v1.0.256`。明確執行 `gh release edit native-v1.0.44 --latest --prerelease=false --draft=false` 先恢復 stable／non-draft `native-v1.0.44`；`/releases/latest` 隨後準確指向 merge `f7a9eec44aeffdf829f5c07f5eeb364f08a7677f`，並有兩個原生 asset。

**Historical corrective branch proof · 歷史修正分支證明：** Commit `53da21446a5c2cded97e1387f3e36f557770a3c5` passed [run 29673965179](https://github.com/codingmachineedge/WinForge/actions/runs/29673965179) and published [native-v1.0.47](https://github.com/codingmachineedge/WinForge/releases/tag/native-v1.0.47) with official Node 24 actions and exactly two native assets, but its then-current noncurrent check was insufficient. That historical limitation is superseded by `1732af0b` and main `03b1e66f` / run `29695569334`, which now prove automatic enforcement. · 歷史 commit `53da2144` 當時只證明 Node 24／two-asset branch 發佈；上面 `1732af0b` 同 main `03b1e66f`／run 29695569334 而家已證明自動強制。
