...
### 2026-07-18 native text-analysis wave · 原生文字分析批次
...
**Current evidence; exhaustive shell green and visual capture blocked · 目前證據；完整 shell 通過、視覺擷取受阻：** `HasNativeRenderer` and the provisional ledger now account for **27/346 fixed routes**, leaving **319 fixed routes** plus five dynamic route families; the ledger is **27 `in-progress` / 319 `not-started`**. Native Debug and Release builds each exit 0, both core executables pass **714/714** including Text Analysis **84/84**, managed tests pass **7/7**, seven-alias focused UIA passes **40/40**, the DPI-aware utility shell passes **39/39**, the final exhaustive native shell passes **388/388 (388 passed, 0 failed)**, catalog parity covers 346 fixed routes plus five dynamic families, and the managed build exits 0 with 0 errors. LowLevel MCP 1.28.1 launched `textstats`, `wordfreq`, and `similarity` on separate private desktops and resolved 1980×1320 native windows, but every inspected 1958×1264 client was exactly one white RGB color; the repository driver independently rejected each blank/near-uniform fallback with exit 1. All six invalid MCP PNGs were removed, no driver image remained, and no canonical screenshot changed. All three routes therefore remain honestly `capture-blocked` and `in-progress`. · Renderer 計 **27/346**，仲有 **319** 條固定 route 同五組動態家族；714/714、84/84、7/7、40/40、39/39、完整 shell **388/388（388 passed、0 failed）**、catalog parity 同 managed 0-error build 已通過。LowLevel MCP 1.28.1 開過三頁並解析 1980×1320 原生視窗，但檢查過嘅 1958×1264 client 每張都只有一種純白 RGB；repository driver 亦分別拒絕空白／近乎單色 fallback。六張無效 MCP PNG 已刪、driver 冇留無效圖、冇改 canonical 截圖，所以三條 route 如實保持 `capture-blocked` 同 `in-progress`。
...
**EN —** The native shell now measures pages to the available viewport, keeps a vertical overflow path, and enables horizontal scrolling as a last-resort accessibility escape hatch so bilingual headings, action rows, and the Package Manager toolbar are not silently clipped. [`.github/workflows/native-release.yml`](.github/workflows/native-release.yml) is the sole release publisher. It builds and tests the C++ Release x64 runtime, packages one clean runtime, compiles and smoke-tests `WinForge-Native-Setup.exe`, and publishes exactly two native assets: the portable ZIP and installer. Branch pushes publish native-only prereleases. For fresh current main, the amended target explicitly applies Latest + stable + non-draft and verifies `/releases/latest` resolves to that exact native SHA with exactly two assets. Every release run enforces the invariant; a noncurrent run restores a verified current-main stable native candidate if GitHub points to managed or invalid Latest. Official Node 24 actions are `checkout@v7`, `upload-artifact@v7`, `download-artifact@v8`, and `setup-msbuild@v3`. Exact amended branch/main hosted proof remains pending. · 原生 shell 而家會按可用 viewport 量度頁面，保留垂直溢出路徑，並以水平捲動作最後嘅無障礙保底。[`.github/workflows/native-release.yml`](.github/workflows/native-release.yml) 係唯一 release publisher，只發佈可攜 ZIP 同安裝程式兩個原生 asset。Branch push 發佈 native-only prerelease。對最新 current main，修正版目標會明確套用 Latest + stable + non-draft，再驗證 `/releases/latest` 指向同一個準確原生 SHA 同兩個 asset；每個 release run 都要守住呢個 invariant，而 noncurrent run 見到 managed 或無效 Latest 就恢復已核實 current-main stable 原生 candidate。官方 Node 24 actions 係 `checkout@v7`、`upload-artifact@v7`、`download-artifact@v8` 同 `setup-msbuild@v3`。修正版 branch／main 嘅準確 hosted 證明仍待完成。
...
**Corrective branch proof, invariant still pending · 修正分支證明；invariant 仍待完成：** Commit `53da21446a5c2cded97e1387f3e36f557770a3c5` passed [run 29673965179](https://github.com/codingmachineedge/WinForge/actions/runs/29673965179) and published [native-v1.0.47](https://github.com/codingmachineedge/WinForge/releases/tag/native-v1.0.47) with the official Node 24 actions and exactly two native assets. Its old noncurrent check was insufficient: the successful run observed managed `v1.0.256` as Latest without repairing or rejecting it. This proves Node 24/two-asset branch publication, not the automatic invariant. Exact amended branch/main run IDs, tags, and SHAs remain pending. · Commit `53da21446a5c2cded97e1387f3e36f557770a3c5` 通過 [run 29673965179](https://github.com/codingmachineedge/WinForge/actions/runs/29673965179)，用官方 Node 24 actions 發佈 [native-v1.0.47](https://github.com/codingmachineedge/WinForge/releases/tag/native-v1.0.47)，準確只有兩個原生 asset。但舊 noncurrent check 唔夠嚴格，run 成功時仍見到 managed `v1.0.256` 做 Latest，亦冇修復或拒絕。呢個只證明 Node 24／two-asset branch 發佈，未證明自動 invariant；修正版 branch／main 嘅準確 run ID、tag 同 SHA 仍待完成。
