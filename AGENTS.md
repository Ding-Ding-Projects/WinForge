...
- **Prepare the runnable artifact (do not launch it directly):** a plain Debug build is framework-dependent and only shows an *"install .NET"* dialog here — you must **publish self-contained**:
  `dotnet publish WinForge.csproj -c Debug -r win-x64 --self-contained true -p:Platform=x64 -p:WindowsAppSDKSelfContained=true`
- **Only run it headlessly — use the skill `.agents/skills/run-winforge/`** (SKILL.md + driver.ps1): builds-if-needed, creates a unique non-input Win32 desktop, launches any page there via `--page <alias>`, and screenshots only that owned off-screen window. The driver uses native `CreateDesktop` + `CreateProcess`, never switches the input desktop, and must not let WinForge, a terminal, or any helper process steal focus. e.g.
  `powershell -ExecutionPolicy Bypass -File .agents/skills/run-winforge/driver.ps1 -Page reactor -Out shot.png`
- **LowLevel MCP GUI/a11y/clipping checks:** the local LowLevel Computer Use MCP checkout is expected at `C:\Users\Administrator\Documents\GitHub\lowlevel-computer-use-mcp`. When its tools are exposed in Codex, run changed GUI pages in a dedicated off-screen desktop: `create_headless_desktop` → `launch_on_headless_desktop` → `list_headless_windows` → drive/check the live window and capture `screenshot` with the fresh `hwnd` → close the app/window → `close_headless_desktop`. Never reuse stale HWNDs or leave the desktop handle open. If the MCP is present on disk but not callable in the current tool list, record that exact blocker and fall back to the repository driver, which creates and destroys its own off-screen desktop, plus the relevant managed test/smoke scripts; do not claim LowLevel MCP evidence. Agent automation must never launch WinForge with `Start-Process` or directly on the interactive desktop.
- **Reactor/dependent headless tests:** `dotnet run --project tests/ReactorSim.Tests -c Debug` (**73/73**). It returns **0 only when every scenario passes** and **1 when any scenario fails or throws**, so it is a CI gate; use `-- --verify-exit-code-contract` for the fast contract self-test.
- The app is NOT a Start-menu app. Every agent launch and capture must remain headless/off-screen: use LowLevel on a dedicated headless desktop or the repository driver's unique non-input desktop. DEBUG builds render the owned live WinUI visual tree, and only validated HWND-targeted `PrintWindow` against that owned desktop is a fallback. Never call `SwitchDesktop`, `SetForegroundWindow`, or `CopyFromScreen`; never show a helper console or app window on the user's interactive desktop.
...
- **Self-updates:** auto-update is normal-integrity only. `WinForgeUpdater` verifies GitHub's SHA-256 asset digests for `Setup.exe`, `RELEASES`, and the versioned Squirrel package, stages the unsigned Squirrel update outside the install directory, and hands off only after the user chooses restart; persistent logs remain bounded and the validated launcher relaunches the app.
...
- Services: `FuelFactoryService`, `NuclearWasteService`, `WaterTreatmentService`, `ReactorStatusApiService` (+ `Sdk/ReactorStatusClient.cs`), `ReactorSystemLinkService`, `ReactorHomeAssistantMirror`, `PersistenceService`, `AwakeService`, `DieselVoucherService` (consumer half of the Material Cookie Clicker diesel-voucher exchange — cookie-bought fuel for the safety EDGs; see `docs/features/reactor-loads/diesel-voucher-exchange.md`).
...
- **Visual changes require fresh evidence.** Launch every changed page only through `.agents/skills/run-winforge/driver.ps1` on its owned off-screen desktop, capture a high-detail current screenshot, inspect it, and replace the old canonical screenshot in `docs/` and any corresponding GitHub Pages/wiki image. Do not retain stale screenshots for a changed page. If capture is blocked, document the exact blocker in the task handoff and do not claim visual verification; a visible interactive fallback is not evidence.
