...
for ($index = 0; $index -lt $batch.Count; $index++) {
    ...
    $results += [pscustomobject]@{
        ...
        evidence = if ($retried) {
            "launch-only driver; initial exit=$($attempts[0].exitCode) at $($attempts[0].waitMs)ms; retry exit=$($finalAttempt.exitCode) at $($finalAttempt.waitMs)ms; log=$([System.IO.Path]::GetFileName($logPath))"
        }
        else {
